...
| `verify-i18n.mjs` | 六语系 i18n 全量门禁：key parity（缺键=ERROR / 孤儿键=WARNING）、叶子类型一致、ICU 占位符变量一致、翻译壳检测（`collectTranslationShells` + 豁免见 `i18n-shell-allowlist.json`）、glossary forbidden（de/ko，`i18n-glossary.json`）、异常哨兵；另含 SSR shell/deferred namespace 门禁 + 关键 namespace keys（`pretest` + CI） |
