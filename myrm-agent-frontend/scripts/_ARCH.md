...
| `check-orphan-modules.mjs` | 孤儿模块门禁：扫描 `src/` 下所有源码模块（`app/` 路由页与生成物除外），解析真实模块说明符（`import`/`export-from`/动态 `import()`，含 barrel `index.ts` 与 `next/dynamic`）判定「无任何生产代码引用」，拦截测试绿但入口不可达的孤儿（测试 import 被测文件，孤儿不会让套件变红）；按文件名子串匹配会命中同名标识符产生大量误报，故按模块解析判定；扫描数为 0 时 fail-closed；`scripts/ci/orphan_modules_baseline.txt` 登记存量孤儿，仅拦截新增且对已修复条目报 drift；`@orphan-ok <理由>` 注释可显式豁免；`--ratchet` 重写基线（`pretest` 前置） |
