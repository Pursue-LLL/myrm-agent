...
| `check_file_line_budget.py` | 单文件 400 行**本地自查工具**（非 CI 阻断；超限台账 `ci/file_line_budget_baseline.txt`，`--write-baseline` 维护）。强制阻断已移除：行数只影响可读性，不影响运行正确性，接入 CI 会把「文档/风格」与「真 bug」混在同一道红灯里 |
...
| `ci/file_line_budget_baseline.txt` | 400 行自查的存量超限台账（仅本地自查读取，非 CI 阻断） |
...
| `verify-i18n-references.mjs` | 代码引用 → en.json（SSOT）键存在性门禁（9l）：静态扫描 `src/**\/*.{ts,tsx}`，提取 `useTranslations('ns')`/`useTranslations()`/`getTranslations({namespace})` 绑定的 `t('key')` 调用，按「最近前驱同名绑定」归因命名空间后逐一 resolve 到 en.json；调用位于同名绑定声明之前（helper 前置定义、t 由组件体传入，如 PushNotificationCard 的 `stateLabel`）时退化为「同名绑定命名空间并集」验证（任一候选存在即通过）；无同名绑定文件的盲区治理（props/参数传入 t 的文件自身无 useTranslations 绑定，直接跳过会漏检根 t('error') 类缺失）：先解析各文件 import/export-from 语句构建真实「被导入 → 引用方」映射（相对路径 + @/ 别名，避免同名 basename 串链），再对每个调用方做「调用点实参推导」——若调用方把翻译绑定作为函数实参或 JSX 属性值传给当前模块（`resolveScopeNote(..., tHumanize)`、`<Comp t={t} />`）则直接采用该绑定命名空间（比按同名 `t` 归因精确，防止 scopeNote 的 t 实参 tHumanize→humanize 被误归因到 toolApproval），两层失效再沿 import 链向上 **BFS 递归**（含根 '' 候选；seen 防环、无固定深度上限，兼容 DoctorPanel→AdvancedPanels→Panels→MemoryCommandCenter 的 4 层 helper 转发链，避免 depth 截断漏检）；任一候选命名空间存在该键即通过，链断裂跳过；动态命名空间绑定（`useTranslations(<变量>)`，如 SchemaForm 的 translationNamespace prop、ThemePackageImportPreview 的 translationNamespace prop）运行时命名空间由调用方决定，无法静态归因，与动态 key 同理跳过（不得递归继承，避免误归因到调用方其他翻译绑定）；`showI18nToast` 的 `titleKey`/options 中 `descriptionKey`/`action.label` 为完整键引用，独立于命名空间归因直接校验（正则锚定在 `showI18nToast(` 之后避免误抓普通对象属性；三元 descriptionKey/action.label `cond ? 'a' : 'b'` 由专用正则提取两个分支键，比较操作数如 `=== 'no_boards'` 位于 `?` 前不误报）；所有 `t()` 调用一律纳入检查——`{ fallback: 'x' }` 只是 next-intl 插值对象而非兜底（use-intl 的 translateFn 只有第 4 位置参数才是 fallback，且 TS 类型签名禁止该用法），不得因假 fallback 跳过（否则漏检 Agent.fleet.builtin 类真实缺失）；动态/模板 key、测试文件与构建产物跳过；补 verify-i18n.mjs 的测量盲区——后者只校验 locale 间 parity/纯净性，无法发现「组件引用但 SSOT 不存在」的键（运行时无 getMessageFallback 直接显示原始 key = MISSING_MESSAGE）；接入 `verify:i18n`（`pretest` + CI 继承） |
...
| `check-orphan-components.mjs` | 孤儿组件门禁：扫描 `src/components/` 下 PascalCase 组件，解析真实模块说明符（`import`/`export-from`/动态 `import()`，含 barrel `index.ts` 与 `next/dynamic`）判定「无任何生产代码引用」，拦截测试绿但入口不可达的组件（测试 import 被测文件，孤儿不会让套件变红）；按文件名子串匹配会命中同名标识符产生大量误报，故按模块解析判定；组件数为 0 时 fail-closed；`scripts/ci/orphan_components_baseline.txt` 登记存量孤儿，仅拦截新增且对已修复条目报 drift；`@orphan-ok <理由>` 注释可显式豁免；`--ratchet` 重写基线（`pretest` 前置） |
| `ci/orphan_components_baseline.txt` | 孤儿组件存量基线（相对 myrm-agent-frontend 路径，仅登记门禁上线前已存在项；新增孤儿由门禁拦截，条目不再孤儿会被报 drift 需移除） |
