...
| `verify-i18n-references.mjs` | 代码引用 → en.json（SSOT）键存在性门禁（9l）：静态扫描 `src/**\/*.{ts,tsx}`，提取 `useTranslations('ns')`/`useTranslations()`/`getTranslations({namespace})` 绑定的 `t('key')` 调用与 `showI18nToast`/`descriptionKey`/`action.label` 完整键引用，按「最近前驱同名绑定」归因命名空间后逐一 resolve 到 en.json；补 verify-i18n.mjs 的测量盲区——后者只校验 locale 间 parity/纯净性，无法发现「组件引用但 SSOT 不存在」的键（运行时无 getMessageFallback 直接显示原始 key = MISSING_MESSAGE）；跳过带 `fallback:` 参数的调用（next-intl 兜底不报缺失）、动态/模板 key、测试文件与构建产物；`SKIP_KEYS` 临时豁免并行开发中的键（键补上后自动失效）；接入 `verify:i18n`（`pretest` + CI 继承） |
