...
export async function registerSubscription(subscription: PushSubscription, userAgent: string = ''): Promise<string> {
...
export async function sendTestPush(): Promise<number> {
  ...
  if (!res.ok) {
    throw new Error(`Failed to send test push: ${res.status}`);
  }
