...
describe('parseContentDispositionFilename', () => {
  it('parses RFC 5987 UTF-8 filename correctly', () => {
    const header = "attachment; filename=\"session_test.zip\"; filename*=UTF-8''session_%E6%B5%8B%E8%AF%95_123.zip";
    expect(parseContentDispositionFilename(header, 'fallback.zip')).toBe('session_测试_123.zip');
  });

  it('parses standard ASCII filename when RFC 5987 is absent', () => {
    const header = 'attachment; filename="session_archive.zip"';
    expect(parseContentDispositionFilename(header, 'fallback.zip')).toBe('session_archive.zip');
  });
  ...
  it('uses fallback when header is null or malformed', () => {
    ...
    expect(parseContentDispositionFilename('attachment', 'default.zip')).toBe('default.zip');
  });
});
...
describe('exportSessionZipPack', () => {
  ...
  it('performs HEAD preflight then GET streaming and triggers download', async () => {
    ...
    const mockFetch = vi.fn().mockImplementation((url: string, init?: RequestInit) => {
      ...
      return Promise.resolve({
        ...
        status: 200,
        headers: new Headers({
          'content-disposition': "attachment; filename=\"test.zip\"; filename*=UTF-8''test_%E5%BD%92%E6%A1%A3.zip",
        }),
  ...
  it('aborts when HEAD preflight returns 404', async () => {
    ...
    await expect(exportSessionZipPack('non-existent')).rejects.toThrow('会话不存在或已被删除');
