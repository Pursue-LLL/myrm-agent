...
const ABSOLUTE_SHARE_URL = 'https://myrm-x.example.com/api/v1/public/artifact-share/abc.def';
...
describe('buildPublicArtifactShareUrl', () => {
  ...
  it.each(['https://', 'http://'])('passes through absolute server-provided %s share URLs untouched', (scheme) => {
  ...
  it('prefers the configured backend base over the browser origin', () => {
    ...
    expect(buildPublicArtifactShareUrl(RELATIVE_SHARE_PATH)).toBe(`https://backend.example.com${RELATIVE_SHARE_PATH}`);
  });

  it('falls back to the current origin when no backend base is configured', () => {
    vi.stubGlobal('location', { origin: 'http://localhost:3000' });
    expect(buildPublicArtifactShareUrl(RELATIVE_SHARE_PATH)).toBe(`http://localhost:3000${RELATIVE_SHARE_PATH}`);
  });
});
