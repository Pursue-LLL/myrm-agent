 ...
 *
 * [OUTPUT]
 * - POST: Validate embedding retrieval config against the FastAPI backend.
 *
 * [POS]
 * Embedding retrieval validation proxy. Bridges the settings UI to the backend
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { status, text } = await postJsonHttp(`http://${apiHost}:${apiPort}/api/v1/retrieval/embedding`, body);
    try {
    ...
    console.error('Embedding validation proxy error:', error);
