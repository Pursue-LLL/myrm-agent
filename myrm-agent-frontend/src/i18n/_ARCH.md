...
代码引用门禁（`scripts/verify-i18n-references.mjs`，随 `verify:i18n` 执行）：静态扫描 `src/**\/*.{ts,tsx}` 的 `t()`/`showI18nToast()`/`descriptionKey`/`action.label` 引用，按最近前驱同名翻译绑定归因命名空间，逐一校验 en.json（SSOT）键存在——补 locale 间 parity 门禁的测量盲区，杜绝「组件引用但 SSOT 缺失」导致的运行时原始 key（MISSING_MESSAGE）。
