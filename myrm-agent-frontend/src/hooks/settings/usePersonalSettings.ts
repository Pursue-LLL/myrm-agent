...
function selectPersonalSettings(state: ReturnType<typeof useConfigStore.getState>): PersonalSettingsConfigValue {
  return {
    ...
    activeThemeProfileId: state.personalSettings?.activeThemeProfileId,
    themeProfiles: state.personalSettings?.themeProfiles,
    themeFontOverride: state.personalSettings?.themeFontOverride,
