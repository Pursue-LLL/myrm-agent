...
| `useSystemConfig.ts` / `usePersonalSettings.ts` / `useMCPConfig.ts` | 设置与 MCP 配置状态；`useMCPConfig` 负责 `hostSerial`/`keepaliveInterval`（remote transport 下最小 5 秒）校验；`useSystemConfig` 在 save/reset/saveAndRestart 时同步 `myrm-tauri-system-config` 供 `deploy-mode` 读端口 |
