...
| `useSystemConfig.ts` / `usePersonalSettings.ts` / `useMCPConfig.ts` | 设置与 MCP 配置状态；`useMCPConfig` 负责 `hostSerial`/`keepaliveInterval`（remote transport 下最小 5 秒）校验，并将 `undefined` keepalive 视作“未设置”避免误报；`useSystemConfig` 在 save/reset/saveAndRestart 时同步 `myrm-tauri-system-config` 供 `deploy-mode` 读端口 |
...
| `useBrowserTakeoverActions.ts` | 浏览器 HITL Complete/Skip；Done/Skip 经 `resolveBrowserTakeoverMessageId()` 解析 messageId 后 `sendMessage('', msgId, { action })` resume（依赖 `messageRequest` HITL resume 守卫 + SSE handler 已 `setLoading(false)`） | Extension 横幅 + VNC 面板共用 |
