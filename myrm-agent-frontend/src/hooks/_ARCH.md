...
| `useVoiceSession.ts` / `useRealtimeVoice.ts` / `useGeminiLiveVoice.ts` / `useTTS.ts` | 语音会话与 TTS（useVoiceSession 含 PTT 屏幕上下文融合，支持 OpenAI Realtime WebRTC + Gemini Live WebSocket 双 Provider） |
...
| `useTrayStatus.ts` | Tauri tray tooltip / taskbar 进度条：chat loading + `/background-tasks` running 数；hidden 窗口时 bg finish `requestUserAttention`（仅 Tauri，Web 不额外打 API） |
