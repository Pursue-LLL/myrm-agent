  ...
  useEffect(() => {
    ...
    function handleEvent(payload: SSEPayload) {
      ...
      if (payload.type === 'pairing_pending') {
        ...
        const staleDuration =
          typeof payload.data.stale_duration_seconds === 'number' ? payload.data.stale_duration_seconds : 0;
        ...
        window.dispatchEvent(new CustomEvent(MANAGED_POLICY_UPDATED_EVENT, { detail: payload.data }));
    ...
    if (!existingLeader) {
      ...
      setTimeout(() => {
        if (!disposed && !isLeaderRef.current) {
          ...
          if (!current || current === tabId.current) {
            becomeLeader();
          }
        }
      }, HEARTBEAT_TIMEOUT);
