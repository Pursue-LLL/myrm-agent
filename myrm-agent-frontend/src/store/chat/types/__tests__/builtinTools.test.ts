...
import { getBuiltinToolDisplayLabel, resolveToolSnapshotDisplayName } from '../builtinTools';

describe('builtinTools display helpers', () => {
  it('returns localized cron label from builtin_tool_id', () => {
    expect(resolveToolSnapshotDisplayName({ name: 'cron_manage_tool', builtin_tool_id: 'cron' }, 'zh')).toBe(
      '定时任务',
    );
  ...
  it('falls back to internal tool name when no product id', () => {
    expect(resolveToolSnapshotDisplayName({ name: 'bash_code_execute_tool' }, 'en')).toBe('bash_code_execute_tool');
  });
