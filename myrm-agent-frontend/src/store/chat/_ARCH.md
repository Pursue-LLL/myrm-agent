...
| `streamConsumer.ts` | 读 SSE 行 → `parseSseEnvelope` → reducer；`executeStreamWithRetry` 网络重试保留同一 `requestMessageId`；409 `AgentBusyError` fail-fast 不重试（由 `useMessageInput` requeue）；`StreamInterruptedError` 走 attach/loadMessages 而非 blind 重 POST；支持 `e2ee_frame` 解密与 multiplex `accepted.message_id` 握手复用；按发送标记在首个有效业务 SSE 帧到达后补记 chat `query_submitted(success)`（复用 `wikiEvidenceContextCore` 统一 `context_key/turn_distance`）；同时消费 steer 挂起 success（`chatId + expectedMessageId`）避免 accepted 即 success 的提前误计；流结束 `finally` 调用 `recoverPendingApprovals()` 补全 missed HITL |
