...
| `messageRequest.ts` | 组装请求并启动流（含 Smart Updater 路由、kanban 发送前看板 guard）；发送前统一归一化 `mcp_cfg` transport/keepalive 语义；fast/deep 发送前 search guard block；agent 模式 web_search 未配置由 SSE preflight gap 通知（无 client 重复 toast）；`resumeValue` 存在时跳过 loading/isProcessing 守卫以支持 browser takeover Done/Skip resume；新 send 时 clear pending gap；统一携带 `reasoning_display_mode`（off/collapsed/inline）到 API；当有单轮 Skill/MCP 覆写时附带 `turn_capability_telemetry`（source + effective counts）供服务端写入终态埋点 |
...
| `chatNavigationSnapshotCache.ts` | 侧边栏 LRU snapshot（20 条，跳过 incognito）；含 agentConfig/actionMode/selectedModels/contextBranches/contextPinnedFiles 及 load error 态 |
