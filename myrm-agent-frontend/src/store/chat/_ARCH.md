...
| `streamConsumer.ts` | 读 SSE 行 → `parseSseEnvelope` → reducer；支持 `e2ee_frame` 解密与 multiplex `accepted.message_id` 握手复用；按发送标记在请求被接受后补记 chat `query_submitted(success)`（复用 `wikiEvidenceContextCore` 统一 `context_key/turn_distance`）；流结束 `finally` 调用 `recoverPendingApprovals()` 补全 missed HITL |
