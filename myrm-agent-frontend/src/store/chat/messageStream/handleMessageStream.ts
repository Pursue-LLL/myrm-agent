 ...
 * ./streamHelpers (POS: Normalization and merge helpers)
 * ./fileDiffMerge (POS: FILE_DIFF merge scoring)
 * ./textSanitize::sanitizeStreamText (POS: Stream chunk sanitization)
 * ../archiveRestoreActions (POS: Archive restore parsing)
 * ../messageUtils::findAssistantMessageIndex (POS: Message index lookup)
 * ../memoryCitationUtils (POS: Memory citation merge)
 *
 * [OUTPUT]
 * handleMessageStream: merges Agent SSE events into chat Message state
 *
 * [POS]
 * Main chat stream reducer implementation (event branches).
 */
...
import { sanitizeStreamText } from './textSanitize';
...
export const handleMessageStream = async (
  ...
  if (data.type === AgentEventType.REASONING) {
    if (data.data && data.data.length > 0) {
      const reasoningChunk = sanitizeStreamText(data.data as string);
  ...
  if (data.type === AgentEventType.MESSAGE) {
    ...
    if (data.data && data.data.length > 0) {
      recievedMessage += sanitizeStreamText(data.data as string);
