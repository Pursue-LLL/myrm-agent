...
| `statusStreamProgressSteps.ts` | 辅助 | STATUS `progress.step_key` 分支与 toast（含 stream recovery；`model_failover` 与 `modelNotifyEvents` MODEL_FAILOVER 双通道按 `model_failover*` 前缀去重为单 step；**restart 协议**：`model_failover`/`safety_fallback_active` 或 `data.restart === true` 的 STATUS（transient_retry/context_compaction 等）到达时经 `clearAssistantDraft` 清空已流式的 content/reasoning 废稿并重置 reasoning 计时，同时 `scheduler.cancel()` 丢弃挂在调度器上的旧渲染闭包（防止其把清空前的废稿写回），与 MODEL_FAILOVER SSE 幂等；`model_failover_unconfigured` → `/settings/defaultModel`；`safety_fallback_unconfigured` → `/settings/agents#loadout`；`turn_prewarm_*` · `wiki_knowledge_lane`；`waiting_for_turn` 项目锁等待 step（early recovery 占位 + i18n 标题）） | ✅ |
...
| `modelNotifyEvents.ts` | 核心 | MODEL_ESCALATED、MODEL_FAILOVER、MODEL_RECOVERY（toast 经 `modelNotifyToastKey` 6-locale i18n SSOT；MODEL_FAILOVER progress step 与 STATUS 通道去重；**restart 协议**：MODEL_FAILOVER 到达时经 `clearAssistantDraft` 清空已流式的 content/reasoning 废稿并 `scheduler.cancel()` 丢弃旧渲染闭包——fallback 从头重跑，避免旧废稿拼接完整回答；MODEL_ESCALATED payload `restart:true` 时同样清空废稿——升级模型重跑本轮） | ✅ |
