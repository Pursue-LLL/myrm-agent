 ...
 *
 * [OUTPUT]
 * AgentAvatar: 智能体头像组件，支持 icon/lucide/emoji/image/首字母五种模式
 *
 * [POS]
 * 智能体头像统一渲染层。根据 avatar 字符串前缀自动选择渲染策略：
 * icon: → 定制 SVG 符号，lucide: → Lucide 图标，emoji: → emoji，home:///URL → 图片，否则 → 首字母。
 */
...
export function AgentAvatar({ url, name, agentId, className, size = 'md' }: AgentAvatarProps) {
  ...
  if (parsed?.type === 'lucide') {
    return <LucideAgentIcon iconName={parsed.iconName} size={size} className={className} />;
  }
