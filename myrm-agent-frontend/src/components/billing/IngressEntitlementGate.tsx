...
export default function IngressEntitlementGate({ children }: { children: React.ReactNode }) {
  const t = useTranslations('billing.gates');
  const { canUsePublicIngress, isLoading } = useFeatureEntitlements();

  if (!isSandbox() || isLoading || canUsePublicIngress) {
    return <>{children}</>;
  }

  return (
    <div className="rounded-2xl border border-dashed border-border/70 bg-muted/20 px-5 py-8 text-center space-y-4">
      <p className="text-sm font-medium text-foreground">{t('ingressTitle')}</p>
      <p className="text-sm text-muted-foreground">{t('ingressDescription')}</p>
      <Link
        href="/pricing"
