...
| `TeamAssetsHub.tsx` | 核心 | 团队资产一屏：嵌入 SharedContextPanel + Wiki/Skills/Memory 入口卡片 | ✅ |
| `useAgentLoadoutSummary.ts` | 辅助 | 并行拉取 agent/readiness/SC bindings/proposals + 全局 memory 策略；bindings/proposals/readiness 失败时返回 `unavailable` 避免假 0/假 None/假 Ready；`refreshKey` 驱动 bind/unbind 后重拉 | ✅ |
