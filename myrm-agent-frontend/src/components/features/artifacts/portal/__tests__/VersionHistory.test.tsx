...
vi.mock('next-intl', () => ({
  useTranslations: () => {
    const t = (key: string, params?: Record<string, unknown>) => {
      ...
      if (key === 'versions.minutesAgo') {
        return `${params?.count}m ago`;
      }
      if (key === 'versions.hoursAgo') {
        return `${params?.count}h ago`;
      }
      if (key === 'versions.daysAgo') {
        return `${params?.count}d ago`;
      }
      if (key === 'versions.totalVersions') {
        return `${params?.count} versions`;
      }
      if (key === 'versions.viewingHistoryBanner') {
        return `Viewing v${params?.version}`;
      }
...
describe('VersionHistoryBanner', () => {
  ...
  it('shows banner with correct version number', () => {
    render(<VersionHistoryBanner versions={versions} viewingIndex={1} onBackToLatest={vi.fn()} />);
  ...
  it('applies amber styling for historical warning banner', () => {
    render(<VersionHistoryBanner versions={versions} viewingIndex={0} onBackToLatest={vi.fn()} />);
