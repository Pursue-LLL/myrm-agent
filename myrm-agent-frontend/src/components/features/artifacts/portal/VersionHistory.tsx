...
function formatRelativeTime(dateString: string, t: ReturnType<typeof useTranslations>): string {
  ...
  if (diffMins < 1) {
    return t('versions.justNow');
  }
  if (diffMins < 60) {
    return t('versions.minutesAgo', { count: diffMins });
  }
  if (diffHours < 24) {
    return t('versions.hoursAgo', { count: diffHours });
  }
  if (diffDays < 7) {
    return t('versions.daysAgo', { count: diffDays });
  }
