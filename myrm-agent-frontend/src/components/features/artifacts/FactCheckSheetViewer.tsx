      ...
      const matchKw =
        ...
        item.sources.some(
          (s) =>
            ...
            s.claimed_value.toLowerCase().includes(kw)
        );
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-4">
          ...
          {!loading && filteredItems.map((item) => {
            ...
            return (
              <div
                key={item.id}
                className="border rounded-xl bg-card transition-all shadow-sm overflow-hidden"
              >
                {/* Item Header */}
                <div
                            ...
                            <tbody>
                              {item.sources.map((src, sIdx) => (
                                ...
                                >
                                  ...
                                  <td className="p-2 font-mono text-primary font-medium">
                                    {src.claimed_value}
                                  </td>
                                  <td className="p-2 font-mono text-muted-foreground text-[11px]">
                                    {src.line_anchor || src.timestamp_hint || '-'}
                                  </td>
                                  <td className="p-2 text-muted-foreground text-[11px] max-w-[240px] truncate" title={src.snippet}>
                                    {src.snippet || '-'}
                                  </td>
                                </tr>
                              ))}
