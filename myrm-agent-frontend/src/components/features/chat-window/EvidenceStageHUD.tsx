...
export const EvidenceStageHUD = memo<EvidenceStageHUDProps>(
  ({ steps = [], loading = false, className = '', defaultExpanded = false }) => {
    ...
    const derivedStages = useMemo(() => {
      ...
      const execSummary: EvidenceStageSummary = {
        ...
        detail: loading ? t('stages.executing.running') : t('stages.executing.completed', { count: toolSteps.length }),
      ...
      const verSummary: EvidenceStageSummary = {
        ...
        detail:
          verifySteps.length > 0
            ? verStatus === 'success'
              ? t('stages.verified.passed', { count: verifySteps.length })
              : t('stages.verified.failed')
          ...
          <div className="flex items-center gap-1.5 flex-wrap">
            {derivedStages.stages.map((stage) => (
              ...
              >
                ...
                {stage.itemCount > 0 && <span className="text-[10px] opacity-70 ml-0.5">({stage.itemCount})</span>}
              </button>
            ))}
          </div>
        ...
        {expanded && (
          ...
          >
            <div className="flex items-center justify-between text-[11px] text-muted-foreground px-1">
              ...
              <span>{t('drawer.itemCount', { count: filteredEvidenceSteps.length })}</span>
            </div>

            <div className="space-y-1.5 max-h-56 overflow-y-auto pr-1">
              {filteredEvidenceSteps.length === 0 ? (
                <div className="py-3 text-center text-muted-foreground text-[11px]">{t('drawer.empty')}</div>
              ) : (
                filteredEvidenceSteps.map((step, idx) => (
                  ...
                  >
                    ...
                    {step.reason && (
                      <p className="text-muted-foreground text-[10px] leading-relaxed line-clamp-2">{step.reason}</p>
                    )}
