...
| `KanbanConfigSection.tsx` | Helper | Kanban chat target board: 按 active project 调用 `listBoards({ projectId })`，0-board Settings link，multi-board 选择与项目作用域本地持久化；与 `messageRequest` 发送前 guard 配套（stale id recovery） |
...
| `TemplateMarket.tsx` | Feature | Individual / Team template market with 统一检索口径；Team 卡显示 `members` + `use_cases`，支持键盘可达召唤；实例化链路统一接入召唤漏斗埋点并预填首条消息 |
...
Tool IDs are defined in `@/store/chat/types/builtinTools.ts` and must stay in sync with server `resolve_builtin_tool_flags()` in `profile_resolver.py` (`planning`, `answer_tool`, `render_ui`, `external_cli` → `enable_external_cli`, `cron` → `enable_cron_eager`, …). `render_ui` shows `renderUiWebOnlyHint` when enabled (Web Chat / desktop only).
