...
| `BuiltinToolsPanel.tsx` | Core | Built-in tool toggles; sandbox cron card gated by `useFeatureEntitlements` (mirror CronSection); browser sub-config; `MediaCredentialInline` when media tools enabled without API keys |
...
Tool IDs are defined in `@/store/chat/types/builtinTools.ts` and must stay in sync with server `resolve_builtin_tool_flags()` in `profile_resolver.py` (`planning`, `answer_tool`, `render_ui`, `cron` → `enable_cron_eager`, …).
