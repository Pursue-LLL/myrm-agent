...
const OrgMarketplace = ({ className, onInstalled }: OrgMarketplaceProps) => {
  ...
  const handleInstall = useCallback(
    async (entry: MarketplaceEntry) => {
      ...
      try {
        ...
        const agent = await importMarketplaceAgent(result.profile_data, { marketplaceEntryId: result.entry_id });
        ...
        setEntries((prev) =>
          prev.map((e) => (e.id === entry.id ? { ...e, install_count: e.install_count + 1, is_installed: true } : e)),
        );
      } catch (e) {
        console.error(e);
  ...
  const handleForcePush = useCallback(
    async (entry: MarketplaceEntry) => {
      ...
      const confirmed = window.confirm(t('forcePushConfirm', { name: entry.name, version: entry.latest_version }));
      if (!confirmed) {
        return;
      }
      ...
      try {
        ...
        toast.success(t('forcePushSuccess'), {
  ...
  return (
    <div className={cn('space-y-3 pt-2', className)}>
      <div className="flex items-center gap-2 px-1">
        ...
        <span className="text-xs text-muted-foreground">{t('orgMarketplace')}</span>
        <div className="flex-1 h-px bg-border/50" />
      </div>
      ...
      {search && entries.length === 0 && !loading && (
        <p className="text-center text-xs text-muted-foreground py-3">{t('noResults')}</p>
      )}
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <div className="flex-1 min-w-0">
                ...
                {entry.description && <div className="text-xs text-muted-foreground truncate">{entry.description}</div>}
              </div>
            ...
            <div className="flex items-center gap-2 text-[10px] text-muted-foreground/70">
              ...
              <span>·</span>
              <span>
                {entry.install_count} {t('installs')}
              </span>
              {entry.is_installed && (
                <>
                  <span>·</span>
                  <span className="text-green-600 font-medium">{t('installed')}</span>
                </>
              )}
              {isOrgAdmin && entry.install_count > 0 && (
                ...
                >
                  {pushingId === entry.id ? <Loader2 size={10} className="animate-spin" /> : <RefreshCw size={10} />}
                  {t('forcePush')}
                </button>
              )}
            </div>
