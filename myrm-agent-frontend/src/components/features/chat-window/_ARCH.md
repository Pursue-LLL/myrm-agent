...
| `SamplePrompts.tsx`                         | 组件      | 空会话情境感知操作算子与示例 prompt 列表：时段智能感知（晨间/午后/晚间/深夜四时段自动自适应与微胶囊切换）+ 模式与等级加权过滤                                                                                                                 | ✅                                                                |
...
| 文件                                       | 地位 | 职责                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | I/O/P |
| ------------------------------------------ | ---- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----- |
| `ChatInlineTeamRunVisibilityStrip.tsx`     | 组件 | 聊天输入区上方多智能体常驻微进度条：动态消费 `useSubagentStore`，展示当前 pending/running/verifying/pending_approval/interrupted/checkpoint 及 stale 异常停滞的多智能体头像堆叠与优先级抢占微条，提供加权平均微进度轨与 2.5s 完成退出缓冲，支持头像点击深链唤起 `SubagentDashboard` 聚焦高亮 | ✅    |
| `SubagentDashboard.tsx`                    | 核心 | 子代理控制面板入口：两视图 tab（树/画布）；支持响应 `open_subagent_dashboard` 自定义事件携带 `taskId` 自动切换树视图并执行平滑滚动与闪烁高亮聚焦；树视图内排序/过滤条（`subagent-sort-*`/`subagent-filter-*`）、MiniGantt、fission 摘要条（`subagent-fission-summary`）、头部汇总、预算 token/cost（`extractBudgetTokens`/`extractMaxCostUsd`）、取消全部/委托暂停、overtime/stale 汇总态、画布 tab 挂载 `AgentWorkMap`，画布节点点击切树视图定位（`data-subagent-tree-id`）；SSE `subagents_updated`/`teammate_message` 消费 + 轮询拉取；面板/触发器 `data-testid` 供 E2E 选择         | ✅    |
