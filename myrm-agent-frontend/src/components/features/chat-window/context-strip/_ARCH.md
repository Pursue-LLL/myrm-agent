...
作为输入框发射前（Pre-flight）所有挂载项的唯一 SSOT 视图中枢，优雅聚合显式技能激活、工作流模板、单轮能力范围、@ 提及引用，并提供 Token 负载 Amber 警示与一键能力剪枝联动。
...
- 上游 Hook：`@/hooks/message-input/useComposerContextChips` 提供 `chips` 列表与 `summary` 负载信息。
