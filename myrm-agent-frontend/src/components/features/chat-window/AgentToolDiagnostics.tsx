    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-[600px]">
        ...
        <div className="flex flex-col gap-4 py-4 max-h-[60vh] overflow-y-auto pr-2">
          {loading ? (
            <div className="flex items-center justify-center p-8 text-muted-foreground">Loading diagnostics...</div>
          ) : sortedData.length === 0 ? (
            ...
            <div className="space-y-3">
              {sortedData.map((tool) => {
                ...
                return (
                  <div
                    key={tool.tool_name}
                    className={`p-3 rounded-lg border flex flex-col gap-2 ${isCritical ? 'border-red-200 bg-red-50/50 dark:border-red-900/50 dark:bg-red-950/20' : isWarning ? 'border-amber-200 bg-amber-50/50 dark:border-amber-900/50 dark:bg-amber-950/20' : 'border-border/60 bg-muted/20'}`}
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <div className="text-xs text-muted-foreground">{tool.total_calls} calls</div>
                    </div>

                    <div className="flex flex-col gap-1.5 mt-1">
                      <div className="flex items-center justify-between text-xs">
                        <span className="text-muted-foreground">Success Rate</span>
                        <span className={`font-medium ${isCritical ? 'text-red-600 dark:text-red-400' : ''}`}>
                          {((1 - errorRate) * 100).toFixed(1)}%
                        </span>
                      </div>
                      <Progress
                        value={(1 - errorRate) * 100}
                        className="h-1.5"
                        indicatorClassName={isCritical ? 'bg-red-500' : isWarning ? 'bg-amber-500' : 'bg-green-500'}
                      />
                    </div>
