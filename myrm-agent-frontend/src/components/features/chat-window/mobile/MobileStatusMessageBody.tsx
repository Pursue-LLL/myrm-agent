...
import { Activity, BrainCircuit, ShieldCheck, Check, X, Download, ExternalLink, FileText } from 'lucide-react';
...
import { getArtifactIcon, formatBytes } from '@/components/features/artifacts/artifactUtils';
...
export function MobileStatusMessageBody({
  ...
  return (
    <>
      ...
      {lastAssistantMessage.artifacts && lastAssistantMessage.artifacts.length > 0 && (
        <MobileArtifactList artifacts={lastAssistantMessage.artifacts} />
      )}
...
function MobileArtifactList({ artifacts }: { artifacts: Artifact[] }) {
  const t = useTranslations('agent.mobileCommand');

  const handlePreview = useCallback((artifact: Artifact) => {
    const url = getStorageUrl(artifact.preview_url);
    window.open(url, '_blank', 'noopener,noreferrer');
  }, []);

  const handleDownload = useCallback(async (artifact: Artifact) => {
    try {
      const response = await fetch(getStorageUrl(artifact.download_url));
      ...
      // download failure is non-critical on mobile
    }
  }, []);
