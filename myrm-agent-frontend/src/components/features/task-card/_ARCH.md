...
| 文件                      | 地位 | 职责                                                                | I/O/P |
| ------------------------- | ---- | ------------------------------------------------------------------- | ----- |
| `ImageTaskCard.tsx`       | 核心 | 图像类后台任务进度与结果摘要卡片（支持进行中取消与重试）            | ✅    |
| `VideoTaskCard.tsx`       | 核心 | 视频类后台任务进度与结果摘要卡片（支持进行中取消与重试）            | ✅    |
...
| `TaskCardPlaceholder.tsx` | 辅助 | 任务加载/生成中占位（展示进度、状态信息，支持原地取消与状态反馈）   | ✅    |
