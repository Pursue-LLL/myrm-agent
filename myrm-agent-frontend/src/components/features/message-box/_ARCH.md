...
| `MarkdownContent.tsx` | 核心 | Markdown 渲染（数学公式/代码块/图表/GFM Alerts/脚注/citation），支持 web/mcp/kb/conversation 四种 citation 类型；KB citation 可点击打开 SourceChunkDrawer，并传递分层来源 level（L0/L1/L2）+ `contextKey(chat:<messageId>)` 用于证据语义展示与复问口径隔离。 | ✅ |
...
| `SourceChunkDrawer.tsx` | 组件 | KB 引用原文片段 Drawer：点击 KB citation 后以右侧 Sheet 展示原文 snippet，支持 section + 分层来源标签（L0/L1/L2）与分段渲染；按 `surface(chat/settings)` + `contextKey` 记录证据展开与核验停留时长埋点。 | ✅ |
