...
| `local-backend-unavailable-banner.tsx` | 组件/模块 | `LocalBackendUnavailableBanner`：后端未就绪告警；`ConfigReadinessDegradedBanner`：readiness 降级非阻塞告警（`common.readinessDegraded`） | `checkBackendReadyOnce`, `resolveLocalBackendSetupHint`, `isLocalMode` |
