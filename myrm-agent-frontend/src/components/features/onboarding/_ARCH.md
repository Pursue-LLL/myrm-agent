...
首次启动向导：欢迎屏 → 可选外部助手迁移 → 本地能力配置（模型 + 搜索 + Hardware Cookbook + 云端快速开始） → 条件性 Smart Routing 引导（≥2 模型且未启用时展示）。
...
| `LocalCapabilitiesSetup.tsx` | 本地 Ollama/LM Studio 探测、SearXNG、**HardwareCookbook**（无 provider 时展示硬件推荐）、**云端快速开始卡片**（无本地模型时展示云端 Provider 引导） |
