...
const PluginManagerDialog = memo(({ open, onOpenChange, onPluginChanged }: PluginManagerDialogProps) => {
  ...
  return (
    <>
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-w-2xl max-h-[80vh] flex flex-col p-0 overflow-hidden">
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="flex flex-wrap items-center gap-1.5 mt-1">
                          {plugin.servers.length === 0 ? (
                              ...
                              >
                                <span
                                  ...
                                  className={`inline-block w-1.5 h-1.5 rounded-full ${
                                    server.enabled ? 'bg-emerald-500' : 'bg-amber-500'
                                  }`}
                                />
                                <span className="text-muted-foreground break-all">{server.name}</span>
                                <span className={server.enabled ? 'text-emerald-600' : 'text-amber-600'}>
                                  {server.enabled ? t('serverState.enabled') : t('serverState.disabled')}
                                </span>
                                {!server.enabled && (
                                  <span title={t('serverState.disabledHint')} className="cursor-help text-amber-600/80">
                            ...
                            <p className="text-xs text-muted-foreground break-all">{plugin.servers.join(', ')}</p>
                          )}
                        </div>
                      </div>
                      <Button
          ...
          <div className="p-4 border-t flex items-center justify-between gap-3">
            <Button variant="ghost" size="sm" onClick={() => void refresh()} disabled={isLoading} className="gap-2">
      ...
      <AlertDialog open={!!pendingUninstall} onOpenChange={(v) => !v && setPendingUninstall(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
          ...
          <AlertDialogFooter>
            <AlertDialogCancel disabled={uninstalling !== null}>{t('actions.cancel')}</AlertDialogCancel>
            <AlertDialogAction
