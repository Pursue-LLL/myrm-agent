...
| `MemoryCommandCenterDoctorPanel.tsx` | 组件 | 记忆系统 Doctor 诊断面板；含静态检查、可执行诊断、结构化 benchmark 指标卡片与历史趋势（最近 N 次 recall@5/ndcg/mrr + 上次 delta + 类别通过率 + run 状态徽章 + embedding 模型漂移提示） | ✅ |
