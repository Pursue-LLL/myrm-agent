...
export function useSharedContextPanel() {
  ...
  const loadContexts = useCallback(async () => {
    ...
    try {
      ...
      setSelectedContextId((current) => {
        if (current && response.items.some((context) => context.id === current)) {
          return current;
        }
  ...
  const loadTargetOptions = useCallback(async () => {
    ...
    if (channelResponse.status === 'fulfilled') {
      setChannels(channelResponse.value);
    }
    if (cronResponse.status === 'fulfilled') {
      setCronJobs(cronResponse.value.items);
    }
  }, []);
  ...
  useEffect(() => {
    if (selectedContextId) {
      void loadContextDetails(selectedContextId);
    }
  ...
  const refreshSelected = useCallback(async () => {
    await loadContexts();
    if (selectedContextId) {
      await loadContextDetails(selectedContextId);
    }
  ...
  const handleArchiveContext = useCallback(async () => {
    if (!selectedContext || selectedContext.status !== 'active') {
      return;
    }
  ...
  const handleCreateBinding = useCallback(async () => {
    if (!selectedContextId || !selectedContextIsActive || !targetId.trim()) {
      return;
    }
  ...
  const handleCreateProposal = useCallback(async () => {
    if (!selectedContextId || !selectedContextIsActive || !proposalContent.trim()) {
      return;
    }
  ...
  const handleSaveProposal = useCallback(
    async (proposalId: string) => {
      if (!selectedContextIsActive || !editingContent.trim()) {
        return;
      }
      setActionId(proposalId);
      try {
  ...
  const handleSearchHistory = useCallback(async () => {
    if (!selectedContextId || !historyQuery.trim()) {
      return;
    }
  ...
  const handlePromoteHistory = useCallback(
    async (message: SharedContextHistoryMessage) => {
      if (!selectedContextId || !selectedContextIsActive) {
        return;
      }
