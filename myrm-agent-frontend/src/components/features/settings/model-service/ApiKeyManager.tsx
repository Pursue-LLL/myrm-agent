...
const ApiKeyManager = memo<ApiKeyManagerProps>(
  ({ apiKeys, onChange, onProbeKey, credentialPoolStrategy, onStrategyChange }) => {
    const t = useTranslations('settings.modelService');
    ...
    useEffect(() => {
      if (!onProbeKey || !apiKeys || apiKeys.length === 0) {
        return;
      }
            ...
            {onStrategyChange && (
              <Tooltip>
                ...
                <TooltipContent side="bottom" className="max-w-[260px]">
                  {t(
                    STRATEGY_OPTIONS.find((o) => o.value === (credentialPoolStrategy ?? 'round_robin'))?.descKey ??
                      'strategyRoundRobinDesc',
                  )}
                </TooltipContent>
              </Tooltip>
            )}
