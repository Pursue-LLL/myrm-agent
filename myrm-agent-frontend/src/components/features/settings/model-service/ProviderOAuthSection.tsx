...
const ProviderOAuthSection = memo<ProviderOAuthSectionProps>(
  ({ providerId, hasApiKey, onOAuthStatusChange }) => {
    ...
    const handleConnect = useCallback(async () => {
      ...
      try {
        ...
        setIsLoading(false);
        toast.error(
          locale === 'zh' ? '启动授权失败，请重试' : 'Failed to start authorization',
        );
      }
    ...
    const handleDisconnect = useCallback(async () => {
      ...
      try {
        ...
        toast.success(
          locale === 'zh' ? 'OAuth 已断开' : 'OAuth disconnected',
        );
      } catch {
        toast.error(
          locale === 'zh' ? '断开失败' : 'Failed to disconnect',
        );
      }
