...
import { WikiVideoImportDialog } from './wiki/WikiVideoImportDialog';
...
function maintainSkippedBadgeLabel(reason: string, translate: (key: string) => string): string {
...
export function WikiSection() {
  ...
  const [videoImportDialogOpen, setVideoImportDialogOpen] = useState(false);
  ...
  const scopeLabel = agentScopeId
    ? getBuiltinAgentName(agentScopeId, agents.find((agent) => agent.id === agentScopeId)?.name ?? agentScopeId, locale)
    : t('agentScopeDefault');
  ...
  const retryImportWithSupersede = async (reason: string) => {
    ...
    try {
      if (retry.kind === 'folder' && retry.folderPath) {
        ...
        setIsImporting(true);
        const result = await wikiService.importZip(retry.zipFile, '.md,.txt,.org', true, agentScopeId, conflictOptions);
        await finishImportResult(result, null);
      } else if (retry.kind === 'obsidian-folder' && retry.folderPath) {
        setIsImportingObsidian(true);
        const result = await wikiService.importObsidianFolder(retry.folderPath, true, agentScopeId, conflictOptions);
        await finishImportResult(result, null);
  ...
  const handleWebFolderSelect = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (webFolderInputRef.current) {
        webFolderInputRef.current.value = '';
      }
    }
  };
  ...
  const handleImportObsidianZip = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setIsImportingObsidian(false);
      if (obsidianZipRef.current) {
        obsidianZipRef.current.value = '';
      }
    }
  };
  ...
  const loadHealthReport = useCallback(async () => {
    ...
    const tryE2eFetch = async (): Promise<boolean> => {
      if (typeof window === 'undefined' || sessionStorage.getItem('e2e_warm_platform_readiness') !== 'true') {
        return false;
      }
      try {
        const response = await fetch(buildWikiApiPath('/wiki/health-report', agentScopeId), { cache: 'no-store' });
        if (!response.ok) {
          return false;
        }
  ...
  const loadStats = async () => {
    ...
    const tryE2eStatsFetch = async (): Promise<boolean> => {
      ...
      try {
        const response = await fetch(buildWikiApiPath('/wiki/stats', agentScopeId), { cache: 'no-store' });
        if (!response.ok) {
          return false;
        }
  ...
  const { connected: ingestLive, snapshot: ingestSnapshot } = useWikiIngestSubscription(agentScopeId, {
    onSnapshot: (snap) => {
      ...
      if (typeof snap.synthesis_pending_count === 'number') {
        setStats((prev) => (prev ? { ...prev, synthesis_pending: snap.synthesis_pending_count } : prev));
      }
      ...
      const active = snap.stats.processing > 0 || snap.stats.pending > 0 || snap.compile_run?.state === 'paused';
  ...
  const isWikiCompileBusy =
    isCompiling || compileRun?.state === 'running' || (ingestSnapshot?.stats.processing ?? 0) > 0;
  ...
  const handleMaintain = async () => {
    ...
    try {
      ...
      if (removed > 0) {
        ...
        const displayPaths = pathsJoined.length > 120 ? `${pathsJoined.slice(0, 117)}...` : pathsJoined;
        toast.success(
        ...
        if (openActions > 0) {
          toast.message(t('healthReport.maintainToastOpen', { open: openActions, fixed }));
        } else if (fixed > 0) {
  ...
  return (
    <WikiAgentScopeProvider agentScopeId={agentScopeId} scopeRevision={scopeRevision} scopeLabel={scopeLabel}>
        ...
        <div>
          ...
          <div className="mt-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex flex-col gap-1.5 sm:flex-row sm:items-center sm:gap-3">
              ...
              <Select value={agentScopeId ?? 'default'} onValueChange={handleAgentScopeChange} disabled={agentsLoading}>
            ...
            <p className="text-sm text-primary/80">{agentScopeId ? t('agentScopeNotice') : t('defaultScopeNotice')}</p>
          </div>
        </div>
          ...
          <TabsContent value="overview" className="space-y-6">
            ...
            <Card>
              ...
              <CardContent>
                ...
                {isLoadingStats && (
                  <div className="text-center py-4 text-muted-foreground" data-testid="wiki-stats-loading">
                    {t('loading')}
                  </div>
                )}
                ...
                {stats && (
                  <div className="space-y-4" data-testid="wiki-stats-panel">
                    ...
                    {stats.maintain_state?.last_run_at &&
                    !stats.maintain_state.last_skipped_reason &&
                    (stats.maintain_state.last_issues_found > 0 || stats.maintain_state.last_issues_fixed > 0) ? (
                    ...
                    {(stats.asset_index?.total_files ?? 0) > 0 && !stats.asset_index?.enabled ? (
                      <p className="text-xs text-amber-800/90 dark:text-amber-200/90">{t('stats.assetsVisionHint')}</p>
                    ) : null}
                    {(stats.structural_issues?.broken_links ?? 0) > 0 ||
                    (stats.structural_issues?.invalid_frontmatter_types ?? 0) > 0 ||
                    (stats.structural_issues?.provenance_gaps ?? 0) > 0 ? (
                      !healthReport && !isLoadingHealthReport && !healthReportLoadError ? (
                        <p className="text-xs text-amber-800/90 dark:text-amber-200/90">{t('stats.structuralHint')}</p>
                      ) : null
                    ) : null}
              ...
              <CardContent className="space-y-4">
                <div className="flex flex-col gap-3 lg:flex-row lg:items-end">
                  <div className="flex w-full flex-col gap-1.5 lg:w-56">
                    ...
                    <Select value={queryMode} onValueChange={(value) => setQueryMode(value as 'auto' | 'raw_claim')}>
                  ...
                  <div className="flex w-full flex-1 flex-col gap-2 sm:flex-row">
                    <Input
                      ...
                      className="flex-1"
                    />
                    <Button onClick={handleQuery} disabled={isQuerying || !query.trim()} className="w-full sm:w-auto">
                      ...
                      (retrievalTrace.index_hits.length > 0 ||
                        ...
                        <div className="mt-4 space-y-2 rounded-lg border border-border/60 bg-card/40 p-3">
                          ...
                          {retrievalTrace.seeds.length > 0 && (
                            <div className="space-y-1">
                              <div className="text-xs font-medium text-muted-foreground">
                                {t('query.retrievalTraceSeeds')}
                              </div>
                              <div className="space-y-1">
                                {retrievalTrace.seeds.slice(0, 6).map((seed) => (
                                  <div
                                    key={`${seed.source}-${seed.concept_name}`}
                          ...
                          {retrievalTrace.selected_concepts.length > 0 && (
                            <div className="space-y-1">
                              <div className="text-xs font-medium text-muted-foreground">
                                {t('query.retrievalTraceConcepts')}
                              </div>
                              ...
                              >
                                <div className="flex items-start gap-3">
                                  ...
                                  <div className="min-w-0 flex-1">
                                    ...
                                    {sectionLabel && (
                                      <div className="mt-1 text-xs text-muted-foreground">{sectionLabel}</div>
                                    )}
                                    {snippet.path && (
                  ...
                  <div className="rounded-lg border border-amber-500/30 bg-amber-500/10 p-4 space-y-3">
                    ...
                    <div className="flex flex-wrap gap-2">
                      <Button
                        size="sm"
                        variant="outline"
                        disabled={isResumingCompile}
                        onClick={() => void handleResumeCompile()}
                      >
