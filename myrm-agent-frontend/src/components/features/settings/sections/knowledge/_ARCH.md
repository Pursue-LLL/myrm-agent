...
| `WikiSection.tsx` | 容器 | Wiki 子 Tab；路由 `/settings/wiki`；可选 query `agentId` 切换 per-agent vault 统计与操作；Query 卡片展示 `source_snippets`（path/section/snippet/level）并支持点击打开 `SourceChunkDrawer` 查看证据片段 | ✅ |
