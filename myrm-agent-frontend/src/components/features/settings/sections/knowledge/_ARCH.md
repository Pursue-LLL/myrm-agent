...
| `WikiSection.tsx` | 容器 | Wiki 子 Tab；Compile/Repair toast；**stale-summary banner** + **compileRun pause banner** + Compile disabled when paused + **raw ingest 树/图例**；Query **检索模式**（auto/raw_claim）；Overview tab refresh compileRun；**ingest SSE 订阅** + **tree_sync_required → silent raw/concept 树 REST 刷新** | ✅ |
