...
| VideoKnowledgePlayer.tsx                             | UI   | 视频知识播放器（Bilibili/YouTube/原生视频自适应内嵌、秒级时间戳 seek、章节列表导航）                                                                                                                               | ✅    |
...
| **tests**/wikiTreeUtils.test.ts                      | Test | 树工具 + 溯源 frontmatter 解析 + `stripYamlFrontmatter` 单测                                                                                                                                                      | ✅    |
