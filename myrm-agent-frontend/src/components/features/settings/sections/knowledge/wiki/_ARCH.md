...
| WikiConceptDetailPanel.tsx                           | UI   | 预览 + 视频笔记内嵌播放（`VideoKnowledgePlayer`）+ 反向链接卡片点击多帧渐进小节锚点聚焦与平滑滚动 + **四标签编辑**（truth/advanced 用分屏实时预览编辑器）+ 结构化 claims 展示（`lib/wiki/claimStatusDisplay` badge）；来源对话消息级/会话级跳转                    | ✅    |
