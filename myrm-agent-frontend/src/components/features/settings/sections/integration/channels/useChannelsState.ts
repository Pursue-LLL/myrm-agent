...
export function useChannelsState(t: (key: string, values?: Record<string, string | number>) => string) {
  ...
  const [failureEmoji, setFailureEmoji] = useState('❌');
  ...
  const saveAllPolicies = useCallback(
    async (
      ...
      try {
        ...
        await saveChannelsConfig({
          ...
          failureEmoji: reactionOverrides?.failure ?? failureEmoji,
    ...
    [t, reactionLevel, processingEmoji, completionEmoji, failureEmoji, freeResponseChats],
  );
  ...
  const handleFailureEmojiChange = useCallback(
    async (emoji: string) => {
      setFailureEmoji(emoji);
      await saveAllPolicies(dmPolicy, groupPolicy, groupTrigger, channelOverrides, {
        failure: emoji,
      });
    },
