...
type ProbeRecommendedMode = 'local_or_tauri' | 'start_local_editor_mcp' | 'verify_local_network_and_editor';
...
export const IntegrationConnectDialog = memo<IntegrationConnectDialogProps>(
  ({ entry, locale, onClose, onConnected }) => {
    ...
    const [probeStatus, setProbeStatus] = useState<
      'idle' | 'probing' | 'reachable' | 'unreachable' | 'cloud_not_supported'
    >('idle');
    ...
    const connectGuide =
      locale === 'zh' && entry.postConnectGuideZh ? entry.postConnectGuideZh : entry.postConnectGuide;
    const mcpConfig = entry.mcpConfig as Record<string, unknown> | null;
    const probeUrl = resolveProbeUrl(mcpConfig);
    const deploymentScope = entry.deploymentScope ?? (mcpConfig?.deploymentScope as string | undefined) ?? null;
    ...
    const persistCatalogConfig = useCallback(
      ...
      [entry.name, mcpConfig, mcpConfigs, onConnected, setMCPConfigs, t],
    );
    ...
    const runCatalogSecurityGate = useCallback(
      ...
      [persistCatalogConfig, t, tSettings],
    );

    const handleConfirmCatalogAck = useCallback(async () => {
      if (!pendingCatalogAck) {
        return;
      }
    ...
    const handleConnect = useCallback(
      async (options?: { skipProbe?: boolean }) => {
        if (entry.authType === 'oauth2') {
          ...
          try {
            ...
            // 1. Start OAuth flow
            const startRes = await apiRequest<{ authorization_url: string; state: string }>(
              '/integrations/mcp/oauth/start',
              {
                method: 'POST',
                body: JSON.stringify({
            ...
            pollIntervalRef.current = setInterval(async () => {
              try {
                const statusRes = await apiRequest<{ status: string }>(
                  `/integrations/mcp/oauth/status/${startRes.state}`,
                  { silent: true },
                );
          ...
          <DialogContent className="sm:max-w-md">
                ...
                <div className="flex flex-col items-center justify-center space-y-4 py-4">
                  ...
                  <div className="text-center">
                    <h3 className="font-medium">
                      {t('oauthTitle', { name: entry.name, default: `Connect to ${entry.name}` })}
                    </h3>
                    <p className="text-muted-foreground mt-1 text-sm">
                      {t('oauthDescription', {
                        default: 'You will be redirected to the provider to authorize access.',
                      })}
                    </p>
                  </div>
                </div>
            ...
            <DialogFooter>
              ...
              <Button onClick={() => void handleConnect()} disabled={connecting || oauthPolling}>
                {oauthPolling
                  ? t('waitingAuth', { default: 'Waiting for authorization...' })
                  : connecting
                    ? t('connecting')
                    : t('connect')}
              </Button>
