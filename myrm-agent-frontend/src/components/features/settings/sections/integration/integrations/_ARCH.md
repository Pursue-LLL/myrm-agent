...
| `IntegrationConnectDialog.tsx` | OAuth / API Key 连接对话框；消费 probe `shouldBlockConnect`、`reasonCode` 与 `recommendedMode`，在失败态提供本地化诊断与可执行下一步动作（重试成功后自动续接连接；`local_or_tauri` 打开本地部署指引并引导切换模式） |
