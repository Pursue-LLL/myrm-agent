...
| `LifecycleWebhookSection.tsx`       | 生命周期出站 Webhook 管理卡片：端点 CRUD（含卡片内 Edit 抽屉）、Agent Profile 范围选择、事件多选（六国 i18n）、HMAC 徽标（`has_secret`）、`/{id}/ping` 探测；响应式 sm:flex 布局                                                                                                                                                  |
| `WebhookEndpointForm.tsx`           | Create/Edit 共用表单：名称、URL、Agent 范围、HMAC 密钥（Edit 留空不改）、事件多选                                                                                                                                                                                            |
