...
| `IntegrationConnectDialog.tsx` | OAuth / API Key 连接对话框；消费 probe `shouldBlockConnect` 与 `reasonCode`（含 `tls_verification_failed` 本地化提示）并执行 cloud/local-only 阻断 |
...
| `IntegrationConnectDialog.test.tsx` | Dialog 行为级单测：覆盖无 probeUrl 兜底阻断、probe `shouldBlockConnect=true` 阻断与 `tls_verification_failed` 文案分流 |
