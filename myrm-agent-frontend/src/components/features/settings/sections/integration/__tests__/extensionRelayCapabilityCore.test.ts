...
describe('extensionRelayCapabilityCore', () => {
  ...
  it('detects partial capability sets for upgrade-required UI', () => {
    ...
    expect(rows.filter((row) => row.available).map((row) => row.key)).toEqual([
      'navigate_url',
      'list_tabs',
    ]);
    expect(listMissingRelayCapabilities(['navigate_url', 'list_tabs'])).toEqual([
      'attach_debugger',
      'detach_debugger',
    ]);
  });
  ...
  it('resolves relay status kind across connection lifecycle', () => {
    ...
    expect(
      resolveRelayCapabilityStatusKind(true, true, [
        'navigate_url',
        'list_tabs',
        'attach_debugger',
        'detach_debugger',
    ...
    expect(
      resolveRelayCapabilityStatusKind(true, true, ['navigate_url', 'list_tabs']),
