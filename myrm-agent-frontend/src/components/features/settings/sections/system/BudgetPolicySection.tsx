...
const BudgetPolicySection = memo(() => {
  ...
  const [fleetItems, setFleetItems] = useState<FleetQuotaItem[]>([]);
  ...
  const fetchData = useCallback(async () => {
    try {
      const [p, s, f] = await Promise.all([
        getBudgetPolicy(),
        getBudgetStatus(),
        getFleetQuotaDeck().catch(() => ({ items: [] })),
      ]);
      setPolicy(p);
      setStatus(s);
      setFleetItems(f.items || []);
    } catch {
      // silent
    }
  }, []);
    ...
    >
      ...
      {policy.enabled && (
        <div className="space-y-4 pt-2">
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  if (!isNaN(v) && v >= 10 && v <= 100) {
                    updateField('warning_threshold', v / 100);
                  }
                }}
          ...
          {/* Four-Tier Progressive Spend Control Ladder */}
            ...
            {fleetItems.length > 0 && (
              <div className="mt-4 pt-3 border-t border-border/30 space-y-2">
                ...
                <div className="space-y-1.5">
                  {fleetItems.map((item) => (
                    <div
                      key={`${item.dimension}-${item.identifier}`}
                    ...
                    >
                      ...
                      <div className="flex items-center gap-3">
                        <span className="text-muted-foreground">
                          ${item.spendUsd.toFixed(2)} / ${item.allocatedQuotaUsd.toFixed(2)}
                        </span>
                        <span
                          className={cn(
