...
| `DesktopPermissionsCard.tsx`                | 桌面自动化就绪检测：首屏 grant-only 三态（verified=`capture_ready` / unverified / missing）+ 刷新 `?probe_capture=true` + 功能捕获行 + 始终信任应用（`GET/DELETE /webui/desktop/trust/apps`）                                                                                                                                    |
| `__tests__/DesktopPermissionsCard.test.tsx` | vitest：首屏未验证 / Recheck 达 allReady / deeplink / API fail / trusted revoke / trust load error                                                                                                                                                                                                                            |
