...
export default function KanbanSection() {
  ...
  const fetchBoards = useCallback(async () => {
    try {
      ...
      const result = isDeepLink ? await listBoards() : await listBoards({ projectId: activeProjectId });
  ...
  useEffect(() => {
    ...
    if (statusParam) {
      ...
      const candidates = lastId
        ? [boards.find((b) => b.board_id === lastId), ...boards.filter((b) => b.board_id !== lastId)].filter(
            (b): b is KanbanBoard => Boolean(b),
          )
        : boards;
      let cancelled = false;
      (async () => {
  ...
  if (selectedBoard) {
    return <KanbanBoardView key={selectedBoard.board_id} board={selectedBoard} onBack={() => selectBoard(null)} />;
  }
