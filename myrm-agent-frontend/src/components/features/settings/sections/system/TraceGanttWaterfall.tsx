...
const TraceGanttWaterfall = memo<TraceGanttWaterfallProps>(({ performance, totalDurationMs }) => {
  ...
  const { minStart, maxEnd, timeRange } = useMemo(() => {
    if (spans.length === 0) {
      return { minStart: 0, maxEnd: Math.max(totalDurationMs, 1), timeRange: Math.max(totalDurationMs, 1) };
    }
    ...
    return { minStart: minS, maxEnd: maxE, timeRange: range };
