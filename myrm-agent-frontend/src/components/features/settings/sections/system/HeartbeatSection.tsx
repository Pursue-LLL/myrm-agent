...
function parseCronExpr(expr: string): { hour: string; minute: string; dayPreset: DayPreset } {
  ...
  if (parts.length < 5) {
    return { hour: '9', minute: '0', dayPreset: 'everyday' };
  }
  return {
    minute: parts[0] ?? '0',
    hour: parts[1] ?? '9',
    ...
    getHeartbeatStatus()
      .then((s) => {
        ...
        if (s.schedule_kind === 'cron' && s.cron_expr) {
          ...
          if (s.interval_ms) {
            setIntervalMs(String(s.interval_ms));
          }
        }
        if (s.prompt) {
    ...
    >
      ...
      {agents.length > 0 && (
        <div className="space-y-2">
          ...
          {inheritedModel && (
            <p className="text-xs text-muted-foreground">{t('agentModelHint', { model: inheritedModel })}</p>
          )}
        </div>
      )}
