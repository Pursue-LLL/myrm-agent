...
| `AgentAuditView.tsx` | 子模块 | 组织级 Agent 行为审计视图：消费 CP `/api/enterprise/org/{org_id}/agent-audit/events` 聚合事件；KPI（总事件/工具调用/安全拦截/扫描沙箱数）直接使用 CP 聚合的全量分类计数 `tool_call_total`/`security_deny_total`（截断前统计，非返回事件 filter；「安全拦截」= harness 权威 deny 语义 BLOCK/DENY/REDACT/LEAK 决策数，>0 才用警示红、0 用常规色）；failed_sandboxes 告警条、事件流编排、时间范围选择（24h/7d/30d） | ✅ |
