...
| `EnterpriseMembersDialogs.tsx` | 子模块 | AddMember / UnlinkOauth 对话框（从 MembersTab 拆出，控制行数） | ✅ |
| `EnterpriseHandoffDialogs.tsx` | 子模块 | Offboard / Transfer 对话框（成员下拉选择，避免手输 user_id；source 非 owner、target 全员、自转禁用） | ✅ |
