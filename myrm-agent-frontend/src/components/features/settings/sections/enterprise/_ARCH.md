...
| `EnterpriseMembersTab.tsx` | 子 Tab | 组织信息、成员列表、离职交接、Volume 转移；全部管理操作（Add/Remove/Offboard/Transfer/Unlink）按 `isOrgAdmin` 门控，非 admin 仅只读；角色徽标本地化；Transfer source 仅列**已归档（offboard completed）成员**，防止 409 报错；错误消息全 i18n 化（6 语 `loadFailed`/`addMemberFailed`/`removeMemberFailed`/`offboardFailed`/`transferFailed`/`unlinkOauthFailed`），加载失败渲染「加载失败 + 详情」而非误导性 notAvailable | ✅ |
...
| `EnterpriseAuditTab.tsx` | 子 Tab | 审计大盘（Tabs 容器）：`platform` 平台审计（KPI 卡片、时间线图表、事件列表、导出）+ `agent` Agent 行为审计（`AgentAuditView`）；平台审计错误消息 i18n（`loadFailed`/`exportFailed`） | ✅ |
| `AgentAuditView.tsx` | 子模块 | 组织级 Agent 行为审计视图：消费 CP `/api/enterprise/org/{org_id}/agent-audit/events` 聚合事件；KPI（总事件/工具调用/安全拦截/扫描沙箱数）直接使用 CP 聚合的全量分类计数 `tool_call_total`/`security_deny_total`（截断前统计，非返回事件 filter；「安全拦截」= harness 权威 deny 语义 BLOCK/DENY/REDACT/LEAK 决策数，>0 才用警示红、0 用常规色）；failed_sandboxes 告警条、事件流编排、时间范围选择（24h/7d/30d）；错误状态结构化 `{title, detail}`（标题区分 `agentLoadFailed` 活动加载失败 / `agentOrgLoadFailed` 组织加载失败，明细透出 Error.message），错误消息全 i18n 化，无硬编码英文 | ✅ |
| `AgentEventRow.tsx` | 子模块 | Agent 审计事件行（含 tone 分类徽标、tool_name/事件类型标题、security decisions 详情、展开 JSON 明细）；工具事件识别基于 harness 真实事件类型 `tool_start`/`tool_end`；`eventTone` 分类以 security/approval/session/error 先行、`tool_` 前缀兜底（`tool_failure`/`tool_cancelled`/`tool_timeout` 归 error，`tool_approval_request` 与 `desktop_control_approval_request` 等含 `approval` 的事件归 approval）；`isDenyDecision` 与 harness 权威 deny 语义一致（`BLOCK|DENY|REDACT|LEAK`，`myrm-agent-harness/core/security/audit.py` `record_decision` 口径），决定 decision 行是否标红；消费 CP 注入的 `user_id`/`sandbox_id`/`user_display` 归属 → 渲染成员徽标（优先 `user_display` 邮箱/显示名，tooltip 含完整 user_id）；导出 `eventKey`（sandbox_id 前缀防跨沙箱 key 碰撞）与 `AgentEventRow` | ✅ |
| `EnterpriseUsageTab.tsx` | 子 Tab | 成本报表：月度用量进度、成员排行、分类分布、预算设置；错误消息 i18n（`loadFailed`/`saveFailed`） | ✅ |
