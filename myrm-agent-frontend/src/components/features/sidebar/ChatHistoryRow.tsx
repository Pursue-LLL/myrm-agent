...
export interface ChatHistoryRowProps {
	...
	onExport: (
		id: string,
		mode: "markdown" | "json" | "copy" | "html" | "print",
	) => void;
		...
		<div className="relative flex items-start">
					...
					<div
						className={cn(
							...
							isSelected
								...
								: "border-black/25 dark:border-white/25 hover:border-primary/60",
						)}
					>
						{isSelected && (
			...
			<div className="flex-1 min-w-0">
				<Link
					...
					title={
						sessionDragEnabled && !batchMode
							? t("chat.dragDrop.sessionDescription")
							: undefined
					}
					style={
						chat.projectId
							? { borderLeftColor: getProjectColor(chat.projectId) }
							: undefined
					}
				>
					{chat.source && chat.source !== "web" ? (
						<ChannelIcon
							channelId={chat.source}
							size={isMobile ? 14 : 16}
					...
					<div className="flex-1 min-w-0">
						...
						<p
							className={cn(
								"text-xs text-black/60 dark:text-white/60 truncate mt-1",
								isMobile && "text-[10px] mt-0.5",
							)}
						>
							{chat.lastMessage}
						</p>
						<div className="flex items-center gap-2 mt-1">
							...
							{chat.isCompacted && (
								<div className="flex items-center gap-0.5 text-[10px] px-1 py-[1px] rounded bg-primary/10 text-accent-warm font-medium">
									...
									<span className="scale-90 origin-left whitespace-nowrap">
										{t("chat.compact.badge")}
									</span>
								</div>
							)}
						</div>
							...
							>
								...
								<DropdownMenuSub>
									...
									<DropdownMenuSubContent>
										...
										<DropdownMenuItem
											onClick={() => {
												exportSessionZipPack(chat.id).catch((err: unknown) => {
													toast.error(
														err instanceof Error
															? err.message
															: t("chat.exportChat.exportZipFailed", {
																	defaultMessage: "导出会话日志包失败",
																}),
													);
												});
											}}
										...
										>
											...
											{t("chat.exportChat.exportZipPack", {
												defaultMessage: "导出会话日志包 (.zip)",
											})}
										</DropdownMenuItem>
									</DropdownMenuSubContent>
								</DropdownMenuSub>
								...
								{onRevealArtifacts && (
									...
									>
										...
										{isRevealingArtifacts
											? t("chat.revealArtifacts.revealing")
											: t("chat.revealArtifacts.action")}
									</DropdownMenuItem>
								)}
								{onCaptureEvalCase && (
									<DropdownMenuItem
								...
								<DropdownMenuItem
									...
									className={cn(
										...
										isMobile && "py-3 text-xs min-h-[44px]",
									)}
								>
...
export const SortablePinnedRow = memo<ChatHistoryRowProps>(
	({ chat, ...rest }) => {
		const {
			attributes,
			listeners,
...
function MoveToProjectMenu({
	...
	const handleMove = async (projectId: string | null) => {
		...
		useChatStore.setState({
			chatHistoryItems: items.map((item) =>
