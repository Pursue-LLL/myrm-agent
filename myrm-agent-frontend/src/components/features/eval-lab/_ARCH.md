...
- 单测（Vitest）：`__tests__/BenchmarkSources.test.tsx` 覆盖基准卡片渲染、评分徽标、下载/运行按钮，以及 Memory A/B 按钮 + 确认对话框（取消/确认调用 `onMemoryAb`）；`__tests__/MemoryAbHistoryTable.test.tsx` 覆盖历史表渲染（per-arm pass-rate + `memory_tool_calls`）、双模型披露（agent 模型显示/隐藏 + LLM judge 显示、native 隐藏）与选中回看。
