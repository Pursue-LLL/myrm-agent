...
| `CronJobCard.tsx` | 核心 | 单条 Cron 任务卡片（状态/下次运行/快捷操作）；active 周期任务若 next_run 超期 10min 显示 overdue 诚实警示条（本地=本机休眠/未运行 Myrm 原因；云端=调度顺延） | ✅ |
...
| `cron-utils.ts` | 辅助 | Cron 表达式解析、格式化与校验纯函数；`isCronOverdue`（active 周期任务 `now - next_run_at > 10min` 判定错过，供卡片逾期警示） | ✅ |
