...
export class SpriteEngine {
  ...
  private clampRow(row: number): number {
    if (this.effectiveRows === 0) {
      return 0;
    }
    if (row < 0) {
      return 0;
    }
    if (row >= this.effectiveRows) {
      return 0;
    } // fallback to idle (row 0)
    return row;
  }

  private clampFrame(frame: number): number {
    if (this.effectiveCols === 0) {
      return 0;
    }
  ...
  private renderFrame() {
    if (!this.ctx || !this.image || this.loadState !== 'ready') {
      return;
    }
