...
function rollStats(
  ...
  const values: number[] = STAT_NAMES.map((_, i) => {
    if (i === peakIdx) {
      return Math.min(floor + 50 + Math.floor(rand() * 20), 100);
    }
    if (i === dumpIdx) {
      return Math.max(floor - 10, 1);
    }
...
export function computeMood(input: MoodInput): Mood {
  ...
  if (input.isGoalComplete || input.isEvolutionRecent) {
    return 'excited';
  }

  const recentPet = input.lastPetAt && now - input.lastPetAt < 5 * 60 * 1000;
  if (recentPet && input.petCount >= MOOD_HAPPY_PET_THRESHOLD) {
    return 'happy';
  }

  if (input.conversationCount >= MOOD_CURIOUS_CONVERSATION_THRESHOLD) {
    return 'curious';
  }
...
export function generateCompanion(userId: string): CompanionBones {
  if (cachedUserId === userId && cachedBones) {
    return cachedBones;
  }
