 ...
 *
 * [OUTPUT]
 * - InlineDiffViewer: Diff 对比预览组件
 *   - 支持 unified/split 视图模式
 *   - 语法高亮（基于文件扩展名）
 *   - 新增/删除行数统计
 *
 * [POS]
 * Markdown 渲染层的 Diff 预览组件。将 unified diff 格式
 * 解析并以可视化方式内联展示，支持行号、颜色标记、统计信息，支持多端。
 */
...
import { CLIFileIcon } from '../cli-visualization/CLIFileIcon';
 ...
 * 内联 Diff 预览组件
 */
export const InlineDiffViewer: React.FC<InlineDiffViewerProps> = memo(
  ({ diff, filePath, onClose, defaultViewMode = 'unified', className }) => {
    ...
    return (
      <motion.div
        ...
        exit={{ opacity: 0, y: 10 }}
        className={cn('bg-background border border-border rounded-lg overflow-hidden my-6', className)}
      >
        {/* Header */}
