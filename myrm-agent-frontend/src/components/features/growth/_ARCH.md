...
| `ManifestPredictionsPanel.tsx` | 组件 | 变更清单与可证伪预测归因面板：展示演进前预期指标提升（方向/基准/目标/容差）与评测后实测对比归因判定（CONFIRMED/REFUTED/REGRESSION）及回滚补丁建议          | ✅    |
...
| `SkillHealthPanel.tsx`         | 组件 | 技能健康度与周复盘治理面板。支持全量/待治理/健康分类筛选，展示 Harness 纯规则健康分与治理处置建议                                                          | ✅    |
