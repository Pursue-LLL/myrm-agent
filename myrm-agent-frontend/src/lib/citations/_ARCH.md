...
| 文件 | 地位 | 职责 | I/O/P |
| ---- | ---- | ---- | ----- |
| `preprocessCitationMarkers.ts` | 核心 | 解析单引用、复合引用（`[1, 2]`、`【1, 2】`、`【1-3】`）、变体括号与 `[citation:Title](url)` → `<citation>` 标签；按 `source.index` 匹配 | ✅ |
...
| `__tests__/preprocessCitationMarkers.test.ts` | 测试 | 单引用、复合引用、范围展开、变体括号与 code mask 单测 | — |
