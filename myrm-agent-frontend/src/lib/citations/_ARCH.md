...
| 文件 | 地位 | 职责 | I/O/P |
| ---- | ---- | ---- | ----- |
| `preprocessCitationMarkers.ts` | 核心 | 解析单引用、复合引用（`[1, 2]`、`【1, 2】`、`【1-3】`）、变体括号与平衡小括号 URL `[citation:Title](url)` → `<citation>` 标签；泛称标题智能域名回退；清洗大模型私有控制 token（`\ue200cite`）；按 `source.index` 匹配 | ✅ |
