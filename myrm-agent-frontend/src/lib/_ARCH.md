...
| 目录 / 文件 | 职责 | 文档 |
|-------------|------|------|
| `api.ts` | 通用 fetch 封装；`apiRequest` 本地 gate 强制（`BACKEND_UNREACHABLE`）+ Next 代理纯文本 500 归一化；`fetchWithTimeout` SSOT 注入 mobile pair header | — |
...
| `local-backend-dev.ts` | Boot session SSOT + health-aware local setup hint（Boot/Banner/Settings）；`resolveBackendUnreachableMessage`（api 层无 i18n 指引） | — |
