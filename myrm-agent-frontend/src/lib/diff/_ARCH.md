...
| `DiffViewer.tsx` | 核心 | 共享 Diff 可视化组件（Unified/Split 视图、Prism 语法高亮；支持 `embedded` 内联模式） | ✅ |
| `TextDiffViewer.tsx` | 核心 | 基于 `createPatch` 的纯文本 old/new 对比薄包装 | ✅ |
| `__tests__/parseUnifiedDiff.test.ts` | 测试 | 覆盖 new-file、multi-hunk、`---/+++` 回退等解析分支 | ✅ |
| `__tests__/TextDiffViewer.test.tsx` | 测试 | 纯文本 old/new diff 渲染冒烟 | ✅ |
