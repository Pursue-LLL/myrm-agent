...
export interface DiffViewerProps {
  ...
  className?: string;
  /** Skip chrome header/toolbar for inline embedding in dialogs and cards. */
  embedded?: boolean;
...
export const DiffViewer: React.FC<DiffViewerProps> = memo(
  ({ diff, filePath, onClose, defaultViewMode = 'unified', className, embedded = false, maxHeight }) => {
    ...
    const content = (
      <>
        ...
        <div
          className={cn('overflow-auto', embedded ? 'max-h-[300px]' : 'max-h-[400px]')}
    ...
    if (embedded) {
      return <div className={cn('overflow-hidden text-xs font-mono', className)}>{content}</div>;
    }
