...
| 握手 | 扩展发送 `hello`（version、browser、capabilities）；Server 校验 `settings.extension_auth_token` 并按 capability 执行动作门禁 |
