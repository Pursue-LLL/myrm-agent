...
| `src/background.js` | 核心 | Service Worker：WebSocket 连接（四态 badge：ON/…/空/!）、心跳保活、CDP 请求分发、标签页生命周期、智能 Tab 选择（同 domain 多 tab 优先 active + tabId 直传）、断线原因追踪、后台窗口隔离（`ensureBackgroundWindow` 非聚焦窗口管理、持久化/复用/自动清理） | ✅ |
