...
| `src/sidepanel/sidepanel.js` | 核心 | Side Panel 控制器：通过 HTTP+SSE 与 server chat API 通信、SSE 流消费、消息渲染、工具进度可视化、上下文自动附加（当前标签页信息）、选中文本引用、工具审批 UI、Glow 控制、新建聊天 | ✅ |
...
| `/api/v1/approvals/{approvalId}/respond` | POST | 响应工具审批 |
