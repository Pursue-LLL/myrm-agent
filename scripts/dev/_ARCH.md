...
| `ensure-browser-orchestrator.sh` | Unix | 启动/复用单一 Browser Orchestrator daemon；单实例、CDP 健康检查和 build 收敛；正式 E2E 的唯一浏览器数据面 |
...
8. **client_hot + infra browser registry**：`ready --chrome` 用短生命周期 exact target 预热 client chunk，hydrate 后立即关闭并注销到 `infra-browser-targets.json`；`./myrm doctor --chrome` 输出 Tab Hygiene（`detail` 含 `cdp_pages`/`wave_bound`/`infra_registry`/`unbound`；JSON 含 `unboundPages`。`ok` = CDP 可达 ∧ 账本可读（fail-closed：账本不可读即不 green）。`unbound` 是**观测信号**而非硬门禁：只统计**非默认 context**（orchestrator 每 session 一个专用 context）中、无任何账本认领的页面；Chrome 默认 context 的 AOS 锚点与 infra 预热页本无 session 账本，**不计入**（否则空闲健康面会被误报漂移）。create→publish 窗口与活跃测试期间 `unbound>0` 属正常，据此翻红会产生假红；泄漏漂移看它的长期单调上升，不看单次快照）；mux 按 client 隔离 page ownership；改码后 UI 测 **`./myrm restart --chrome`** 开新 wave
