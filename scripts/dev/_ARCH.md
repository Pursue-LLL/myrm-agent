...
| `chrome-e2e-seed-providers.mjs` | Bun | seed 逻辑模块（local 模式免 WebUI 登录；`LITE_MODEL` → `liteModel.primary`；已有 default 时 env 漂移 patch **`apiUrl`/`apiKey`/`name`**，避免 openai-like 仍指向旧 gateway） |
