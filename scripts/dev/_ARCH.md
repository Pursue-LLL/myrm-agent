...
| `dev-stack.sh` | Unix | 本地 dev 栈 SSOT：`ensure` / `attach` / `reset` / `status`；`attach` 只读等待 **`MYRM_STACK_ATTACH_WAIT_SEC` 总预算**（warm + 剩余 health）；`cmd_ensure` 三分支：已热栈 idempotent OK；冷栈 + wave pin + 端口在听 → attach-wait；冷栈 + wave pin + 栈 down → `STACK_FAIL`；**必须**委托 **stack_supervisor** 单写者；state `~/.local/state/myrm-dev/` |
...
| `chrome-e2e-preflight.sh` | Unix | 首 Agent 完整 reconcile/client_hot；attach 聚合快照；private backend 路径 `_wait_shared_ui_reachable`（默认 180s，signoff contract 600s）；attach `_heal_mux_request_timeout_drift`（`request-timeout-ms-at-daemon-start`）；mux 超时 stamp 漂移且 contexts=0 时可安全重启 daemon；输出 `CHROME_E2E_HEALTH_JSON` |
