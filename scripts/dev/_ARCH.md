...
| `verify-frontend-dev-pause-gate.sh` | Unix | **frontend dev pause 集成验收**：isolated pause dir 下 dev-stack SKIP · warm_ui `paused_skip` · `dev.ts` refuse · 裸 `bunx next dev` blocked · clear-pause 解除（`FRONTEND_PAUSE_GATE_OK/FAIL`） |
