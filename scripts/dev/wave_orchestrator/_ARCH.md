...
- [runtime_probe.py](../lib/runtime_probe.py) — `runtimeId` 探针；`reap()` 在 signoff matrix（活跃 lease 或 `MYRM_SIGNOFF_MATRIX=1` bootstrap）时原地 heal runtimeId，否则 drift invalidate
