...
| `ledger register <leaseId> <kind> <ref>` | 登记测试资源（须 RESOURCE_WRITE lease） |
...
**Resource Ledger**：`RESOURCE_WRITE` 租约创建 chat 等业务资源后，须 `./myrm wave ledger register <leaseId> chat <chatId>`；`lease release` / TTL 过期自动 HTTP 清理（`resource_cleanup.py` → `DELETE /api/v1/chats/...`）。
