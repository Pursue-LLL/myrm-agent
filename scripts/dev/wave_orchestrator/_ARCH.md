...
- [runtime_probe.py](../lib/runtime_probe.py) — `runtimeId` 探针；signoff matrix / signoff-chrome.lock 期间 `core.probe_runtime_id()` 与 `e2e_bootstrap` 同源使用 `_read_shared_hot_stack_runtime_id()`；`reap()` 在 signoff matrix（活跃 lease 或 `MYRM_SIGNOFF_MATRIX=1` bootstrap）时原地 heal runtimeId，否则 drift invalidate
