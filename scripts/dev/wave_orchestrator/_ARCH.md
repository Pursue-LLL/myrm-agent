...
| 文件 | 职责 | I/O/P |
|------|------|-------|
| `paths.py` | 解析 state dir（`MYRM_WAVE_STATE_DIR` 优先）+ `resolve_dev_state_dir()` SSOT（signoff lock / mux admission）；`state_file` 为 `wave-orchestrator.json` 路径 | ✅ |
...
- [runtime_probe.py](../lib/runtime_probe.py) — `runtimeId` 探针；signoff matrix / signoff-chrome.lock 期间 `core.probe_runtime_id()` 与 `e2e_bootstrap` / `e2e_runtime_guard` 同源使用 `_read_shared_hot_stack_runtime_id()`；`reap_runtime_drift` 在 signoff lock / matrix 活跃时仅 heal 或 no-op，**永不** drift-invalidate 共享 immutable wave
