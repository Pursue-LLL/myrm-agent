...
| `frontend-client-warmup.py` | Unix | CDP `Target.createTarget(background=true)` 预热 `:3000/` 直至 `[data-testid="app-layout"]` + `[data-chat-input]`；注册 `infra-browser-targets.json`。实际位置 [browser_orchestrator/frontend_client_warmup.py](browser_orchestrator/frontend_client_warmup.py) |
