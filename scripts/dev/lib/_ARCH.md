...
| `browser_orchestrator_e2e.py` | Unix | Orchestrator 路径 E2E page lifecycle（`MYRM_BROWSER_ORCHESTRATOR=1`）；body 入口 `_ensure_orchestrator_session` idempotent create；parallel `open_page` 遇 stale/`No context for session` 时 `_recreate_orchestrator_session` destroy→create 后重试 |
...
| `cdp_chat_{transport,bootstrap,input,submit,turn,support}.py` | Unix | transport-independent chat UI 工作流；MCP 与 client warmup 复用；**R72-FINAL** `cdp_chat_submit.send_chat_message_atomic` 仅 `submitAndObserveTurn` fail-closed；`cdp_chat_turn.send_message` 强制 `sendTurnSealed`；**§26.22** `wait_turn_done` 统一观测链（bridge→goal API→DOM）、`chat_id_hint` 初值、`okViaGoal` 经 `is_persisted_e2e_goal` 校验、节流 `E2E_OBSERVE`；**R50** SHPOIB 缩放 + bridge fallback；computer_use/builtin-tools 须 React bridge |
...
| `e2e_session_runtime/snapshot.py` | Unix | per-pid session snapshot；**R96-B6** `nodeStartedMonotonic`（同 `currentNode` 不重置）· `body_elapsed` / `progress_stale` / `node_elapsed` |
| `e2e_live_chrome_pytest_scan.py` | Unix | live chrome_e2e pytest 进程扫描 SSOT（wrapper dedupe + snapshot pid 关联）；`list_live_chrome_e2e_pytest_rows()` canonical per-test_id 一行 |
| `e2e_session_runtime/heartbeat.py` | Unix | 统一 `heartbeat_once` SSOT（coordinator + wave lease + runtime）；shell 30s 循环在无显式节点时从 pytest snapshot 桥接真实 currentNode |
| `e2e_session_runtime/registry.py` | Unix | 统一 E2E session registry — ADMIT through BODY (R144 SSOT)；`list_live_e2e_sessions()` 去重 session 列表；P0-A: coordinator 活跃时禁用 ps fallback |
...
| `e2e_session_runtime/lifecycle.py` | Unix | **R62/R96-R62** 四相位 budget SSOT（ADMIT/BOOTSTRAP/BODY/TEARDOWN）；dev LIVE_AGENT BODY **600s** · READ/signoff **600s**；**R67** `begin_bootstrap_phase` 始终切 BOOT 180s · `complete_bootstrap_phase` 重置 BODY |
| `stack_mutation_policy.py` / `stack_mutation_policy.sh` | Unix | R30 SMP SSOT：shared-stack drift heal defer under active wave leases **or dev-stack ensure.lock.d**；`pending-stack-drift.json`；`shared_api_http_ok` 读 `E2E_API_BASE`/`MYRM_BACKEND_PORT`；preflight/bootstrap/supervisor 统一入口；**R46** attach crash heal（3× `backend-only ensure` + backoff 5s/10s；preflight wait 每 30s 再 heal max 2）；**R46.1** attach 健康探针（`CHROME_E2E_ATTACH_HEALTH_PROBE_*`）先判 backend/pending drift，再进入 mux attach；idle apply 若命中 harness import 失败会自动执行一次 `./myrm harness install` 后重试 `backend-only ensure` |
...
| `e2e_api_verify.py` | Unix | Agent `./myrm verify-api` / `e2e-context` SSOT：stored/workspace fingerprint epoch 匹配路由；blocked+epoch mismatch 时 stderr **`E2E_BLOCKED_EPOCH`**；operation 饱和返回可立即 launch 的 `OPERATION_BACKPRESSURE`，不复用 PRIVATE session wait token；`muxColdAttachSaturated`/`muxHandProbeAllowed`；默认 human/json 含 `parallelSnapshot` + `capHeadroom` + `leaseLiveness`；`launch-check` 子命令 |
| `e2e_parallel_status.py` | Unix | `capHeadroom`/`queueLayer` SSOT：`compute_queue_layer()` 区分 session（仅 PRIVATE 排队）与 operation（mux 背压）；两者并存时 `queueLayer=operation` 保留 SHARED launch 语义，`privateWaiting` 独立呈现 PRIVATE 队列 |
