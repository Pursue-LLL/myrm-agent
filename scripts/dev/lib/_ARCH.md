...
| `module_alias.py` | Unix | moved Dev Gate 模块的 proxy-module helper（兼容导入共享同一 runtime object，供 monkeypatch） |
...
> 根级 `dev_gate_*.py` / `cdp_chat_*.py` / `chrome_mcp_*.py` / `mcp_*.py` / `e2e_*.py` / `browser_orchestrator_*.py` / `mux_*.py` / `transport_*.py` 为 **P1–P5 域重组的 module-alias shim**（见 `DOMAIN_REPACK_PLAN.md`），canonical 实现在上表子包中；P6 计划删除根 shim 前，调用方仍可经根平铺名导入。
