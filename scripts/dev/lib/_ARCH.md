...
| `e2e_shared_ui_session.py` | Unix | chrome_e2e Shared UI Session Contract（marker `e2e_search_policy` · RESET/BIND/BRIDGE/SEARCH 四阶段 · bootstrap + `click_new_chat` hook；**R55** empty 两态契约：`RESET_GLOBALS_KEEP_SEARCH_BLOCK_JS` 保留 `__MYRM_E2E_BLOCK_SEARCH_SYNC__`，同 nodeid+api 首轮强清理，重试快路径 short-circuit） |
...
| `dev_gate_contract.py` | Unix | Dev Gate v2 SSOT：**R47** `MUX_PAGE_RECLAIM_HARD_TIMEOUT_SEC` / `SHELL_PROBE_STALL_FAIL_FAST_SEC` / `MUX_RECLAIM_STALL_TOKEN`；并行 cap（LIVE SHPOIB **4** / shared_hot **1** / mux **6** / cold attach **3**）；**`E2E_UNIFIED_WAIT_SEC=300`**；**`LIVE_SINGLE_TEST_WALL_CLOCK_SEC=600`**；lane pytest timeout（READ=1110 / LIVE=1710 / desktop=600）；`chrome_e2e_skips_shared_approval_preflight(lane, shpoib)`；**R57** `TRANSIENT_MUX_ERROR_TOKENS` 新增 `"Chrome MCP transport closed"` |
