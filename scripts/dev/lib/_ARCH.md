...
| `e2e_stale_lease_reap.py` | Unix | hung pytest SIGINT + wave reap：**R62** body≥600 · progress_stale≥90 · **R96-B6** `E2E_NODE_STUCK` transport node≥120s |
...
| `e2e_lease_liveness.py` | Unix | **R74-OBS-3/4** wave snapshot SSOT · `E2E_LEASE_LIVENESS` · `live_agent_shpoib` vs `read_page_leases` cap 语义 |
