...
| `e2e_session_runtime/heartbeat.py` | Unix | 统一 heartbeat SSOT（coordinator + wave lease + runtime）；shell 30s 循环在无显式节点时从 pytest snapshot 桥接真实 currentNode；`heartbeat_e2e_lease` 为兼容别名 |
