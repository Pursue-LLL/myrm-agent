...
| `chrome_mcp_client.py` / `chrome_mcp_errors.py` / `mcp_protocol.py` / `mcp_chat_ui.py` | Unix | 正式 pytest UI E2E 的 MCP JSON-RPC client；**R47** reclaim **120s** 硬墙 + `MUX_RECLAIM_STALL` → `recover_mux_transport`；`mcp_chat_ui.evaluate` 实例 `asyncio.Lock` + `wait_for(to_thread)` wall deadline；`cdp_chat_bootstrap._wait_shell_layout_ready` stall fail-fast；`call_tool` transient/timeout 有限重试；page client 须 active 父 lease；`reclaim_owned_page` |
