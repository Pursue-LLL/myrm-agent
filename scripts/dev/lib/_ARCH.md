...
| `chrome_mcp_client.py` / `chrome_mcp_errors.py` / `mcp_protocol.py` / `mcp_chat_ui.py` | Unix | 正式 pytest UI E2E 的 MCP JSON-RPC client；**R47** reclaim **120s** 硬墙 + `MUX_RECLAIM_STALL` → `recover_mux_transport`；**R48** `abandon_inflight_requests` generation token + shell probe `evaluate_task.cancel()`；**R49** abandon/recover **同步**（禁止 `to_thread(abandon)` 线程池死锁）；**R50** abandon 替换 `_request_lock` + recover 前必 abandon；**R51** `reset_after_orphan()` 单入口（generation++ · teardown · 新 Lock · recover）替换 scattered abandon+recover；`_wait_shell_layout_ready` 用 `layout_wait_started` 防 hydrate 重置 stall 时钟；`mcp_chat_ui.evaluate` wall deadline；`cdp_chat_bootstrap._hydrate_chat_home_surface` SHPOIB；`call_tool` transient/timeout 有限重试；page client 须 active 父 lease；`reclaim_owned_page` |
| `cdp_chat_{transport,bootstrap,input,submit,turn,support}.py` | Unix | transport-independent chat UI 工作流；MCP 与 client warmup 复用；**R50** `ensure_chat_surface` SHPOIB 缩放 + `_hydrate_chat_home_surface` + `ensure_react_e2e_bridge` fallback；`cdp_chat_input._heal_empty_chat_shell_for_bridge` 走 `_shared_ui_burst`；`cdp_chat_input.ensure_react_e2e_bridge` 拒绝 DOM fallback，blank shell 时 heal 重导航；`cdp_chat_bootstrap._wait_providers_hydrated` 的 API readiness probe 走 `to_thread + wait_for` 非阻塞守门；computer_use/builtin-tools 须 React bridge |
...
| `e2e_shared_ui_session.py` | Unix | chrome_e2e Shared UI Session Contract（marker `e2e_search_policy` · RESET/BIND/BRIDGE/SEARCH 四阶段 · bootstrap + `click_new_chat` hook；R56 empty 两态契约：同 nodeid+api 首轮强清理，重试快路径） |
...
| `dev_gate_contract.py` | Unix | Dev Gate v2 SSOT：**R47** `MUX_PAGE_RECLAIM_HARD_TIMEOUT_SEC` / `SHELL_PROBE_STALL_FAIL_FAST_SEC` / `MUX_RECLAIM_STALL_TOKEN`；并行 cap（LIVE SHPOIB **4** / shared_hot **1** / mux **6** / cold attach **3**）；**`E2E_UNIFIED_WAIT_SEC=900`**；**`LIVE_SINGLE_TEST_WALL_CLOCK_SEC=600`**；lane pytest timeout（READ=1110 / LIVE=1710 / desktop=600）；`chrome_e2e_skips_shared_approval_preflight(lane, shpoib)` |
| `e2e_wall_budget.py` | Unix | R39 monotonic **600s** wall budget SSOT：`assert_wall_budget` · `stream_wait_cap_sec` · holder stale helpers |
| `stack_mutation_policy.py` / `stack_mutation_policy.sh` | Unix | R30 SMP SSOT：shared-stack drift heal defer under active wave leases；`pending-stack-drift.json`；preflight/bootstrap/supervisor 统一入口；**R46** attach crash heal（3× `backend-only ensure` + backoff 5s/10s；preflight wait 每 30s 再 heal max 2）；**R46.1** attach 健康探针（`CHROME_E2E_ATTACH_HEALTH_PROBE_*`）先判 backend/pending drift，再进入 mux attach |
