...
[POS]
myrm-agent-desktop L1 架构文档。Tauri 桌面壳整体架构、双进程编排与模块导航。

> **许可**: MIT · **父仓**: [myrm-agent/ARCHITECTURE.md](../ARCHITECTURE.md)  
> **定位**: Tauri 桌面壳 — WebView + 系统 API + Python/Next.js 双进程编排
...
| 路径 | 职责 | 文档 |
|------|------|------|
| `src-tauri/` | Rust 主程序、Tauri 配置、图标 | [src-tauri/_ARCH.md](src-tauri/_ARCH.md) · [src-tauri/src/_ARCH.md](src-tauri/src/_ARCH.md) |
| `src-tauri/permissions/` | AppManifest 命令权限清单（构建期 autogenerated ACL） | 与 `src-tauri/src/_ARCH.md` 同步维护 |
...
| `sidecar/` | PyInstaller 后端 Sidecar 构建入口 | [sidecar/_ARCH.md](sidecar/_ARCH.md) |
...
- `scripts/check-fractal-docs.ts` — 必检 `_ARCH [POS]` + 递归目录 `_ARCH` + 核心 `[INPUT]` + 核心安全文件与 `permissions/autogenerated/*.toml` 全量 git 跟踪状态 + 命令清单/ACL 一致性 + Rust 行数预算
