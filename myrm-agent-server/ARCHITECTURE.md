...
- **Server 工具可见性**：运行时由 harness `attenuator`（执行层）与 `DeferredToolMiddleware`（`tool_choice.allowed_tools` 模型层）约束；Server `app/tasks/` 不含闲置工具驱逐后台任务。
...
Agent 级 `tool_gateway_config` 与 Harness `ToolGatewayConfig` 支持 Search / Image / TTS 等工具经 Control Plane `tool_relay` 转发；Local BYOK 由 Models、Search、Media Generation 等 Settings 页配置。CP `experimental/tool_relay` 未挂载生产路由时，WebUI 不展示 Gateway 配置入口。Harness 在 gateway 失败且本地 API key 可用时自动 fallback 至直连 provider。
