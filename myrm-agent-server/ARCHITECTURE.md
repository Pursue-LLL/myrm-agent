...
- **依赖分层**：主依赖 `httpx`、`filelock`（cron/多进程锁）、`tenacity>=9.1.4`（`vercel_client`）、`myrm-agent-harness[...,computer-use]`（桌面键鼠经 harness `computer-use` extra 传递 pyautogui，server 不重复 pin）。可选 extra：`channels-sdk`、`matrix` / `matrix-e2ee` / `wechat-silk`（GPLv3）/ `voice-tts`（GPL-3.0）/ `local-stt`（faster-whisper，`voice/stt.py`）/ `data-viz`。Desktop PyInstaller 与 OSS 开发默认安装（`scripts/lib/server_sync_flags.sh` → `setup.sh` / `myrm update` / `sync-server-venv.sh`）均用 `--all-extras --no-extra matrix-e2ee --no-extra voice-tts --no-extra wechat-silk`，排除 GPL 与 E2EE 重量依赖；Docker 官方镜像仍 `--all-extras` 全装。本地 STT：`uv sync --extra local-stt`；Edge TTS：`uv sync --extra voice-tts`。`[dependency-groups] sandbox`：`prometheus-fastapi-instrumentator` / `granian`。
