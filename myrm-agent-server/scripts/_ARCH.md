...
| dev/ | 工具 | Server 本地 pytest / profiling 辅助 · [dev/_ARCH.md](dev/_ARCH.md) | ✅ |
...
scripts/ci/ ──→ run_architecture_gates.sh（fractal + line budget + ruff lint + promtool rules check + architecture pytest）
scripts/ci/ ──→ run_default_tests.sh（ruff lint + 默认 pytest；-m 'not e2e and not performance'）
```
