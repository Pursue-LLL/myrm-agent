...
| `factory.py` | ✅ 核心 | Agent 实例组装工厂…`needs_runtime_pool()` + `enable_external_cli` 双门控 RuntimePool/`delegate_to_agent_tool`；Sandbox 下 `_should_enable_cron_tools()` 控制 cron 是否加载；`_should_setup_computer_use_tools()` 按 deploy/VNC 能力门控 desktop×4 + DESKTOP_CONTROL_RULES；subagent 为基础能力（WU 余额门禁消费）。 | ✅ |
