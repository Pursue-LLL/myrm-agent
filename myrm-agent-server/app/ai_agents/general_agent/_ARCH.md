...
| `tool_setup.py` | ✅ 核心 | 工具初始化混入（ToolSetupMixin）。搜索、图片/视频、`_setup_x_live_search_tool`（x-live-search skill → deferred `x_search_tool`，不依赖 enable_web_search）、定时任务、记忆、浏览器、local_browser、`_setup_clarification_tools`（ask_question HITL）、A2UI 等。继承 ExternalAgentsMixin。 | ✅ |
