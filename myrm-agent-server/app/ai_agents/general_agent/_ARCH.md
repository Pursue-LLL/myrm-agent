...
历史会话召回在 Server 层装配为 `conversation_search_tool`（GeneralAgent **eager**，与 memory_recall 对称）；Harness 只消费 Protocol，不感知数据库或产品身份语义。首 turn 常见召回由 `auto_session_recall_middleware` 自动注入。
...
| `conversation_search_setup.py` | ✅ 辅助 | 将 Server 会话历史召回 Provider 绑定到 GeneralAgent eager `tools`；CustomAgent 仍 scoped eager。 | ✅ |
