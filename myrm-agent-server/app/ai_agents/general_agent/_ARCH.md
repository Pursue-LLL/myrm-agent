...
| `factory.py` | ✅ 核心 | Agent 实例组装工厂…`active_tool_groups.derive_active_tool_groups` 为 Gap/skill group SSOT；`enable_external_cli` + `should_mount_delegate_tool()` 双门控 `delegate_to_agent_tool` Turn1 挂载；Sandbox 下 `_should_enable_cron_tools()` 控制 cron 是否加载；subagent 为基础能力（WU 余额门禁消费）。 | ✅ |
...
| `external_agents.py` | ✅ 核心 | 外部 Agent 委托层（ExternalAgentsMixin）。RuntimePool 初始化、CLI/ACP/SDK 后端注册、本地自动检测、直接委托流式转发。`should_mount_delegate_tool`：CLI Visual 或 `force_delegate_agent` 时跳过 Turn1 挂载（与 `factory.py` 的 `enable_external_cli` 开关 AND）。有 chat scope 时经 `services/external_agents/runtime_pool_registry` 跨消息复用 pool（CLI `--resume`）+ Facade per-chat turn lock；无 scope 时 ephemeral pool。会话内 HealthMonitor。 | ✅ |
