...
| `tool_setup.py` | ✅ 核心 | 工具初始化混入（ToolSetupMixin）。搜索、图片/视频生成、定时任务、记忆、浏览器、交互式澄清（ask_question_tool）、A2UI（`render_ui_tool`，`enable_render_ui` 时 Deferred 加载）等工具创建逻辑。继承 ExternalAgentsMixin。 | ✅ |
