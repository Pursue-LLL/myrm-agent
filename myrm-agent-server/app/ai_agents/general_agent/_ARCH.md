...
| `factory.py` | ✅ 核心 | Agent 实例组装工厂…`active_tool_groups.derive_active_tool_groups` 为 Gap/skill group SSOT；Sandbox 下 `_should_enable_cron_tools()` 控制 cron 是否加载；subagent 为基础能力（WU 余额门禁消费）。 | ✅ |
