...
| `tool_setup.py` | ✅ 核心 | 工具初始化混入（ToolSetupMixin）；GeneralAgent 工具装配；`_create_memory_tools` 绑定 `memory_search_tool` sessions/wiki ACL；`_setup_clarification_tools` 解析 `tool_description_locale` 注入中英双语提示词；`factory.py` 条件挂载 `skill_market_tool` / `skill_manage_tool`。 | ✅ |
