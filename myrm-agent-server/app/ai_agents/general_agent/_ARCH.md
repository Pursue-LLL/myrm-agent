...
历史会话召回在 Server 层装配为 `conversation_search_tool`（GeneralAgent **deferred**，经 discover_capability 挂载）；Harness 只消费 Protocol，不感知数据库或产品身份语义。首 turn 常见召回由 `auto_session_recall_middleware` 自动注入。
...
| `factory.py` | ✅ 核心 | Agent 实例组装工厂，将所有组件构建为 LangGraph 引擎。Kanban：`enable_kanban=False` 时不 bind；为 True 时经 `kanban_tool_mode.py` 解析 bind；worker lifecycle prompt 与 resolve 同条件（task_id→worker）。`_build_session_cleanup_callback` 组合 commitment extraction 和 correction propagation 两个会话清理回调。通过 AgentExtension 模式注册功能模块。Action Space Opt-In: 根据 UserSkillConfig 传递 `allowed_prebuilt_ids` 白名单给 `create_skill_backend`，根据 `enabled_builtin_tools` 控制 `enable_file_tools`/`enable_bash`/`enable_wiki`/`enable_planning`/`enable_answer_tool`，MCP 技能自动注入 PTC 依赖。Active Goal 会话强制 `enable_planning=True`。从 `enable_xxx` 布尔标志推导 `AgentRuntimeSpec.tool_groups`（含 `planning`、`answer_tool`）用于 skill 条件激活过滤。 | ✅ |
...
| `tool_setup.py` | ✅ 核心 | 工具初始化混入（ToolSetupMixin）。搜索、图片/视频/TTS（`enabled_builtin_tools` + 凭证 → AgentDeclared eager，壳在 `../media_tools/`）、`_setup_x_live_search_tool`（x-live-search skill → deferred `x_search_tool`）、定时任务、记忆、浏览器、local_browser、`_setup_clarification_tools`（ask_question HITL）、A2UI 等。继承 ExternalAgentsMixin。 | ✅ |
...
| `conversation_search_setup.py` | ✅ 辅助 | 将 Server 会话历史召回 Provider 绑定到 GeneralAgent `deferred_tools`（discover 按需挂载）；CustomAgent 仍 scoped eager。 | ✅ |
