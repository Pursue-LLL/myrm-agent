...
| `factory.py` | ✅ 核心 | Agent 实例组装工厂…Kanban：`_resolve_kanban_default_board_id()` 优先 `kanban_default_board_id`（chat 所选板），无效时 fallback 最新板；`needs_runtime_pool()` + `enable_external_cli` 双门控… | ✅ |
