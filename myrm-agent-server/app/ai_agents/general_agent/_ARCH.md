...
| `tool_setup.py` | ✅ 核心 | 工具初始化混入（ToolSetupMixin）。… `factory.py` 条件挂载 `skill_market_tool`（profile `skill_market`）与 `skill_manage_tool`（profile `skill_evolution` 或 `/learn` `force_skill_manage`）。… | ✅ |
