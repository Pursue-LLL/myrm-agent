...
历史会话召回在 GeneralAgent 通过 `memory_search_tool(corpus=sessions)` ACL 启用（用户设置 `memoryEnableConversationSearch`，默认关闭；无痕模式禁用 sessions corpus）。CustomAgent 仍可直接挂载 standalone `conversation_search_tool`。Harness 只消费 Protocol，不感知数据库或产品身份语义。
...
| `conversation_search_setup.py` | ✅ 辅助 | Legacy Turn1 bind helper（仅测试/兼容）；GeneralAgent 会话搜索走 `tool_setup._create_memory_tools` 的 `memory_search_tool` sessions ACL。CustomAgent 使用 `custom_agent_factory._append_scoped_conversation_search`。 | ✅ |
