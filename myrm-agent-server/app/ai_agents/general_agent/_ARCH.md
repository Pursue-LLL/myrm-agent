...
| `tool_setup.py` | ✅ 核心 | 工具初始化混入（ToolSetupMixin）。`web_fetch_tool`（Turn1，`enable_web_fetch` 由 Agent Security `net_fetch` 门控）、搜索、图片/视频/TTS（`enabled_builtin_tools` + 凭证 → AgentDeclared eager，壳在 `../media_tools/`）、`_setup_x_live_search_tool`（x-live-search skill 已绑定 → eager `x_search_tool`）、`render_ui`/`computer_use`（用户开关 ON + `_should_mount_render_ui_tools` WEB_CHAT/web|tauri → Turn1 eager；IM/cron/headless 不挂载）、`../desktop_control/gate.py` `DesktopControlGate` + LOCAL `background_strict`）、`enable_render_ui` 时 seed A2UI spec 至 `{workspace}/.agent/docs/A2UI_REFERENCE.md`、定时任务/cron（`enable_cron_eager` ON → Turn1 eager；OFF 不加载）、记忆、浏览器、`_setup_clarification_tools`（`ask_question` HITL；`structured_clarify` ON + `web_chat` + `!unattended` + `prompt_mode!=search`；900s 后端 timeout 自动 no_answer resume）等。继承 ExternalAgentsMixin。 | ✅ |
| `external_agents.py` | ✅ 核心 | 外部 Agent 委托层（ExternalAgentsMixin）。RuntimePool 初始化、CLI/ACP/SDK 后端注册、本地自动检测（请求热路径 `detect(include_version=False)`）、直接委托流式转发。挂载 `delegate_to_agent_tool` 时注入 `delegate_cwd`（与 executor workspace 对齐）与 `session_scope=chat_id`。`needs_runtime_pool()` / `should_mount_delegate_tool()` 门控；chat scope 经 `runtime_pool_registry` 复用 pool + Facade turn lock。RuntimePool fingerprint 对齐 `RuntimeConfig` 关键字段（含 `env/cwd/timeout/maxResponseChars/permissionMode/maxTurns`）。 | ✅ |
