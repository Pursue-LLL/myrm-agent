...
| `tool_setup.py` | ✅ 核心 | 工具初始化混入（ToolSetupMixin）。搜索、图片/视频生成、定时任务、记忆、浏览器（支持并发异步恢复持久化身份凭证）、交互式澄清（ask_question_tool）等工具创建逻辑。支持低频工具的 Deferred 延迟加载。继承 ExternalAgentsMixin。 | ✅ |
...
| `agent_middlewares/tool_selection_middleware.py` | ✅ 核心 | 工具约束中间件 — tool_choice 状态机 + 收敛保护（112行） |
| `tools/_tool_layer_bootstrap.py` | ✅ 核心 | Server 专属工具向 harness _TOOL_LAYERS 的动态注册入口（4 个 SDK 依赖工具） |
| `tools/x_search_provider.py` | ✅ 辅助 | X/Twitter 搜索工具（依赖 xAI SDK） |
