...
[OUTPUT]
append_conversation_search_tool: Attach the read-only conversation_search tool to a main agent tool list.

[POS]
GeneralAgent 会话搜索装配辅助模块。保持 ToolSetupMixin/Factory 轻量，避免在大文件中继续堆叠工具装配逻辑。
"""
...
def append_conversation_search_tool(
    ...
    *,
    ...
    """Attach conversation_search to a main agent run."""
    ...
    logger.debug("Loaded conversation_search tool (chat_id=%s, agent_id=%s)", current_chat_id or "", agent_id or "")
