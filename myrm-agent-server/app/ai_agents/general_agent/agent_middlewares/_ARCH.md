...
|------|------|-------------|-------|
| `citation_rules_middleware.py` | Core | Injects citation formatting rules via transient HumanMessage (request.override, cache-safe) when the current user turn contains UNTRUSTED external sources; skips naked/search modes; dedupes once per user turn; includes task integrity reminder and language-adaptive locale resolution. | ✅ |
