...
| `video_agent_tool.py` | `create_video_generation_tool` → `video_tool`（generate 强制非空 prompt，支持 negative_prompt 与 seed，自动调用 `clamp_image_payload` 守护参考图片输入流，Dynamic Schema Diet 根据活跃 Provider 能力动态裁剪不支持参数；精准捕获 422 敏感词内容拦截快速熔断 `MODERATION_BLOCKED`；优先 async enqueue 到 TaskStore；status 支持按 task_id 查统一任务状态） |
