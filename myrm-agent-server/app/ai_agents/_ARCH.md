...
| `agents.py` | ✅ 核心 | AgentFactory 统一创建入口（Agent 类延迟导入），含 ImageGenerationParams / VideoGenerationParams（支持 api_key 传递）、GeneralAgentParams（含 `auto_restore_domains`、`enable_browser`、`client_surface`、`unattended_mode`、`max_iterations`、`quote`、`engine_params`、`search_depth`、`prompt_mode`）。`create_general_agent` 在 `enable_browser=True` 且 `prompt_mode!="search"` 时 peripheral 绑定 prebuilt `browser-automation` skill；event_log backend 仅在 `chat_id` 通过 `is_safe_session_id` 白名单时创建（写路径防路径逃逸） |
...
| `custom_agent_factory.py` | ✅ 核心 | CustomAgentFactory + EphemeralAgentFactory — 子 Agent 重绑 `memory_search_tool`；MCP 经 `prepare_mcp_configs_for_runtime` 与主 Agent parity；`AgentRuntimeSpec` 透传 `engine_params` / `openapi_services` / `mcp_surface_mode` | ✅ |
| `subagent_presets.py` | ✅ 核心 | 子 Agent 配置预置（adversarial-reviewer/analysis/browser/coding/deep-audit/search/structure-planner/layout-designer/format-verifier），启动时注册 |
...
| `prompts/` | ✅ 辅助 | 共享提示词（通用 Agent 多模式提示词、搜索建议、共享规则、deliverable_discipline SSOT） |
...
- 新增任何 `app/**/*.py` 内对 `GeneralAgentParams(` 或 `GeneralAgentParams.model_validate` 的调用前，必须核对 Web / Channel / Cron / Kanban / Eval / Voice 与 `ResolvedAgentProfile` 字段一致性（含 `auto_restore_domains`、`enable_browser` 等）。
- **Profile 输出后缀**：Web / Channel / Cron / Kanban / Eval / Voice（agent_bridge + realtime + gemini_live）/ Subagent / Goal stream 入口在 profile 解析后调用 `apply_profile_output_suffixes()`。Goal stream 续跑另经 `resolve_agent_mount(WEB_CHAT)` 与 `resolve_enable_web_fetch(security_overrides)` 对齐 turn1 工具与安全门控。EphemeralAgentFactory（JIT 蓝图）无 DB engine_params，不在守卫范围。守卫：`test_profile_output_suffixes_guard.py`（10 路径）。
