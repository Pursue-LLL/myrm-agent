...
| `agents.py` | ✅ 核心 | AgentFactory 统一创建入口（Agent 类延迟导入），从 `agent_params.py` 重新导出配置参数。`create_general_agent` 在 `enable_browser=True` 且 `prompt_mode!="search"` 时 peripheral 绑定 prebuilt `browser-automation` skill；event_log backend 仅在 `chat_id` 通过 `is_safe_session_id` 白名单时创建（写路径防路径逃逸） |
| `agent_params.py` | ✅ 核心 | Agent 配置参数模型定义（BaseAgentParams、GeneralAgentParams、ImageGenerationParams、VideoGenerationParams、TTSParams 等） | ✅ |
