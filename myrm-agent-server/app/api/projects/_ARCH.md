...
| `test_fixtures.py` | 路由 | Local-only 项目锁 seed 端点：`POST /projects/test/seed-turn-lock`（创建 project + 绑定 chat + 确定性占用项目锁 `hold_ms`，供 Chrome E2E 验证 `waiting_for_turn` 等待提示真实链路，不依赖真实并发时序）。 | ✅ |
