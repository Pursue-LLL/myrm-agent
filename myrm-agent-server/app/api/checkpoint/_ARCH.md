...
损坏的 subagent checkpoint：`delete` 容错删除损坏文件（异常类型 `CheckpointCorruptedError` 由 harness `saver.load` 抛出，router 捕获映射），损坏文件不阻塞用户清理。

`GET /list` 暴露 `task_description` 字段：shutdown 保存的 checkpoint 同步状态提取无法恢复 LangGraph 消息，无法真正续跑，因此前端统一提供**「重新发起（Re-initiate）」**入口——用保存的任务描述填充聊天输入框，让用户重跑该任务；subagent 树与设置页两个入口行为一致。`CheckpointInfo` 不返回可恢复性字段（前端无消费者，UI 不区分可恢复性）。
