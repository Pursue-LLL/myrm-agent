...
| `matrix_router.py` | 路由 | Cross-profile matrix eval endpoints (`/eval/matrix/run`, `/eval/matrix/abort`, `/eval/matrix/status`, `/eval/matrix/stream`, `/eval/matrix/reports/latest`). Kept separate so `router.py` stays within the eval module line budget. | ✅ |
