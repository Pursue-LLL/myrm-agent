...
| `benchmarks_router.py` | 路由 | External benchmark endpoints (`/eval/benchmarks` list, `/eval/benchmarks/run`, `/eval/benchmarks/download` + legacy `/eval/wb-bench/sources`, `/eval/wb-bench/run`, `/eval/wb-bench/download`) as a self-contained sub-router under the `/eval` prefix. The `/eval/benchmarks/run` endpoint runs a web-search pre-flight when the benchmark declares `web_search` in benchmark mode (a missing/unreachable search provider would otherwise silently produce a near-zero score), then synchronously re-checks `is_running` after the awaited pre-flight to close the concurrent-start race window (two requests can both pass the opening guard; the awaited probes keep the window open long enough to matter). | ✅ |
