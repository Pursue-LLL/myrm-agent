...
async def get_catchup_briefs(user_id: str = Depends(get_deploy_identity)) -> Any:
    ...
    async with session_factory() as db:
        ...
        stmt = select(Chat).where(
            Chat.source == "web",
            (Chat.last_read_at.is_(None)) | (Chat.updated_at > Chat.last_read_at)
        ).order_by(Chat.updated_at.desc()).limit(20)
        ...
        for chat in chats:
            ...
            approval_stmt = select(ApprovalRecord).where(
                ApprovalRecord.chat_id == chat.id,
            ...
            # Extract brief
            brief = CatchupBriefExtractor.extract(
                messages=msg_dicts,
