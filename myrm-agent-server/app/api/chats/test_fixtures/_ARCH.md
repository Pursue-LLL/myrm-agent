...
`__init__.py` 为聚合门面：定义 inline 的 citation / skill chip / embed / Kanban closure / Kanban IN_REVIEW seed 端点，并 include 全部 21 个子模块的 router。上层通过 `from ..test_fixtures import router` 挂载。
...
| 文件 | 职责 |
|------|------|
| `__init__.py` | 聚合门面：citation / skill chip transcript / skill chip composer / embed / Kanban closure / Kanban IN_REVIEW seed + 挂载全部子模块 |
