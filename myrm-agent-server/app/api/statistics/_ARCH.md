...
| `growth_dashboard.py` | 模块 | Growth Dashboard API — aggregated view of agent growth metrics, cost/savings summary, per-skill usage efficiency trends, and 7-day compounding skill health with actionable governance recommendations. | ✅ |
| `learning_timeline.py` | 模块 | Unified Learning Timeline API — chronologically aggregates memory records and skill evolution events with cursor pagination and inline mutation (edit/delete/archive). | ✅ |
| `learning_loop.py` | 模块 | Five-Ring Closed Learning Loop Status API — aggregates reflection, distillation, advancement (improvement gate), consolidation, and profiling metrics for continuous self-evolution observability. | ✅ |
...
| `session_analytics.py` | 模块 | 会话级分析 API。提供单个会话的详细统计（token、工具、LLM 耗时构成、事件时间线、任务指标、Context Doctor 上下文构成四色细分与热点诊断）和执行追踪。`llm_breakdown` 由 `_build_llm_duration_breakdown` 从 event_log `token_usage` 事件按模型分桶聚合（call_count/total_duration_ms，过滤无 duration_ms 的 legacy/失败事件）；`streamTtft` 由 `usage_aggregation` 产出后透传（消除幽灵类型）。`/session/{id}/trace` 访问条件：Chat 记录存在 **或** event_log 文件存在（支持 kanban 任务无 Chat 记录但有 event_log 的回放）；trace 的 tool_calls 由 `_attach_security_labels` 按 `tool_call_id` 合并 `security_audit` 决策（decision/reason/tainted/ts 透出，无审计 no-op）。session_id 白名单 `[A-Za-z0-9:_-]` 校验（两个端点共用 `_validate_session_id`，内部调用共享 `is_safe_session_id`（app/core/utils/session_id.py），拒绝 `..`/反斜杠/空字节构造的路径逃逸）。 | ✅ |
| `session_model_sessions.py` | 模块 | Model-specific session usage listing API（`GET /usage/model-sessions`），按模型维度列出会话用量。 | ✅ |
| `session_trace.py` | 路由 | 会话执行轨迹与甘特可观测性 API：从 event_log 恢复任务级执行流；挂载安全审计决策；丰富 LLM 思考 vs 工具执行性能甘特切片；透传重试与异常元数据；提供基于线程池的安全自然语言任务检索（`/traces/search`）。 | ✅ |
| `session_trace_enrichment.py` | 模块 | 会话轨迹丰富辅助：提供执行轨迹与甘特图指标丰富、性能统计、安全标签关联以及记忆事件投影辅助。 | ✅ |
| `prompt_cache_radar.py` | 路由 | 系统级 Prompt Cache 命中率雷达（`/traces/prompt-cache-radar`）：跨会话异步聚合 prompt cache 命中率、新鲜输入 vs 缓存读取与预估成本节省，消除事件循环阻塞。 | ✅ |
...
| `wiki_evidence_alerts.py` | 模块 | Wiki 证据治理告警辅助：`maybe_emit_governance_alerts`（质量告警 + 冷却去重）、`reset_wiki_evidence_alert_state_for_test`（test-only 内存状态重置）。 | ✅ |
| `quota_runtime_router.py` | 路由 | 搜索配额水库与浏览器计算度量运行时 REST API：提供搜索源免费用量与耗尽状态统计、月度配额重置、配额上限动态配置，以及浏览器自动化会话、计算时长、页面请求流量统计。 | ✅ |
