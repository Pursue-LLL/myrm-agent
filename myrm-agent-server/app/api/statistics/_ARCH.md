...
| `session_trace.py` | 路由 | 会话执行轨迹与甘特可观测性 API：从 event_log 恢复任务级执行流；挂载安全审计决策；丰富 LLM 思考 vs 工具执行性能甘特切片；提供自然语言任务检索（`/traces/search`）。 | ✅ |
| `prompt_cache_radar.py` | 路由 | 系统级 Prompt Cache 命中率雷达（`/traces/prompt-cache-radar`）：跨会话聚合 prompt cache 命中率、新鲜输入 vs 缓存读取与预估成本节省。 | ✅ |
