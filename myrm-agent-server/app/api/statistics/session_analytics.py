...
- app.api.statistics.session_trace (POS: 会话执行轨迹 API)
...
[OUTPUT]
- router: Session analytics APIRouter (get_session_analytics)

[POS]
会话级分析 API。提供单个会话的详细统计（token、工具、事件时间线、任务指标）。
执行轨迹端点见 session_trace 模块。
"""
