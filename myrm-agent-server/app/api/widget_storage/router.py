...
- DELETE /{namespace}/all: Clear all keys in a namespace.
...
async def get_value(namespace: str, key: str, session: AsyncSession = Depends(get_db)):
...
async def batch_write(namespace: str, body: BatchWriteRequest, session: AsyncSession = Depends(get_db)):
...
async def clear_namespace(namespace: str, session: AsyncSession = Depends(get_db)):
...
async def delete_key(namespace: str, key: str, session: AsyncSession = Depends(get_db)):
