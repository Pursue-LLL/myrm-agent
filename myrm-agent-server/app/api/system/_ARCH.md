...
| `router.py` | 路由 | Ingress 需求/URL、LAN 网络信息、存储磁盘信息与会话数据库智能优化（预检/FTS optimize/VACUUM/WAL truncate）、沙箱容器重建（SaaS）、支持排障包与全量资产 Takeout 导出、OpenTelemetry 遥测态势探针 (`GET /telemetry-posture`) | ✅ |
