...
| `history.py` | 模块 | evolution 历史记录接口层。对外提供已处理的 evolution 历史查询（GET /history，含进化 delta）与单条回滚（POST /{id}/rollback）。 | ✅ |
| `pending.py` | 模块 | evolution 审核接口层。对外提供 pending 列表、approve、reject、revise，以 ApprovalRecord 为唯一事实源；列表与详情均携带 `impacted_dependents`（经 core/skills/dependency_guard 查询）。 | ✅ |
