...
| `integration_memory.py` | 模块 | REST API layer for Integration Memory. Exposes sync, browse, status, and remove operations for the frontend to consume. """ | ✅ |
| `llms.py` | 模块 | LLM验证请求模型 | ✅ |
| `mcp.py` | 模块 | 获取 MCP Agent 实例（单例模式） | ✅ |
| `mcp_oauth.py` | 模块 | MCP OAuth 2.0 + PKCE authorization flow API. Frontend-driven flow user triggers auth → backend generates URL → user authorizes in browser → frontend receives ca | ✅ |
