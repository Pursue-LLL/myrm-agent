...
class ImportVideoResponse(BaseModel):
    ...
    relative_path: str = ""
    status: Literal[
        "success", "skipped_conflict", "superseded", "security_blocked", "error"
    ]
    error: str = ""
    message: str
...
async def import_urls(
        ...
        async with semaphore:
            try:
                ...
                try:
                    ...
                    logger.warning(
                        "_fetch_url_as_markdown failed for %s: %s", target_url, exc
                    )
                    markdown = ""
                ...
                if ingress_res.written:
                    return UrlImportItemResult(
                        ...
                        status=(
                            "security_redacted"
                            if ingress_res.security_redacted
                            else "success"
                        ),
                    )
                return UrlImportItemResult(
    ...
    message_parts = [
        f"Processed {len(cleaned_urls)} URL(s), {len(enqueued_paths)} enqueued"
    ]
...
async def import_video(
    ...
    try:
        ...
        logger.warning(
            "Failed to extract or ingest video transcript for %s: %s", target_url, exc
        )
        return ImportVideoResponse(
            success=False,
            url=target_url,
