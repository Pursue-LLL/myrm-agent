...
| `router.py` | 路由 | REST：… **POST /clip** (202 multipart · extension clip) · **GET /clip/{job_id}** · **GET/PUT /wikiignore** · **POST /maintain?mode=structural\|full**（默认 structural）经 `maintain_runner.run_wiki_maintain_job` SSOT；compile  busy 返回 409；**GET /stats** 含 `maintain_state` + `structural_issues`；**POST /query** 经 `knowledge_query_service` SSOT | ✅ |
