...
| `router.py` | 路由 | REST 主 router；**POST /compound** · **GET /{name}**（`get_concept` 返回 `source_chat`/`source_message` 溯源字段）· **POST /reindex-vectors**（published L2 + sidecar + optional asset vector rebuild + errors[]） | ✅ |
