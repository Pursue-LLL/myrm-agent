...
async def get_provider_usage(provider_id: str, db: AsyncSession = Depends(get_db)):
    ...
    for agent in agents:
        ...
        agent_summaries.append(
            AgentSummary(id=agent.id, name=agent.name, model=str(model_name) if model_name else None)
        )
            
    return ProviderUsageResponse(
        has_usage=len(agent_summaries) > 0,
...
async def batch_migrate_provider(
    req: BatchMigrateRequest, db: AsyncSession = Depends(get_db)
):
    """Batch migrate agents from one provider to another."""
    stmt = select(Agent).where(
        Agent.model_selection["providerId"].as_string() == req.from_provider_id
    )
    result = await db.execute(stmt)
    agents = result.scalars().all()
    ...
    if req.preview:
        return BatchMigratePreviewResponse(
