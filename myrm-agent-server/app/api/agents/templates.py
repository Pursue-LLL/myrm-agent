...
async def instantiate_template(template_id: str, request: Request, db: Any = Depends(get_db)) -> JSONResponse:
    ...
    try:
        ...
        for skill_id in prebuilt_skill_ids:
            ...
            if not skill:
                raise HTTPException(
                    status_code=400, detail=f"Template requires skill '{skill_id}' which does not exist in the system."
                )
        ...
        for skill_id in prebuilt_skill_ids:
            try:
                ...
                raise HTTPException(
                    status_code=500, detail=f"Failed to enable required skill '{skill_id}'. Agent creation aborted."
                ) from e
