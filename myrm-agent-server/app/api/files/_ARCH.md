...
| `artifact_share_public.py` | 模块 | 免认证公开入口（`/public/artifact-share/{token}`）三路由（GET+POST）：入口/尾斜杠索引/静态资源；多文件 bundle GET 307 尾斜杠跳转；密码保护走 HMAC unlock cookie，密码门表单 **POST body 提交（CWE-598：密码不进 URL）**，解锁成功 **303 See Other PRG** 重定向到干净 GET URL，旧 `?p=` 链接仍兼容；CSP/nosniff/DENY 安全头（仅 HTML）；所有文件统一 `X-Robots-Tag: noindex, nofollow` + `Cache-Control: no-store`（防搜索引擎收录 + 撤销后即时生效）；撤销检查在认证前（已撤销链接包括密码链接直接 404，不展示密码框）；限流 30/60 | ✅ |
