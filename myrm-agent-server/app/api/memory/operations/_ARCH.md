...
| `command_center.py` | 模块 | 记忆指挥中心 API 操作层。将单用户/单沙箱记忆运行快照暴露给设置页 UI，含 Claim/Evidence 知识图谱（支持 namespace 过滤），`GET /command-center` 支持可选 `project_id` 查询参数将快照聚焦到单个项目的 SharedContext 记忆空间；`GET /command-center/diagnostics/history` 返回持久化的 Memory Doctor 历史趋势（benchmark 标量指标反投影）。 | ✅ |
