...
| GET | `/kanban/pipelines/{skill_id}` | 获取 Pipeline 模板详情（含 discovery_questions、role_templates、task_graph_seed、task_graph_variants） |
| POST | `/kanban/boards/{board_id}/pipeline/instantiate` | 实例化 Pipeline 模板为任务 DAG（支持 variant_id 选择、确定性字符串替换 + 批量创建 tasks + edges） |
