...
| `agent_audit.py` | 模块 | CP-to-sandbox agent audit pull：GET `/api/admin/agent-audit/events` 按时间窗口拉取 harness JSONL event-log（agent 事件 SSOT）；`X-Telemetry-Token` 校验（`secrets.compare_digest` 恒定时间）+ hours/limit 钳制；截断前统计 `tool_call_total`（`tool_start`）、`security_event_total`（`security_audit`）与 `security_deny_total`（security_audit 事件内按 harness 权威 deny 语义 `BLOCK|DENY|REDACT|LEAK` 逐条计数，`myrm-agent-harness/core/security/audit.py` `record_decision` 口径）全量分类计数供 CP 聚合 | ✅ |
