...
| `agent_audit.py` | 模块 | CP-to-sandbox agent audit pull：GET `/api/admin/agent-audit/events` 按时间窗口拉取 harness JSONL event-log（agent 事件 SSOT）；`X-Telemetry-Token` 校验 + hours/limit 钳制 | ✅ |
