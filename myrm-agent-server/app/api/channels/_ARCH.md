...
| `feishu_register.py` | 模块 | 飞书/Lark QR 扫码注册路由。提供注册会话管理（TTL）、轮询与凭据落库；多应用场景下 provision 新实例，失败自动回滚持久化凭据；布尔凭据统一转小写字符串。`display_name` 空白归一化为 None（无标签视为刷新默认实例），成功分支以 `consumed` 原子标志保证并发 poll 只创建一次实例。 | ✅ |
