...
| `feishu_register.py` | 模块 | Business layer API. Bridges harness-layer FeishuAppRegistration with frontend QR code display and credential persistence via existing config API. """ | ✅ |
| `instances.py` | 模块 | 频道实例管理路由。提供多实例 CRUD、显示名更新、凭证存取和配置管理端点。 """ | ✅ |
| `login.py` | 模块 | Business layer API router. Integrates framework AsyncLoginProtocol with session management and credentials persistence. """ | ✅ |
| `router.py` | 路由 | Channel 管理核心路由。提供频道状态查询、启用/禁用切换、账号绑定 CRUD 和群组管理。 """ | ✅ |
| `routes_management.py` | 模块 | Routes management endpoints. Provides health reports and visibility into dynamically registered channel HTTP routes. """ | ✅ |
