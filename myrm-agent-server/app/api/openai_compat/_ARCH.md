...
| `completions.py` | 模块 | Agent-only `/v1/chat/completions` (streaming + non-streaming)；端点入口用 `is_safe_session_id` 白名单拒绝非法 `chat_id`（400，防路径穿越，流式/非流式统一提前拒绝）；流式/非流式均注入 `build_agent_runtime_context`（`execution_mode` + `disabled_skill_roots`） | ✅ |
