...
| `channel.py` | 模块 | Telegram Bot channel implementation. DM/group chat, media group debounce, webhook/polling dual mode, inline keyboard, Markdown→HTML, message splitting, /agent inline picker, Forum Topic management. | ✅ |
...
| `inbound.py` | 模块 | Telegram inbound message parsing, polling loop, and media group aggregation mixin. Supports message/edited_message/callback_query (qr/act/sel/ag)/sticker/location/venue/contact. Sets metadata for downstream enrichment. | ✅ |
