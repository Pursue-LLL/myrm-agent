...
- **飞书多应用 QR 注册（`app/api/channels/feishu_register.py`）**：`display_name` 空白归一化为 `None`（无标签 = 刷新默认实例，防止多应用区空标签静默覆盖默认凭据）；`poll` 成功分支以 `_RegistrationSession.consumed` 原子标志（检查与置位间无 `await`）保证并发轮询只创建一次实例
