...
- Edge TTS（`voice-tts` extra）通过 `uv sync --extra voice-tts` 或 `myrm setup --all-extras` 安装，不走频道 install 端点；Settings → Voice 会提示可用性，`GET /health/info` 返回 `edge_tts_available`；Web 朗读 API 失败时前端 toast 提示
