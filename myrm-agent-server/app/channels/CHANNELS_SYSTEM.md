...
- `POST /channels/manage/{name}/install-dependencies` 在 server venv 内 lazy-install（harness `runtime.lazy_deps`：`platform.discord` / `platform.feishu` / `platform.matrix`），成功后 **hot-register** 到 Gateway（无需重启进程）；响应 `registered: false` 表示 pip 成功但频道未上 bus（需配凭证或重启）
...
- `PATCH …/toggle` 启用前 **先** `ensure_channel_dependencies_ready`，成功后再写 DB 并 `enable_channel`
- 可选 extra：`channels-sdk`（discord-py + lark-oapi）、`matrix`、`matrix-e2ee`；`myrm setup` / 官方镜像 `--all-extras` 已包含
