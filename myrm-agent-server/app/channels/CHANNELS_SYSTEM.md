...
- 每个内置渠道在 `inbound_profile.CHANNEL_INBOUND_SPECS` 声明 `outbound` / `inbound` / `conditional` 传输模式（清单与 `registry.CHANNEL_META` 的一致性由 `tests/channels/test_inbound_profile.py` 清单覆盖测试保障）
