...
approval_denial_with_reason_processing =  正在处理拒绝，理由已转达："{ $reason }"
...
followup_receipt_done = ✅ 已完成，用时{ $elapsed }——如需调整请直接在本串回复。[AI 主动跟进]
followup_stall_nudge = 👋 来跟进一下——这个串安静很久了还有未办事项。{ $mention }直接回复我来接手。[AI 主动跟进]
...
durable_outbound_recovered = 已恢复投递，您可能会收到重复消息。
...
error_challenge_blocked = 请求被上游质询或安全验证拦截。
...
busy_ack_queued = ✅ 收到！Agent 正忙，您的消息已排队（第 { $position }/{ $max_pending } 条）。
busy_ack_queue_full = ⚠️ 队列已满，消息未保存，请稍后重发。
