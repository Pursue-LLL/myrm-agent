...
approval_denial_with_reason_processing =  正在處理拒絕：「{ $reason }」
...
retry_reverted = ↩ 重試前已還原 { $count } 個檔案。
retry_files_not_revertible = ↩ 重試前有 { $count } 個檔案變更無法自動還原（檔案過大或緩衝區已滿）。
...
daily_quota_exceeded = 今日對話額度已達上限（{ $quota } 次/天）。額度將於每日 00:00 UTC（北京時間 08:00）自動重設，請明日再試或聯絡管理員提升額度。
...
quota_header = [ 配額與使用狀態 ]
quota_user = • 使用者: { $user }
quota_role = • 角色: { $role }
quota_usage = • 今日用量: { $used } / { $limit }
quota_remaining = • 剩餘額度: { $remaining }
quota_reset_time = • 重設時間: 每日 00:00 UTC（北京時間 08:00）自動重設
quota_unlimited = 無限制
