...
approval_denial_with_reason_processing =  拒否を処理中：「{ $reason }」
...
retry_reverted = ↩ 再試行前に { $count } ファイルを元に戻しました。
retry_files_not_revertible = ↩ 再試行前に { $count } 件のファイル変更は自動的に元に戻せません（ファイルが大きすぎるかバッファが満杯です）。
...
daily_quota_exceeded = 1日の会話上限（{ $quota } 回/日）に達しました。毎日 00:00 UTC（日本時間 09:00 / 北京時間 08:00）に自動更新されます。明日再度お試しいただくか、管理者に問い合わせてください。
...
quota_header = [ クォータと使用状況 ]
quota_user = • ユーザー: { $user }
quota_role = • ロール: { $role }
quota_usage = • 本日の利用量: { $used } / { $limit }
quota_remaining = • 残り利用量: { $remaining }
quota_reset_time = • 更新時間: 毎日 00:00 UTC（日本時間 09:00 / 北京時間 08:00）に自動更新
quota_unlimited = 無制限
