...
| message_effects.py | Core | Message side-effect operations (typing/keepalive, reactions, placeholder, reply, busy ack). `edit_placeholder` uses `render()` pipeline for chunking/format downgrade. Operational replies use `MessagePriority.SYSTEM` for important-mode notify. `send_busy_ack` delivers queued/dropped acknowledgements with `send_with_retry`. | ✅ |
