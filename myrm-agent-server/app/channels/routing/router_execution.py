...
class RouterExecutionMixin:
    ...
    async def _prepare_execution_context(
        ...
        if isinstance(route_agent_id, str):
            ...
            if route_agent is not None and SqlTopicManager._agent_is_search_track(
                route_agent
            ):
                logger.warning(
        ...
        if result:
            ...
            if topic_ctx and topic_ctx.reply_mode == ReplyMode.DRAFT_REVIEW:
                try:
                    ...
                    logger.exception(
                        "Failed to create outbound draft, falling back to direct send"
                    )
                    if placeholder_id:
    ...
    async def _create_outbound_draft(
        ...
        logger.info(
            ...
            chat_id,
            topic_ctx.draft_timeout_minutes,
            topic_ctx.draft_timeout_action.value,
        )
