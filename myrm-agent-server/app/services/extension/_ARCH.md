...
| `bridge.py` | 核心 | WebSocket 生命周期、心跳、浏览器连接、域名授权（`*.example.com` 匹配根域+子域） | ✅ |
...
- **连接策略**：Extension 负责标签页选择与 debugger attach，Server 侧复用主浏览器 CDP endpoint 建立 `connect_over_cdp` 连接。
