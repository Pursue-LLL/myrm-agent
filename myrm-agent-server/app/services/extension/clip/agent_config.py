...
def _parse_config(raw: object) -> ExtensionClipAgentConfig:
    ...
    web_ui_origin = origin_raw.rstrip("/") if isinstance(origin_raw, str) and origin_raw.strip() else None
...
async def set_extension_clip_agent_config(
    *,
    ...
    normalized = ExtensionClipAgentConfig(
        ...
        web_ui_origin=(web_ui_origin.rstrip("/") if web_ui_origin and web_ui_origin.strip() else None),
    )
