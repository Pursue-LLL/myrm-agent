...
| `import_archive.py` | 核心 | 导出（JSON + Markdown ZIP）、归档、导入、回滚；竞品 dry-run 四车道；confirm 返回 `readiness` + `workspace_bind_candidates`；`POST /import/readiness-recheck` 用当前 provider 状态重算 readiness | ✅ |
