...
记忆提取失败持久化重试队列（幂等入队 + 退避重试 + 重启恢复 + 终态账本）。后台 worker（lifespan 管理）启动即扫描、每 60s 扫描与 `wake()` 即时唤醒；对指定 chat 最近一轮重新调度 `auto_extract_memories`，与 agent run 路径一致的隐私保护上下文重建；chat→agent extraction LLM 解析（与 auto-extract SSOT 对齐）。
...
| `resolve_chat_extraction_llm.py` | 核心 | 解析 chat 的 extraction LLM：经 `ChatService` 取 chat metadata → `create_agent_llms` + `apply_lite_context_downgrade`（返回 effective lite cfg），与 auto-extract SSOT 对齐；不 replay per-turn privacy_routing | ✅ |
