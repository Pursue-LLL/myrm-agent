...
async def batch_insert_canvas_elements(
    ...
    # Map logical node IDs to real tldraw shape IDs
    id_map: dict[str, str] = {}
    ...
    # Insert arrows connecting shapes using coordinate-based endpoints.
    # We position the arrow at the midpoint and set start/end relative to the
    # source/target node centers for reliable rendering without binding records.
    inserted_arrows: list[dict[str, Any]] = []
    shape_centers: dict[str, tuple[float, float]] = {}
    for shape_def in shapes:
        lid = shape_def.get("id", "")
        cx = float(shape_def.get("x", 0)) + 140  # default width/2
        cy = float(shape_def.get("y", 0)) + 60   # default height/2
