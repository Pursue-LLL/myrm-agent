...
| `clip_runner.py` | SSOT | `schedule_wiki_clip` async jobs — extension multipart → harness `publish_clip_ingress` → optional compile enqueue + incremental dedup scan | ✅ |
| `clip_form.py` | 辅助 | `clip_form_payload_bytes` + `MAX_CLIP_PAYLOAD_BYTES` — multipart 8MB cap（全部 form 字段 + asset 字节） | ✅ |
