...
| `vault_resolver.py` | SSOT | 路径解析 + legacy 迁移 + `seed_agent_vault_from_default`（Second Brain preset 默认 vault→新 agent） | ✅ |
...
| `source_sync/` | 核心 | Gmail/RSS 确定性 pull → `publish_raw` + compile enqueue；Integration mirror；Cron router；闭包：HTML2Markdown、Cron hygiene、sync state、Second Brain Gmail 默认 | ✅ |
