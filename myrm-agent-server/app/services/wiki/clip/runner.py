...
async def _purge_stale_jobs() -> None:
    async with _lock:
        ...
        stale_ids = [
            ...
            if record.state in terminal and record.finished_at is not None and now - record.finished_at > _JOB_TTL_SEC
        ]
        ...
        finished = sorted(
            ((record.finished_at or 0.0, job_id) for job_id, record in _jobs.items() if record.state in terminal),
