...
| `resolver.py` | SSOT | 路径解析 + 友好库名标签映射（`resolve_shared_wiki_vault_labels`） + legacy 迁移 + `seed_agent_vault_from_default` + `is_vault_ready` / `vault_has_wiki_content` 挂载库感知与 `must_exist` 物理防护 | ✅ |
