...
def _read_file_entry(file_path: Path, entry_name: str, total_bytes: int) -> tuple[PublishFile, int]:
    ...
    if file_path.suffix.lower() in TEXT_EXTENSIONS:
        try:
            ...
            return PublishFile(path=entry_name, content=encoded, encoding="base64"), next_total
        return PublishFile(path=entry_name, content=text, encoding="utf-8"), next_total
...
) -> tuple[dict[str, PublishFile], int]:
...
def validate_publish_payload(files: dict[str, PublishFile]) -> None:
