...
| ├─ `runner.py` | ✅ 核心 | KanbanTaskRunner 编排入口；worker 工具绑定 + goal-mode GoalProvider 注入（未终结 PAUSED/BUDGET_LIMITED/WAIT 目标智能断点续跑，终态及 PAUSED/WAIT 挂起态向 KanbanTask 存储原子同步 `BLOCKED` 状态、`block_kind`、`blocked_reason` 与 `acceptance_results` 结构化元数据）；team protocol 与 **`profile_output_suffixes`**（人格 + `response_locale_policy`）注入 `user_instructions` 尾；注入 `event_log_dir` 使 kanban 任务写 event_log（供 RunsHub/看板 drawer trace 回放）；per-task `model_override` 优先于 agent profile 默认模型解析（override 无效时回退默认模型并记录 WARNING）；**`enable_memory` 遵循用户全局 `enableMemory` 开关（`resolve_memory_enabled`，与 channel/voice/cron 一致），看板无人值守任务不写用户已关闭的记忆**；`_augment_context` 把 `workspace_root`（工作目录）与 metadata `context_annotations`（业务注入的执行指令，如批量目录产物要求）追加进 worker 上下文；**`_workspace_lock` 对共享工作目录做 per-workspace 写串行化**（realpath 规范化 key + 同 workspace 任务互斥，worktree/None workspace 天然不阻塞，等待时按用户 locale 写友好 `progress_note`，锁表用后即清） | ✅ |
...
| `criteria_parser.py` | ✅ 核心 | Markdown `- [ ]` checklist → `completion_criteria` semantic 数组；`parse_markdown_criteria` 提取（section 感知：仅识别 `Acceptance criteria`/`验收条件` 标题后清单，无标题 fallback 全局扫描）+ `attach_completion_criteria` 安全附加（不覆盖用户已有 criteria，纯函数不原地修改；specify/decompose apply 时使用，schema 对齐 goal_handler `_parse_im_goal_text`） | ❌ |
