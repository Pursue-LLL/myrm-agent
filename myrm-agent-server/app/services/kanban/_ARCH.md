...
| `task_runner.py` | ✅ 核心 | KanbanTaskRunner 编排入口；执行前确保 `kanban` 在 enabled_builtin_tools（`task_runner.py:243-244`）；`kanban_tool_mode=worker` + `kanban_current_task_id` → worker 5 工具 bind | ✅ |
