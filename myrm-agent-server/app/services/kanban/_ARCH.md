...
| ├─ `worktree.py` | ✅ 核心 | Git worktree 隔离（路径解析、创建、清理、合并编排；merge 前置 git 步骤在 `_worktree_merge.py`；per-base_dir merge 锁与 auto-commit/merge 身份兜底复用共享 git 层 `app.core.utils.git_worktree`） | ❌ |
