...
[INPUT]
- app.core.utils.git_worktree (POS: 共享 git 环境 `_GIT_ENV`)

[OUTPUT]
- _branch_has_commits, _ensure_target_branch_checked_out, _is_valid_git_branch

[POS]
合并前置 git 步骤：判断是否有可合并提交、确保 merge 落在目标分支、校验分支名。
由 worktree.lifecycle.merge_task_worktree 调用。自动提交未提交改动的共享实现
位于 core.utils.git_worktree（kanban 与 sandbox 共用一份，避免语义漂移）。
"""
