...
| `mcp_config_converter.py` | 核心 | 竞品 MCP 配置 → MCPMigrationItem → config dict / preview；无状态转换器；并发策略映射（`supports_parallel_*` ↔ `hostSerial`），可选保活间隔映射（`keepalive*` → `keepaliveInterval`，低于 5 秒时标记 `keepaliveIntervalIgnored` 供前端解释） | ✅ |
