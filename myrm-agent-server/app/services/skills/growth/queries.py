...
def _approval_case_detail(record: ApprovalRecord) -> SkillGrowthCaseDetailRead:
    ...
    verification_proof: dict[str, object] | None = (
        dict(raw_proof) if isinstance(raw_proof, dict) else None
    )
    ...
    is_continual = (
        record.action_type == "continual_overlay"
        or payload.get("source") == "continual"
    )
    source = (
        SkillGrowthCaseSource.CONTINUAL if is_continual else SkillGrowthCaseSource.DRAFT
    )
