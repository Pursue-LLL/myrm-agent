...
| `fingerprint.py` | 核心 | `compute_execution_fingerprint`（模型/技能/MCP/harness epoch/`engine_params` 含 MoA preset 激活态/安全配置/记忆配置/执行网络/通知/看板/子代理/委托——覆盖所有 build 期固化的用户可配置输入；排除每 run 状态与全局静态配置） | ✅ |
