...
class DatabaseSecretBackend:
    ...
    async def list_secret_keys(self, agent_id: str) -> list[str]:
        """List secret keys for an agent without decrypting values.

        Lightweight variant of ``get_all_secrets`` used by config checks that
        only need key existence (e.g. readiness preflight) — never touches
