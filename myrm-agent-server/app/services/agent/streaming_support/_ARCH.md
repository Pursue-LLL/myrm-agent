...
| `stream_collector.py` | 核心 | 流内容收集器；memory citation 持久化；`kanban_add_task`/`cron_manage` 成功结果写入 extra_data；`ui_update`/`data_update` 深合并；跨轮次 `data_update` 排队供 finalize 写回宿主消息 | ✅ |
