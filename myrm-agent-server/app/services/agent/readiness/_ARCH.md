...
- **MCP check**: Static config match — compares agent's mcp_ids against configured MCP servers; the configured set covers both the user's own config (`mcpServers`) and org-managed servers pushed by the Control Plane (`orgMcpServers`), matching the runtime merge in `converter.py`. Bound servers that declare `requiredSecrets` or `{{secret:KEY}}` header references are cross-checked against the agent vault (via `DatabaseSecretBackend.list_secret_keys`, key names only — no decryption), and missing keys surface as a WARNING with a deep-link to the agent Secrets tab (`/settings/agents?agentId={id}#secrets`)
...
- `app.core.channel_bridge.config_parsers` (extract_mcp_configs / extract_org_mcp_configs)
