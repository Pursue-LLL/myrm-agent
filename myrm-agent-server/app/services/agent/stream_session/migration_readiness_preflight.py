...
def build_migration_readiness_gap_sse_event(
    *,
    message_id: str,
    migration_readiness_anchor: MigrationReadinessAnchorRequest | dict[str, object] | None,
    chat_id: str | None,
    ...
    """Backward-compatible sync helper for tests that supply anchor status only.

    Production stream path uses resolve_and_build_migration_readiness_gap_sse_event.
    """
    ...
    raw_status = (
        migration_readiness_anchor.readiness_status
        if isinstance(migration_readiness_anchor, MigrationReadinessAnchorRequest)
        else migration_readiness_anchor.get("readinessStatus", migration_readiness_anchor.get("readiness_status"))
    )
    ...
    if raw_status == "critical":
        ...
        readiness = MemoryImportReadiness(
            status="critical",
            issues=[MemoryImportReadinessIssue(code="providers_not_configured", severity="critical")],
        )

    return build_migration_readiness_gap_sse_event_from_readiness(
        message_id=message_id,
        import_batch_id=import_batch_id,
        readiness=readiness,
        chat_id=chat_id,
