...
| `moa_overlay_setup.py` | 辅助 | Agent-loop MoA overlay：解析 `engineParams.moa_overlay` reference 模型并构建 harness middleware；无可用 ref 时返回 None + `resolve_moa_overlay_skip_reason` 供 stream 发 `moa_overlay_skipped` SSE；skip reason 含 `budget_pressure`、`insufficient_refs`（runtime 由 harness middleware emit） | ✅ |
