...
| `stream_chunks.py` | 核心 | SSE 预检编排（凭据、Vision fallback、**config gap / migration readiness gap / entitlement gap 三轨**：web_search 配置 gap 与 migration critical anchor gap 均在 `resume_value is None` 时独立于 user text；entitlement gap 仍依赖 preflight text）+ `generate_cancellable_stream` 生成器主体;`BaseException` 兜底捕获 → `yield_stream_exception_chunks`,`finally` 调 `finalize_agent_stream_session` | ✅ |
...
| `migration_readiness_anchor.py` | 辅助 | 迁移 readiness 双锚记录助手：消费 `migration_readiness_anchor`（import_batch_id），按 stream finalize 信号归类首轮结果（success/failed/no_output），优先写入 preflight live readiness 状态并回写 memory import 账本元数据 | ✅ |
| `migration_readiness_preflight.py` | 辅助 | 迁移 readiness 软门禁：anchor 携带 batch_id 时 live-resolve readiness，在 stream 早期对 warning/critical 发射 issue-aware `capability_gap`；不阻断执行、不改 Turn1 工具绑定 | ✅ |
