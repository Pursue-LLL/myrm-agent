...
| `entitlement_gap_preflight.py` | 辅助 | 用户消息 entitlement gap 预检 → 早期 capability_gap SSE（不改 Turn1 工具绑定）；render_ui 已开但渠道不可挂载时发射 `reason=surface_unavailable` + `display_message`（Web toast / IM ProgressUpdate） | ✅ |
