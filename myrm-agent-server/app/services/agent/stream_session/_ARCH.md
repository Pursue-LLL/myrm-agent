...
| `stream_loop.py` | 核心 | Agent 主流 SSE 循环;路由 Fast Lane / Wiki Knowledge Lane / Dynamic Workflow (含 auto-escalate 与 Admission Fast-Path 绕行) / GeneralAgent;首 token 前发射 `memory_brief` 预览;`message_end` 注入 `stream_ttft_ms`、`turn_prewarm_hit` / `turn_prewarm_ms` 与 memory brief 遥测;检测 `clarification_required` / `directory_request_required` 挂起表单超时 | ✅ |
