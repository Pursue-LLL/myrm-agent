    ...
    async def _load_from_db(agent_id: str) -> ResolvedAgentProfile | None:
        ...
        try:
            ...
            async with session_factory() as _db:
                ...
                coerced_tools = (
                    coerce_str_tuple(raw_builtin_tools) if raw_builtin_tools is not None else DEFAULT_ENABLED_BUILTIN_TOOLS
                )
                from app.services.agent.builtin_specs.builtin_tool_ids import (
                ...
                stripped_tools = strip_deploy_incompatible_builtin_tools(strip_legacy_builtin_tool_ids(coerced_tools))
                ...
                mcp_tool_selections = coerce_tool_selections(metadata.get("mcp_tool_selections"))
                sub_tuple = coerce_str_tuple(raw_subagent_ids) if raw_subagent_ids is not None else ()
                raw_auto_restore = metadata.get("auto_restore_domains")
                auto_domains_tuple = coerce_str_tuple(raw_auto_restore) if raw_auto_restore is not None else ()
                ...
                return ResolvedAgentProfile(
                    ...
                    default_security_preset=(
                        str(metadata.get("default_security_preset")) if metadata.get("default_security_preset") else None
                    ),
