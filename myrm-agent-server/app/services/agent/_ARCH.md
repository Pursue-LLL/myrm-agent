...
| `profile_resolver.py` | ✅ 核心 | 统一智能体配置解析服务 — `ResolvedAgentProfile`、`AgentProfileResolver` TTL 缓存、`resolve_builtin_tool_flags()`；**读路径优先 `AgentProfile.tools_allowed` 并经 `normalize_enabled_builtin_tools`（legacy fail-fast）**。Kanban bind 见 `kanban_tool_mode.py`。Web/Channel/Cron/Kanban/Eval/Voice 六入口单点解析。 |
| `builtin_tool_ids.py` | ✅ 核心 | `enabled_builtin_tools` canonical ID SSOT（16 IDs）+ `normalize_enabled_builtin_tools` / `persist_enabled_builtin_tools`（DB 写校验） |
