...
| `profile_resolver.py` | ✅ 核心 | 统一智能体配置解析 — `resolve_builtin_tool_flags()`（含 `external_cli`→`enable_external_cli`）+ `apply_agent_baseline_tool_flags()`（六入口非 fast 强制 file/bash）；TTL 缓存；`cron_post_run_verify` 从 DB 列经 repository 镜像到 profile metadata |
| `builtin_tool_ids.py` | ✅ 核心 | `enabled_builtin_tools` SSOT：17 canonical IDs（15 UI 可切换 + 2 Agent 基线无开关）；`external_cli` ON + UserConfig 有 CLI backend → Turn1 挂载 `delegate_to_agent_tool`；`cron` 开启 → Turn1 eager，关闭 → DISCOVERABLE；`structured_clarify` 开启 → 挂载 `ask_question_tool`（默认 ON）；`DEFAULT_ENABLED_BUILTIN_TOOLS=(web_search, memory, structured_clarify)`；`normalize` 静默剥离 baseline ID；`persist_enabled_builtin_tools` DB 写校验 |
