...
async def convert_to_general_agent_params(
    ...
    if (
        request.regenerate_instruction
        and request.regenerate_instruction != "__complaint_up__"
    ):
    ...
    default_model_cfg_raw = (
        providers_dict.get("defaultModelConfig") if providers_dict else None
    )
    base_slot = (
    ...
    if fallback_model_cfg is not None:
        if fallback_model_cfgs:
            fallback_model_cfgs = [
                ...
                *[
                    c
                    for c in fallback_model_cfgs
                    if c.model != fallback_model_cfg.model
                ],
            ]
        else:
    ...
    if fallback_lite_model_cfg is not None:
        if fallback_lite_model_cfgs:
            fallback_lite_model_cfgs = [
                ...
                *[
                    c
                    for c in fallback_lite_model_cfgs
                    if c.model != fallback_lite_model_cfg.model
                ],
            ]
        else:
    ...
    if fallback_lite_model_cfgs:
        ...
        for fl_cfg in fallback_lite_model_cfgs:
            ...
            enriched = enrich_model_capabilities(
                fl_cfg, providers_dict, selection_supports_vision=fl_vision
            )
            enriched_fl_cfgs.append(
                enrich_model_context_window(enriched, providers_dict)
            )
    ...
    if fallback_model_cfgs:
        ...
        for fb_cfg in fallback_model_cfgs:
            ...
            enriched = enrich_model_capabilities(
                fb_cfg, providers_dict, selection_supports_vision=fb_vision
            )
            enriched_fb_cfgs.append(
                enrich_model_context_window(enriched, providers_dict)
            )
        ...
        if memory_shared_context_ids:
            ...
            async with get_session() as session:
                memory_shared_context_names = (
                    await SharedContextService(session).get_context_names(
    ...
    params = GeneralAgentParams(
        ...
        **tool_flags,
            ...
            if request.incognito_mode
                ...
                if (
                    ...
                    and (
                        ...
                        or not getattr(
                            resolved.memory_policy, "enable_auto_extraction", False
                        )
                    )
                )
        ...
        memory_shared_context_names=memory_shared_context_names,
        quote=request.quote,
