    ...
    if routing_tier is None and (request.light_model_selection or request.reasoning_model_selection):
        try:
            ...
            is_complaint_up = bool(
                ...
                and (not request.regenerate_instruction or request.regenerate_instruction == "__complaint_up__")
            )
    ...
    try:
        ...
        if memory_shared_context_ids:
            ...
            async with get_session() as session:
                memory_shared_context_names = await SharedContextService(session).get_context_names(memory_shared_context_ids)
    except Exception as e:
