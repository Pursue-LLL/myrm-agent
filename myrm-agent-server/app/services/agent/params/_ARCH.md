...
| `converter.py` | 核心 | HTTP 请求到 GeneralAgentParams 主编排：模型与密钥、JIT workspace、tool_gateway、记忆开关、builtin 映射、fast/deep search 覆盖；统一解析 `reasoning_display_mode`（request 优先，回退 personalSettings.reasoningDisplayMode，默认 collapsed）；`_apply_session_preset()` 将 chat 级 `security_preset`（hitl/accept_edits/explore）叠加到 security config。 | — |
