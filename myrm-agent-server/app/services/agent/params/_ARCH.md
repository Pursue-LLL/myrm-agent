...
| `provider_legacy_remap.json` | 辅助 | 遗留 providerId → canonical storage id（与前端同名 JSON **字节一致**，pytest 校验） | ✅ |
| `providers.py` | 辅助 | 规范化 providerId、行匹配解析密钥；**仅** WebUI providers，无 env 回退；加载 `provider_legacy_remap.json` | ✅ |
