...
| `conversation_fork_manager.py` | ✅ 核心 | 对话分支管理（checkpoint 克隆 + Fork 关系追踪 + 完整 Chat 元数据继承 + `compacted_before_id` ID 映射 + fork 失败时清理孤儿 checkpoint） | ✅ |
...
- `app/core/channel_bridge/agent_executor.py`：频道 Agent 执行器（持久化 + 历史加载）
