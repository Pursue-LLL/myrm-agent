...
| `conversation_fork_manager.py` | ✅ 核心 | 对话分支与独立验收管理（批量消息构造与插入 + commit 后 DB→checkpoint sync + Fork 关系追踪与多代 root/depth DAG 溯源 + 完整 Chat 元数据继承 + `compacted_before_id` ID 映射 + sandbox 隔离语义：父有活跃沙箱时子回退至原仓库根 + fork 失败时清理孤儿 checkpoint + `acceptance_verifier` 挑剔验收 Prompt 注入与独立审计分支） | ✅ |
