...
| `sandbox_worktree.py` | ✅ 辅助 | Git worktree 生命周期管理：create/cleanup/merge/status，供 converter.py 和 sandbox API 共用；cleanup 默认 safe（dirty worktree 保留防丢数据），用户显式丢弃（`disable_sandbox`/chat 永久删除/清空回收站）或 merge 成功后 `force=True`；merge 失败自动 `git merge --abort` 恢复 repo（防 MERGE_HEAD 阻塞后续 merge）并收集冲突文件列表返回给调用方；git 命令基础设施复用 `_git_shared.py` | ✅ |
| `_git_shared.py` | ✅ 辅助 | 共享 git 命令基础设施（kanban/sandbox 共用一份，避免语义漂移）：`_GIT_ENV`（C-locale env）、per-base_dir merge 锁 `_get_merge_lock`、`_git_identity`（repo 无 user 配置时仅注入缺失的 `user.name/user.email`）、`_auto_commit_dirty_worktree`、`_collect_conflict_files`、`_abort_merge`、`_worktree_is_dirty`（fail-closed） | ✅ |
