...
| 文件 | 地位 | 职责 |
|------|------|------|
| `oauth_store.py` | 核心 | oauthCredentials 加密读写；`is_oauth_issuer_connected`；`google_workspace_write_enabled` / `google_drive_read_enabled`；`extract_copilot_base_url`（Copilot token base URL 解析共享函数） | ✅ |
