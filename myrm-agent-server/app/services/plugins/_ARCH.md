...
- **卸载生命周期**：`uninstall_plugin` 按 `plugin_name` 移除全部插件 MCP 条目（保留用户自建 server）、从所有 Agent 的 `mcp_ids` 解绑对应 server 名（`UnitOfWork` 原子更新）、删除插件文件与数据目录；任一环节失败均被隔离（文件删除失败仅告警，不阻断配置清理），保证插件可彻底卸载、无孤儿残留。
