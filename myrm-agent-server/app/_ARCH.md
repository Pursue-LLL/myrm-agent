...
core/         领域原语层（无 HTTP 的领域能力、适配器、持久化桥接）
  ...
  ↘ remote_access/  远程访问安全域（Pairing/E2EE/tunnel；HTTP 在 api/remote_access/）
```

`channels/`、`remote_access/` 与 `api/` 同级，不是 `services/` 子目录。渠道业务适配在 `core/channel_bridge/`。
...
| `remote_access/` | ✅ 核心 | 远程访问安全域（AdmissionPath、PairingToken、E2EE、CF tunnel）；HTTP 薄层在 `api/remote_access/` | [_ARCH.md](remote_access/_ARCH.md) |
...
api/ → services/ → ai_agents/ → myrm_agent_harness/
         ...
         ↓              ↘ remote_access/（安全域；被 middleware/ 与 api/remote_access/ 消费）
         ↘ schemas/（DTO 契约，无业务逻辑）
         ↘ database.repositories/（防腐层，禁止 core 直接操作 ORM）
```

- 上层可以依赖下层；`channels/`、`remote_access/` 不依赖 `api/`
- 下层不应依赖上层
