...
| 文件 | 地位 | 职责 | I/O/P |
|------|------|------|-------|
| `repo.py` | 核心 | 索引读写仓储：bootstrap、scope/fork/exclusion/health 查询与 FTS5 损坏自愈 | ✅ |
| `lookup_repo.py` | 辅助 | 只读可见性查找：semantic 命中 hydration、`fetch_message_window` expand、`fetch_prior_chat_document` mention | ✅ |
