...
| `fission_repo.py` | 核心 | FissionTaskRecord 数据访问层（高并发子代理任务拓扑持久化） | ✅ |
| `channel_message_repo.py` | 核心 | 多渠道入站明细仓储（Channel Message Data Plane），提供原子写入、上下文按需拉取、知识学习候选集查询与 30 天冷热生命周期滚动修剪 | ✅ |
