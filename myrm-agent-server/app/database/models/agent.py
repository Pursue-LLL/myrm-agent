...
class Agent(Base):
    ...
    mcp_tool_selections: Mapped[dict[str, list[str]] | None] = mapped_column(
        JSON, nullable=True
    )
    ...
    )  # deprecated: unread legacy column
    ...
    security_overrides: Mapped[dict[str, object] | None] = mapped_column(
        JSON, nullable=True
    )
    default_security_preset: Mapped[str | None] = mapped_column(
        String(20), nullable=True
    )

    prompt_mode: Mapped[str] = mapped_column(String(20), default="full", nullable=False)
    personality_style: Mapped[str] = mapped_column(
        String(32), default=DEFAULT_PERSONALITY_STYLE, nullable=False
    )
    suggestion_prompts: Mapped[list[str] | None] = mapped_column(JSON, nullable=True)

    memory_decay_profile: Mapped[str] = mapped_column(
        String(32), default="normal", nullable=False
    )
    memory_extraction_preset: Mapped[str] = mapped_column(
    ...
    workspace_policy: Mapped[str] = mapped_column(
        String(50), default="INHERIT_REQUESTER", nullable=False
    )
    ...
    command_bindings: Mapped[list[dict[str, object]] | None] = mapped_column(
        JSON, nullable=True
    )
    openapi_services: Mapped[list[dict[str, object]] | None] = mapped_column(
        JSON, nullable=True
    )
    session_policy: Mapped[dict[str, object] | None] = mapped_column(
        JSON, nullable=True
    )
    notify_targets: Mapped[list[dict[str, str]] | None] = mapped_column(
        JSON, nullable=True
    )
    tool_gateway_config: Mapped[dict[str, object] | None] = mapped_column(
        JSON, nullable=True
    )
    cron_post_run_verify: Mapped[bool] = mapped_column(
    ...
    secrets: Mapped[list["AgentSecret"]] = relationship(
        "AgentSecret", back_populates="agent", cascade="all, delete-orphan"
    )
