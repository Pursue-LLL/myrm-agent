...
| `matrix.py` | Matrix 评测：`run_matrix_eval_background` 跨配置矩阵评测——同一数据集在多个 AgentProfile 上顺序执行，输出 stable/regression 分类报告；配套 `get_matrix_eval_status`/`abort_matrix_eval`/`get_latest_matrix_report`，报告存 `.myrm/matrix_reports/`。矩阵各 profile 的会话 workspace 在 finally 统一 `LocalEvalExecutor.cleanup()` 用后即焚。 |
