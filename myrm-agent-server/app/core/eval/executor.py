...
class LocalEvalExecutor:
    ...
    def __init__(
        ...
        *,
        ...
        effective_blocked_terms = (
            list(blocked_terms) if blocked_terms is not None else []
        )
        if benchmark_mode:
        ...
        self._blocked_terms = (
            tuple(effective_blocked_terms) if effective_blocked_terms else None
        )
    ...
    async def execute(
        ...
        params = GeneralAgentParams(
            ...
            channel_name="eval",
            enable_web_search=(
                "web_search" in self.benchmark_tools if self.benchmark_mode else True
            )
        ...
        try:
            async for event in agent.process_stream(
                ...
                if event_type == "message" and isinstance(event.get("data"), str):
                    ...
                    # budget (the benchmark-declared ``max_iterations`` turn
                    # ceiling, doubled to graph nodes) is reached;
                    # engine_limit_reached only covers tool-call and replan
                    # caps, so without this branch iteration-truncated cases
                    # would silently lose their limit marker.
