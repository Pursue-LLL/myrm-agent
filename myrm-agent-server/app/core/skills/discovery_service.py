...
[INPUT]
- myrm_agent_harness.agent.skills.market.service::BaseSkillMarketService (POS: Framework-layer market service)
- myrm_agent_harness.agent.skills.market.sources.base::SkillSource (POS: Source protocol)
- myrm_agent_harness.agent.skills.market.sources.github::analyze_github_url (POS: GitHub URL analyzer)
- myrm_agent_harness.backends.skills.market_protocols::InstalledSkillInfo, SkillInstallResult (POS: Market protocols)
- app.core.skills.store.service::skills_service (POS: Business skill store)
- app.core.skills.custom_source_config::load_custom_sources (POS: Custom source config persistence)
- app.config.settings::settings (POS: App settings)

[OUTPUT]
- SkillMarketService: Business-layer market with SSE progress, auto-enable, custom sources
- discovery_service: Module-level singleton instance

[POS]
Business-layer skill market service. Wraps framework BaseSkillMarketService
with app-specific integrations (settings, SSE events, auto-enable/disable).
"""
...
class SkillMarketService:
    ...
    async def install(
        ...
        if (
            result.success
            and result.installed_path
            and "already installed" not in result.installed_path
        ):
...
discovery_service = SkillMarketService()
