...
技能市场、镜像注册表与自定义源子域。提供业务层市场服务（GitHub 源分析、SSE 进度、ClawHub 镜像懒加载）、ClawHub registry URL 持久化/apply（`CLAWHUB_URL` SSOT）、连通性探测与自定义技能源（`.well-known/skills`）持久化管理。属于 Server 业务层。
...
| `market_service.py` | 技能市场业务服务：`SkillMarketService`/`market_service` 单例（GitHub 源分析、SSE 进度、镜像懒加载、卸载后清理权限数据）。 |
