...
| `service.py` | Curator 业务服务：`run_curator_sweep`/`update_curator_config`/`get_curator_history`/`get_stats_collector`/`resolve_skill_path`/`get_skill_diagnostics`/`remediate_skill_finding`，`start/stop_curator_background_task` 后台任务编排。 |
