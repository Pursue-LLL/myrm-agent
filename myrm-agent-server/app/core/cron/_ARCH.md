...
| `blueprints.py` | 核心 | 自动化蓝图单一数据源：11 个内置蓝图（en/zh 内联 + ja/de/ko 经 `blueprint_i18n_supplement.py` 合并；五语系 title/desc/prompt_template、slots（含 `optional`）、schedule builder）。`GET /cron/blueprints` 返回目录；`POST /cron/blueprints/fill` 为 GUI 与 Agent 共用的创建前填充 SSOT。`fill_blueprint()` 校验必填 slot（`optional=True` 允许空 text）、job `name` 取 `title[locale]`；`get_blueprints_for_tool_description()` | — |
