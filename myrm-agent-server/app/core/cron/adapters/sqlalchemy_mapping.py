...
def job_to_domain(m: CronJobModel) -> CronJob:
    ...
    return CronJob(
        ...
        skill_ids=tuple(m.skill_ids) if getattr(m, "skill_ids", None) else (),
...
def job_to_model(job: CronJob) -> CronJobModel:
    ...
    return CronJobModel(
        ...
        skill_ids=list(job.skill_ids) if job.skill_ids else None,
...
def apply_job_to_model(m: CronJobModel, job: CronJob) -> None:
    ...
    m.skill_ids = list(job.skill_ids) if job.skill_ids else None
