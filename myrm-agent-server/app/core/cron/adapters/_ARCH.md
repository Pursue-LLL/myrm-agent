...
| `post_run_verification.py` | Cron 后置交付复核：当 Agent `cron_post_run_verify=true` 且 run 成功且检测到 mutating 工具使用时，spawn `adversarial-reviewer`（`verify_worker_output`，不重跑 worker）。effectful 工具 SSOT：`completion_guard.is_mutating_tool()`（registry fail-closed：未知工具视为 effectful） |
