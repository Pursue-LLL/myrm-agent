...
def build_vision_fallback_config_chain(
    ...
    *,
    ...
    primary = primary_override or _resolve_vision_fallback_primary_config(
        providers_dict
    )
    ...
    if providers_dict:
        ...
        if isinstance(default_model_cfg, dict) and isinstance(providers, list):
            ...
            if slot_fallback is not None:
                ...
                slot_fallback = enrich_model_context_window(
                    slot_fallback, providers_dict
                )
                _append_unique_model_config(configs, seen, slot_fallback)

    main_candidate = main_model_cfg or _resolve_base_primary_model_config(
        providers_dict
    )
...
def _resolve_video_fallback_primary_config(
    ...
    cfg = cfg.model_copy(
        update={"supports_video": _infer_supports_video(cfg.model, providers_dict)}
    )
    return enrich_model_context_window(cfg, providers_dict)
        ...
        if isinstance(default_model_cfg, dict) and isinstance(providers, list):
            ...
            if slot_fallback is not None:
                ...
                slot_fallback = slot_fallback.model_copy(
                    update={
                        "supports_video": _infer_supports_video(
                            slot_fallback.model, providers_dict
                        )
                    }
                )
                _append_unique_model_config(configs, seen, slot_fallback)
