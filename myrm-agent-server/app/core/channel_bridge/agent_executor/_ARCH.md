...
| `executor.py` | 核心 | Executes Agent tasks for inbound channel messages with session-aware auto-reset notification, dual-layer budget interception (global + per-channel), channel cost recording and sender attribution. Injects `cost_metadata` into OutboundMessage metadata (guarded by `enableCostEstimation`) for downstream rendering. Registers `on_artifacts_ready` callback to bridge harness file artifacts into IM outbound media delivery. For shareable artifacts (HTML/PDF/Document), auto-injects ActionButton deep links via HMAC signed share URLs and removes redundant file attachments. | ✅ |
