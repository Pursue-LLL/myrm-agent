...
| `agent_executor/executor.py` | ✅ 核心 | `ChannelAgentExecutor`：映射框架 AgentExecutor→业务 GeneralAgent；流式/持久化、reasoning/tool_steps 累积、审批超时、YOLO、Personality、`ConfigIncompleteError` i18n；搜索可达性门禁；优先 `resolved_identity` 记忆边界，透传 thread/epoch 元数据、`memory_shared_context_ids`/`Shared Context`；`build_channel_inbound_query` 构建渠道 Human 输入（支持多模态图片+文本+ReplyContext 引用注入），加投递横幅并保持群上下文装配；注册 `on_artifacts_ready` 回调桥接 Agent 产出的文件工件到 IM 渠道出站媒体。详情见 `agent_executor/_ARCH.md` | ✅ |
| `executor_helpers.py` | ✅ 辅助 | ChannelAgentExecutor 的辅助模块：历史持久化、标题生成、审批超时调度、流式累积（含 tool_image_output 图片缓存、artifacts 文件附件收集）、快速回复建议、进度标签转换、聊天历史格式化 | ✅ |
...
| `btw_notifier.py` | ✅ 核心 | BtwTaskNotifier：EventBus 订阅器，监听 `BACKGROUND_TASK_DONE` 事件，将 /btw 后台任务的完成/失败结果通过 `send_with_retry` 回推到原始发起渠道（channel/chat_id/thread_id），支持 i18n 多语言通知。与 NotificationDispatcher 并行运行，互不干扰 | ✅ |
