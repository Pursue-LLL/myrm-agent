...
| `executor_helpers.py` | ✅ 辅助 | ChannelAgentExecutor 的辅助模块：历史持久化、标题生成、审批超时调度、流式累积（含 tool_image_output 图片缓存）、快速回复建议、进度标签转换、聊天历史格式化。Artifact 文件附件收集与深链见 `agent_executor/artifact_deep_links.py` | ✅ |
