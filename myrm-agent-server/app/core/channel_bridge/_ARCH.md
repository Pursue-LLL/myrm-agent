...
| `btw_notifier.py` | ✅ 核心 | BtwTaskNotifier：ServerEventBus 订阅器，监听 `BACKGROUND_TASK_DONE` 事件，将 /btw 或带 `source_chat_id` 的 Kanban 任务完成/失败结果通过 `send_with_retry` 回推到原始发起渠道（channel/chat_id/thread_id 或从 Chat 解析 channel_session_key），支持 i18n 多语言通知。与 NotificationDispatcher 并行运行，互不干扰 | ✅ |
