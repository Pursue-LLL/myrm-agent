...
**Agent 绑定**：Channel/IM 仅 General Agent。`SqlTopicManager.bind_topic` 拒绝 `prompt_mode=search`（SSOT）；`resolve_topic` / `get_all_topics` 读时自动清除 legacy Search 绑定。Settings API 与 IM `/bind` 共用写入门禁。Search/Fast 为 Web `action_mode=fast`。
...
| `config_loader.py` | ✅ 核心 | 用户配置加载层：从 UserConfig 表一次性加载模型/搜索/检索/MCP/语音等配置，30s TTL 缓存。加载时自动注入 Provider OAuth token 到 providers_dict（`_inject_provider_oauth_tokens`）。`_resolve_override` 支持自定义 provider（通过 `providerType` 兼容匹配 + `enabledModels` 校验），`_to_litellm_model` 根据 `providerType` 生成正确的 LiteLLM 模型名。`register_custom_model_pricing` 将用户自定义模型定价注册到 litellm.model_cost | ✅ |
...
| `config_parsers.py` | ✅ 核心 | 配置解析器：从前端 dict 结构解析为类型化配置对象（SearchServiceConfig、SessionPolicy、TTSMode 等）。`_build_enriched_model_config` 统一 wire enrich + capabilities + context window，供主模型/vision/video/lite/slot fallback 全路径使用。`extract_search_provider_chain` / `extract_active_search_config` 按 priority 构建 harness `provider_chain`；`extract_web_tts_config` 供 Web `/tts` 朗读；`extract_voice_config` 供频道出站。`merge_org_mcp_configs` 将 Control Plane 推送的 org MCP（`orgMcpServers`，只读、scope=org 标记）追加到用户 MCP 配置列表，是 Web/Channel/Voice/Eval/Subagent/AI Build/记忆集成的统一合并入口（SSOT，替代各入口各自的 org 合并逻辑）。`build_vision_fallback_config_chain` / `resolve_vision_fallback_chain_for_agent` / `build_vision_fallback_engine_from_providers` 从 `defaultModelConfig.visionFallbackModel` ModelSlot 解析有序视觉辅助链（ExecutionSurface / wiki / sticker / health SSOT）。`session_policy_from_agent_dict` 支持 per-agent 会话策略覆盖。`verify_search_service_available`（chain head live probe，30s TTL 缓存）与 `invalidate_search_health_cache`（配置变更时清除缓存） | ✅ |
| `config_cache.py` | ✅ 辅助 | 用户配置 TTL 缓存：30s 过期的内存缓存；`invalidate_user_configs_cache()` 同时失效 `ingress` 公网 URL 缓存 | ✅ |
| `agent_executor/` | ChannelAgentExecutor 及子模块（executor、execute_preamble/ 子包、deliverable/ 子包、stream_events、helpers、session） | [agent_executor/_ARCH.md](agent_executor/_ARCH.md) |
...
| `pairing_store.py` | ✅ 核心 | SqlPairingStore：PairingStore 协议的 SQLAlchemy 实现，管理 sender → user 绑定与角色授权（ADMIN 返回 sandbox、MEMBER 返回 paired_member 前缀以激活 UntrustedIngressFence 防提权）。基于 SQLite 原生 on_conflict_do_update 实现原子 UPSERT 与锁争用退避重试，支持每日调用配额（daily_quota）持久化。新建 PENDING 配对时通过 ServerEventBus 发布 `PAIRING_PENDING` 事件 | ✅ |
