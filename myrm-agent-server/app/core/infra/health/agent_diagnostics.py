...
class SkillHoardingHealthDiagnostic(DiagnosticProtocol):
    """Diagnose skill hoarding and wrong-but-frequent low-quality skills in the skill ecosystem.

    Audits two critical failure modes in long-running agent lifecycles:
    1. Skill Hoarding (Clutter): Active skill count approaching or exceeding max_skills limit.
    2. Wrong-But-Frequent (Corruption): Skills frequently invoked (call_count >= min_call_count)
       with high failure rates (success_rate < min_success_rate). Distinguishes between normal
       unhealthy skills (auto-curatable) and protected/exempt skills (pinned/installed/system) that
       require manual user triage.
    """
