...
| 文件 | 地位 | 职责 | I/O/P |
|------|------|------|-------|
| `settings.py` | ✅ 核心 | 应用配置定义（AppSettings）。仅 [P]/[O]/[S] 进程 env；`ControlPlaneSettings` + `ContextCompactionTelemetrySettings` 供 sandbox 遥测；业务 LLM/Embedding 走 WebUI（`platform_config.py`），不在 settings 中 | ✅ |
