...
| 文件 | 地位 | 职责 | I/O/P |
|------|------|------|-------|
| `system.py` | 核心 | Channel Gateway 启动/关闭编排、OfflineDurableTask 断点续跑（经 `build_agent_runtime_context` 注入 POOLED runtime context；成功/失败均创建 SystemNotification）、**InterruptedTurnMarker 普通回合崩溃自动续跑**（freshness 15min / max 2 attempts crash-loop breaker，同样注入 POOLED runtime context；流内收集 `message_end.token_economics` 作为消息 `extra_data` 持久化，与主路径共享消息级成本记账口径；成功/失败均创建 SystemNotification）、孤儿 Goal 自动暂停 | ✅ |
