...
| `memory_guardian_ops.py` | 辅助 | 记忆守护者维护子任务：过期归档记忆自动清理（TTL 7天）+ 超时冲突自动解决（低风险 72h 后 keep_old；high_risk 冲突无 auto_resolve_at 永不自动解决，需用户显式裁决；每次 auto-resolve 写入专属审计事件）。纯数据操作，无调度状态依赖 | ✅ |
