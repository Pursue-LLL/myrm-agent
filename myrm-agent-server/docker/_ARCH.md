...
Builder：`uv sync --frozen --all-extras`（含 `compiled-core`；`pyproject.toml` 设 `prerelease=allow` 与 `index-url=pypi.org`）。PyPI 未发布时 CI 失败（无 silent fallback）。Runtime：`python -m myrm_agent_harness.runtime.install_guard.verify --matplotlib-cjk`（公开 Dockerfile 不依赖 console script  shim）。
