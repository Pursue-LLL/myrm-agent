...
CI：`myrm-agent/.github/workflows/server-architecture.yml`（PR 全 paths；`main` push 仅分发 paths 触发 `docker-image` job，Linux `docker build` 验证 `uv sync --frozen` 与 `verify-harness-distribution`）；`install-smoke.yml`（`install.sh` 非 frozen 路径；Windows 仅 `workflow_dispatch`）。
