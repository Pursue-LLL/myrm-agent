...
async def _test_hf_chat(page: Page):
    ...
    await box.fill("Please use the huggingface_inference_tool to generate a picture of a red cat. Model: stabilityai/stable-diffusion-3.5-large, task: text-to-image.")
    ...
    assert "error" in last_msg or "data:image" in last_msg or "![generated image]" in last_msg or "red cat" in last_msg, "Did not get expected response."
