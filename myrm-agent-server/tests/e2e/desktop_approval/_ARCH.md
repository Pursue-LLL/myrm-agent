...
| `gate_probe.py` | Core | Desktop tool activity, idle fail-fast, TextEdit foreground during api_only wait, nudge routing；API poll `to_thread + wait_for` wall-timeout guard，防止 shared_hot 长阻塞；新增 fallback budget（synthetic dref / pending seed）超限 fail-fast，支持 strict 模式（`MYRM_DESKTOP_E2E_STRICT_FALLBACK_MODE=1`）验真，且返回 `pendingSource` 区分真实/合成 pending | ✅ |
