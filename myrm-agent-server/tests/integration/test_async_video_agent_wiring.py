"""Integration: GeneralAgent wiring -> video_tool async enqueue (no LLM flake)."""
...
@pytest.fixture
def video_task_store(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> SQLiteTaskStore:
    db_path = tmp_path / "agent-video-wiring-tasks.db"
...
@pytest.mark.asyncio
async def test_general_agent_video_tool_enqueues_task_with_chat_and_agent_ids(
    video_task_store: SQLiteTaskStore,
) -> None:
    """Full tool_setup wiring: video_tool generate must enqueue with payload snapshot."""
    ...
    agent = GeneralAgent(
        ...
        chat_id="chat-video-wiring-int",
        agent_id="agent-video-wiring-int",
        video_generation_params=VideoGenerationParams(
            provider="openai",
            model="sora",
            api_key="sk-video-wiring-int",
        ),
    )
    agent._task_user_id = "user-video-wiring-int"

    tools: list[object] = []
    agent._setup_search_and_basic_tools(tools)

    video_tools = [tool for tool in tools if getattr(tool, "name", None) == "video_tool"]
    assert len(video_tools) == 1

    raw = await video_tools[0].ainvoke(
        {
            "action": "generate",
            "prompt": "a green triangle spinning in space",
        }
    )
    payload = json.loads(str(raw))
    assert payload.get("task_id")
    assert payload.get("status") == "pending"
    assert payload.get("task_type") == "video_generate"

    task = await video_task_store.get_task(str(payload["task_id"]))
    ...
    assert task.payload["chat_id"] == "chat-video-wiring-int"
    assert task.payload["agent_id"] == "agent-video-wiring-int"

    listed = await video_task_store.list_tasks(TaskFilters(task_type="video_generate", limit=5))
