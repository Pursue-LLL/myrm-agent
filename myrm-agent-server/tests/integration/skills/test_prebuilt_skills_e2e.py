...
async def test_skill_env_vars_endpoints_persist_and_resolve(
    ...
    await service.user_config.ensure_prebuilt_enabled_after_sync(["code-review", "systematic-debugging"])
    ...
    resolved = await resolve_skill_env_map(backend, {"code-review": {"GOOGLE_API_KEY": "test-key"}})
...
async def test_tdd_skill_v120_contract_guard(
    ...
    test-passes/test-errors discipline, GREEN cheat-permission/pristine check,
    REFACTOR no-behavior/rollback instruction, Repeat cycle, Red Flags red
    lines, deeper reference file) against regression.
    """
    ...
    md_path = get_skill_file_path(SkillType.PREBUILT, "test-driven-development", SKILL_MD_FILE)
...
async def test_full_pipeline_export_prebuilt_as_agent_plugin_and_validate(
    ...
    export_res = await pkg_service.package_skill("code-review", export_format="agent_plugin")
