...
from tests.integration.test_background_tasks_rest_api import (
    _build_rest_app,
    _spawn_background,
)
...
async def test_auto_yield_whitelist_command(tmp_path: Path) -> None:
    ...
    assert (
        result["metadata"].get("background") is True
        or result["metadata"].get("completed_in_yield_window") is True
    )
...
async def test_rest_preview_redacts_secrets(tmp_path: Path) -> None:
    ...
    pid, job_id = await _spawn_background(
        tmp_path, chat_id=chat_id, command=cmd, reason="redact integration"
    )
...
async def test_count_running_hook_matches_registry(tmp_path: Path) -> None:
    ...
    await _spawn_background(
        tmp_path, chat_id=chat_id, command=sleep_cmd, reason="count hook"
    )
    assert count_running_background_shell_jobs() >= 1
    assert (
        count_running_background_shell_jobs()
        == get_background_registry().count_running()
    )
