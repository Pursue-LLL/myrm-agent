...
async def main():
    print("Starting E2E test for Auto Capture Tool Failure Hook...")
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            # --- Test Tool Failure Hook ---
            print("Testing Tool Failure Hook...")
            ...
            if await chat_input.count() > 0:
                print("Sending message to trigger tool failure...")
                # We ask it to run a command that will definitely fail multiple times
                await chat_input.fill("Please run 'cat /this/file/does/not/exist/at/all.txt' twice using bash_code_execute_tool.")
                ...
                print("Waiting for agent to respond and fail...")
            ...
            if "failed" in body_text.lower() or "pending" in body_text.lower() or "待审批" in body_text:
                print("SUCCESS: Found pending memory for tool failure in Command Center!")
            else:
                print("WARNING: Could not immediately find pending memory text for tool failure.")
