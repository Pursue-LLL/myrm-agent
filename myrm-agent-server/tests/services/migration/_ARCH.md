...
| 文件 | 地位 | 职责 |
|------|------|------|
| `test_competitor_discovery.py` | 核心 | 四源 filesystem discover（confidence、多源共存、edge cases） |
| `test_competitor_payload_loader.py` | 核心 | load_competitor_payload / build_coverage_items |
| `test_competitor_payload_split.py` | 核心 | instruction vs memory 车道拆分 |
| `test_migration_competitor_coverage.py` | 核心 | import source 映射、auto 路由回归、supported_competitor_ids |
...
| `test_competitor_secrets_importer.py` | 辅助 | opt-in API key 导入 |
