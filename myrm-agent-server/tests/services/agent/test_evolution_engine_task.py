...
async def test_too_short_history_skips_llm_and_capture() -> None:
    ...
    with (
        ...
        patch("myrm_agent_harness.agent.skills.evolution.core.engine.SkillEvolutionEngine") as engine_cls,
    ):
...
async def test_proposal_none_skips_approval_flow() -> None:
    ...
    with (
        ...
        patch("myrm_agent_harness.agent.skills.evolution.db.store.SkillStore"),
        patch("app.services.agent.confidence_approval_flow.ConfidenceApprovalFlow") as flow_cls,
    ):
