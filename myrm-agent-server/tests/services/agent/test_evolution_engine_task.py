...
def _engine_class(proposal: object | None) -> MagicMock:
    ...
    engine_cls.return_value.capture_skill_from_trajectory = AsyncMock(
...
async def test_too_short_history_skips_llm_and_capture() -> None:
    ...
    with (
        patch(
            "app.platform_utils.get_session_factory", side_effect=_fake_session_factory
        ),
        patch(
...
async def test_conversation_text_skips_db_load() -> None:
    ...
    with (
        ...
        await _run_evolution_task(
            "chat-dw", MagicMock(), conversation_text="DW outcome text"
        )

    get_factory.assert_not_called()
    get_messages.assert_not_called()
    trajectory = (
        engine_cls_patch.return_value.capture_skill_from_trajectory.call_args.kwargs[
            "trajectory"
        ]
    )
...
async def test_proposal_none_skips_approval_flow() -> None:
    ...
    history = _history(
        ["user", "assistant", "user", "assistant"], ["q1", "a1", "q2", "a2"]
    )
    with (
        patch(
            "app.platform_utils.get_session_factory", side_effect=_fake_session_factory
