...
@pytest.mark.asyncio
async def test_collect_browser_snapshot_with_space_id() -> None:
    gateway = MagicMock()
    session = _FakeBrowserSession()
    gateway.get_active_browser_session.return_value = session
    payload = {
        "screenshot_base64": "abc_sub",
        "mime_type": "image/jpeg",
        "refs": {},
    ...
    with patch("app.services.agent.gateway.get_agent_gateway", return_value=gateway):
        with patch(
            ...
            with patch(
                ...
                result = await collect_browser_snapshot_payload(chat_id="chat-1", space_id="space-1")

    gateway.get_active_browser_session.assert_called_once_with(session_id="chat-1", space_id="space-1")
    capture_mock.assert_awaited_once_with(session, task_space_id="space-1")
    assert result == payload
