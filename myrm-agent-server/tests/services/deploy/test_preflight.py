...
def test_evaluate_deploy_preflight_ok_for_index_html() -> None:
    result = evaluate_deploy_preflight({"index.html": DeployFile(path="index.html", content="<h1>Hi</h1>")})
    assert result.deployable is True
    assert result.reason == "OK"


def test_evaluate_deploy_preflight_rejects_tsx_only() -> None:
    result = evaluate_deploy_preflight({"App.tsx": DeployFile(path="App.tsx", content="export default () => null")})
