...
async def test_duplicate_user_message_id_reuses_existing_row_when_content_matches(
    ...
    result = await db_session.execute(select(Message).where(Message.chat_id == "test-chat-dedup-001", Message.role == "user"))
...
async def test_duplicate_user_message_id_allocates_fresh_id_when_content_differs(
    ...
    result = await db_session.execute(select(Message).where(Message.chat_id == "test-chat-dedup-002", Message.role == "user"))
    user_messages = result.scalars().all()
