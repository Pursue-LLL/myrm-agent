"""Tests for POST /config/video-health."""
...
def test_video_health_not_configured(client: TestClient) -> None:
    mock_cfgs = MagicMock()
    ...
    with (
        ...
        patch(
            "app.core.channel_bridge.config_parsers.extract_video_fallback_model_config",
            return_value=None,
        ),
    ):
        response = client.post("/api/v1/config/video-health")
...
def test_video_health_success(client: TestClient) -> None:
    ...
    mock_cfgs = MagicMock()
    mock_cfgs.providers_dict = {"defaultModelConfig": {"videoFallbackModel": {}}}
    ...
    with (
        ...
        patch(
            "app.core.channel_bridge.config_parsers.build_video_fallback_probe_engine_from_providers",
...
def test_video_health_failure_is_unhealthy(client: TestClient) -> None:
    mock_cfg = SimpleNamespace(
        model="openai/gpt-4o-mini",
        api_key="test-key",
        base_url="https://api.example.com/v1",
