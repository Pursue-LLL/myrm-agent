...
def _write_fake_subset(root: Path, subset_id: str, task_ids: list[str]) -> Path:
...
def test_build_aborts_during_workspace_preparation(tmp_path: Path) -> None:
    ...
    prep_calls = {"n": 0}
    orig_prep = wb._prepare_workspace
    ...
    with (
        patch("app.core.eval.wb_bench.httpx.Client") as mock_client,
        patch.object(wb, "_prepare_workspace", side_effect=_counting_prep),
    ):
...
def test_build_cases_maps_tasks_and_seeds_workspaces(tmp_path: Path) -> None:
    source_root = _write_fake_subset(wb.SOURCES_DIR, "sec", ["t1", "t2"])
    subset = wb._SUBSET_BY_ID["sec"]

    cases: list[wb.MultiTurnEvalCase] = []
    seed_map: dict[str, str] = {}
    for task_dir in sorted(source_root.glob("tasks/*")):
        workspace = wb._prepare_workspace(task_dir, subset)
        case = wb._case_for_task(task_dir, subset, workspace_dir=workspace)
        cases.append(case)
    ...
    for case, message in zip(cases, seed_map.keys(), strict=True):
        ...
        # Security tasks grade natively; the adapter records the mode and ships
        # no assertions — grading runs inside the Harbor container's scorer.
...
def test_workspace_seed_is_isolated_and_idempotent(tmp_path: Path) -> None:
    ...
    ws1 = wb._prepare_workspace(task_dir, subset)
    ws2 = wb._prepare_workspace(task_dir, subset)
    assert ws1 == ws2
...
def test_workspace_archive_top_level_dir_unwrapped(tmp_path: Path) -> None:
    ...
    ws = wb._prepare_workspace(task_dir, wb._SUBSET_BY_ID["code"])
...
def test_scoring_mode_recorded_in_metadata(tmp_path: Path) -> None:
    """Each task is tagged with its grading mode; assertions stay empty."""
    ...
    case = wb._case_for_task(task_dir, subset, workspace_dir=None)
    assert case.turns[0].sandbox_assertions == []
    assert case.metadata["wb_bench_scoring"] == "composite"

    sec_case = wb._case_for_task(task_dir, wb._SUBSET_BY_ID["sec"], workspace_dir=None)
...
def test_no_workspace_archive_returns_none(tmp_path: Path) -> None:
    ...
    task_dir.mkdir()
    assert wb._prepare_workspace(task_dir, wb._SUBSET_BY_ID["code"]) is None
