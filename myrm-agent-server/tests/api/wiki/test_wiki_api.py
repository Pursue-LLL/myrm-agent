...
def test_wiki_stats_endpoint(client: TestClient) -> None:
    ...
    if response.status_code == 200:
        ...
        assert isinstance(data["structural_issues"]["provenance_gaps"], int)
...
def test_wiki_health_report_endpoint(client: TestClient) -> None:
    """Test GET /api/v1/wiki/health-report endpoint."""
    response = client.get("/api/v1/wiki/health-report")
    if response.status_code == 200:
        data = response.json()
        ...
        assert isinstance(data["duplicate_groups_pending"], int)
        assert isinstance(data["synthesis_pending"], int)
    else:
...
def test_wiki_concept_get_omits_missing_provenance(
    ...
    structure = WikiStructure(tmp_path / "wiki")
    structure.ensure_structure()
    concept_path = structure.get_concept_file_path("UntracedConcept")
    concept_path.write_text(
        """---
type: concept
---
...
""",
    ...
    test_client = TestClient(app)
    try:
        response = test_client.get("/api/v1/wiki/concepts/UntracedConcept")
    finally:
        app.dependency_overrides.clear()

    assert response.status_code == 200, response.text
    data = response.json()
    assert data["source_chat"] is None
...
def test_wiki_compound_returns_not_found_for_missing_message(
    client: TestClient,
) -> None:
    payload = {
