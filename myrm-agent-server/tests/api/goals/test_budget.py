...
async def test_update_goal_budget(client: TestClient):
    ...
    response = client.post(f"/api/v1/goals/{session_id}/budget", json={"additional_tokens": 5000})
    ...
    response = client.post(f"/api/v1/goals/{session_id}/budget", json={"additional_tokens": 2000})
    assert response.status_code == 200
    data = response.json()
    assert data["new_budget"]["max_tokens"] == initial_tokens + 7000

    # 3. Test invalid tokens
    response = client.post(f"/api/v1/goals/{session_id}/budget", json={"additional_tokens": -100})
