...
async def _upload_db_session() -> AsyncSession:
    engine = create_async_engine("sqlite+aiosqlite:///file:testdb_upload_e2e?mode=memory&cache=shared&uri=true")
...
def test_upload_large_image_downsampling(client):
    ...
    # Upload the image
    response = client.post("/api/v1/files/upload", files={"files": ("large_test.jpg", img_bytes, "image/jpeg")})
...
def test_upload_small_image_no_downsampling(client):
    ...
    # Upload the image
    response = client.post("/api/v1/files/upload", files={"files": ("small_test.png", img_bytes, "image/png")})
...
def test_upload_exceeds_max_body_size(client):
    ...
    response = client.post("/api/v1/files/upload", files={"files": ("huge_test.txt", large_payload, "text/plain")})
