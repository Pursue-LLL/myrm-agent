...
def test_replay_session_not_found(client: TestClient) -> None:
    with patch("app.services.chat.chat_service.ChatService.get_chat", new_callable=AsyncMock) as mock_get:
...
def test_replay_session_empty_messages(client: TestClient) -> None:
    ...
    with (
        patch("app.services.chat.chat_service.ChatService.get_chat", new_callable=AsyncMock) as mock_get,
        patch("app.services.chat.chat_service.ChatService.get_chat_messages", new_callable=AsyncMock) as mock_msgs,
    ):
