...
def _seed(client: TestClient, *, variant: str, agent_id: str | None = None) -> dict[str, object]:
    ...
    with patch("app.api.chats.test_fixtures.file_edit_batch.is_local_mode", return_value=True):
...
class TestFileEditBatchSeedIntegration:
    ...
    def test_workspace_only_seed_writes_file_for_existing_chat_id(self, client: TestClient) -> None:
        chat_id = f"c-{uuid.uuid4().hex[:12]}"
        with patch(
            "app.api.chats.test_fixtures.file_edit_batch.is_local_mode",
            return_value=True,
        ):
            resp = client.post(f"/api/v1/chats/test/seed-file-edit-batch-workspace?chat_id={chat_id}")
        assert resp.status_code == 200, resp.text
        body = resp.json()
