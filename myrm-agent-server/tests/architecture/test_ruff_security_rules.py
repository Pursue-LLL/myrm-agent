...
(pickle, eval, SSL verify=False, …) without drowning in noise.
"""
...
_REQUIRED_NOISE_SUPPRESSIONS = frozenset(
    {
        "S101",  # assert
        "S104",  # bind 0.0.0.0 (server apps bind all interfaces)
...
class TestRuffSecurityConfig:
    ...
    def test_noise_suppressions_complete(self) -> None:
        ...
        assert not missing, (
            ...
            "These are validated false-positives in a server application."
        )
    ...
    def test_no_overly_broad_suppression(self) -> None:
        ...
        assert not broad_suppressions, (
            f"Overly broad S-rule suppression detected: {broad_suppressions}. Suppress individual rules, not the entire category."
        )
    ...
    def test_server_has_bind_all_suppression(self) -> None:
        """Server apps legitimately bind 0.0.0.0, S104 must be suppressed."""
        lint = _load_ruff_lint()
        ignored = lint.get("ignore", [])
        assert "S104" in ignored, (
            "Server apps need S104 (bind-to-all-interfaces) suppressed. 0.0.0.0 binding is a deliberate server design choice."
        )

    def test_config_syntax_valid(self) -> None:
        ruff = _find_ruff()
        result = subprocess.run(
            [ruff, "check", "--select", "S", "--no-fix", "-q", str(_REPO_ROOT / "app")],
            capture_output=True,
            text=True,
            cwd=str(_REPO_ROOT),
            timeout=60,
        )
        assert result.returncode in (0, 1), f"ruff failed with unexpected returncode {result.returncode}: {result.stderr}"

    def test_security_scan_detects_known_issues(self) -> None:
        """S rules must detect at least 1 real security issue in the server source."""
        ruff = _find_ruff()
        result = subprocess.run(
            [ruff, "check", "--select", "S", "--statistics", "--no-fix", str(_REPO_ROOT / "app")],
            capture_output=True,
            text=True,
            cwd=str(_REPO_ROOT),
            timeout=60,
        )
        stats_lines = [line for line in result.stdout.strip().splitlines() if line.strip() and not line.startswith("Found")]
    ...
    def test_suppressed_rules_absent_from_output(self) -> None:
        ...
        ruff = _find_ruff()
        result = subprocess.run(
            [ruff, "check", "--select", "S", "--no-fix", str(_REPO_ROOT / "app")],
            capture_output=True,
            text=True,
            cwd=str(_REPO_ROOT),
            timeout=60,
