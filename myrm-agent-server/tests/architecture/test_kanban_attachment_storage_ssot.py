...
_TASK_RUNNER_STREAM = _REPO_ROOT / "app/services/kanban/task_runner_stream.py"
...
def test_kanban_multimodal_extract_uses_files_service_get_content() -> None:
    ...
    assert "files_service.get_file_content(" not in source, (
        "FilesService has no get_file_content(file_id); use get_content SSOT "
        "(see app/core/utils/media_file_reader.py)"
    )
