...
def test_resolve_image_generation_config_from_payload_snapshot() -> None:
    ...
    task = Task(
        ...
        payload=seal_task_payload_secrets(plain),
    )
...
def test_resolve_image_generation_config_from_sealed_gateway_payload() -> None:
    plain = {
        "prompt": "a skyline",
        "model": "flux-pro",
        "gateway_config": {
            ...
            "auth_token": "vk-sealed-gateway",
        },
    }
    sealed = seal_task_payload_secrets(plain)
    task = Task(
        task_id="img-test-gateway",
        task_type="image_generate",
        user_id="user-1",
        status=TaskStatus.PENDING,
        payload=sealed,
    ...
    assert config.gateway_config.auth_token == "vk-sealed-gateway"
