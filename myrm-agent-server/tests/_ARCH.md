...
- **muse-spark Responses wire Chrome E2E**：`tests/e2e/test_muse_spark_responses_wire_chrome_e2e.py`（PRIVATE+LIVE×1 SHPOIB：`muse-spark-1.2-contributor` OpenCode Go Responses wire；Turn1 强制 `web_search` → `TOOL_LOOP_OK`；Turn2 reasoning blob replay → `TURN2_OK`；前置 `~/.cursor-byok/opencode-go.json`）。**shared**：`./myrm ready --attach --chrome` → `./myrm test -m chrome_e2e -n0 ::test_muse_spark_responses_wire_two_turn_tool_loop`。**争用隔离**：`./myrm isolate muse-spark-wire restart --chrome` + `MYRM_E2E_ISOLATED_RUNTIME_ID=muse-spark-wire MYRM_E2E_FULL_ISOLATED=1` 同上。Harness 集成：`myrm-agent-harness/tests/toolkits/llms/adapters/wire/test_integration.py::test_chatlitellm_responses_stream`（`-m integration`）
