...
- WebUI E2E：MCP **chrome-devtools** + Myrm E2E Chrome `:9333`（`./myrm ready --chrome`）；禁止 `@playwright/test`。API 断言在 `tests/`（如 `test_capability_gap_integration.py`、`test_channel_notify_agent_stream_e2e.py`、`test_render_ui_agent_stream_e2e.py`）；MCP UI 胶水仅 `scripts/dev/subagent-dashboard-e2e-*.mjs`、`kanban-chrome-e2e-prepare.mjs`、`test-instinct-inbox-e2e.sh`（Instinct Inbox 依赖 `POST /api/v1/skills/drafts/test/seed-mock?agent_id=`，**不** mock `/approvals`；见 `scripts/dev/_ARCH.md`）
