...
- **migration readiness gap Chrome E2E**：`tests/e2e/test_migration_readiness_gap_chrome_e2e.py`（LIVE×1 SHPOIB：`mcp_warning` + `diagnostic_critical` 首聊 toast/SSE；**合批** `test_migration_readiness_gap_chrome_e2e_warning_and_critical_batch` 单 lease 双场景 · S2 `skip_warm_ui` + `provider_preverified` + fresh client/mux retry · signoff **r144 `1 passed` 700s** + 历史 r111/stress（见 `run-chrome-e2e-signoff.sh` · `CHROME_MCP_E2E.md`）；Verification：`test_capability_gap_integration.py` + live seed 三 variant）
