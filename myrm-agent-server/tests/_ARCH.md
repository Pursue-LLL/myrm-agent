...
| `e2e/test_kanban_chrome_e2e.py` | 模块 | Kanban Chrome MCP E2E（READ×10：看板渲染 + source_chat 深链过滤 + Drawer 附件 + Chat 成功卡片→看板 + stats bar running N/M + ready 排队 badge ±（占满显示/未满不显示）+ 真实执行排队释放闭环 + model_override UI 建卡 / 抽屉徽章编辑清除） |
...
- **Kanban Chrome E2E**：`tests/e2e/test_kanban_chrome_e2e.py`（`execution_mode=SHARED` ×10：看板列渲染 + `?source_chat=` 深链 + Chat 成功卡 → 过滤看板 + stats bar running `N/M` 并发占用 + ready 排队 badge ±（并发占满时显示 / 未满不显示）+ 真实执行排队释放闭环（UI 建卡 → dispatcher 真实 LLM 执行 → 占槽 badge 出现 → 释放后 badge 消失 → 双双完成）+ model_override UI 建卡/抽屉徽章编辑清除）。marker 已用现代 profile 字段（`execution_mode/access_scope/workload`），不再支持 legacy `private_backend` 字段（`resolve_e2e_session_profile.py` 强制）。
