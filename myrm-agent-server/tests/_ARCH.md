...
| `api/agent/test_stream_collector_file_mutation.py` | 模块 | StreamContentCollector `file_mutation_failed` → `extra_data.fileMutationFailures` |
...
- **Skill marketplace LIVE Chrome E2E**：`tests/e2e/test_skill_marketplace_live_agent_chrome_e2e.py`（LIVE×1：`skill_market_tool` 外部市场搜索；自定义 Agent system_prompt + `/?agentId=`；自然中文用户消息；API/UI 双路径断言；见 `scripts/dev/CHROME_MCP_E2E.md`）
...
- **migration readiness gap Chrome E2E**：`tests/e2e/test_migration_readiness_gap_chrome_e2e.py`（LIVE×1 SHPOIB：seed `mcp_warning` → anchor → first send toast/SSE；Verification：`test_capability_gap_integration.py::test_agent_stream_emits_migration_readiness_gap_sse`；见 `CHROME_MCP_E2E.md` §Gap Toast）
