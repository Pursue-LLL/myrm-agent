...
- **Wiki citation Chrome E2E**：`tests/e2e/test_wiki_citation_chrome_e2e.py`（READ lane ×2：`/chats/test/seed-citation-fixture` → citation 按钮 reload 持久；`/settings/wiki?agentId=` combobox）。READ 使用共享 `:8080`（`conftest.py:245-247` `private_backend=False`）；**新增 server 路由后须 `./myrm restart` 再跑 chrome e2e**。
