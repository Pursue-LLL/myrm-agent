...
class TestServerToolLayerRegistration:
    ...
    def test_media_tools_registered(self) -> None:
        for name in ("image_tool", "video_tool", "tts_generate"):
            assert _SERVER_TOOL_LAYERS.get(name) == ToolLayer.EXTERNAL, name

    def test_artifact_publish_registered(self) -> None:
        assert _SERVER_TOOL_LAYERS.get("artifact_publish") == ToolLayer.EXTERNAL
